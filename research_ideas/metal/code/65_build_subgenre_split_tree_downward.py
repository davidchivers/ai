...
FIGURE_PATH = OUTPUT_DIR / "subgenre_split_tree_downward.png"
SVG_PATH = OUTPUT_DIR / "subgenre_split_tree_downward.svg"
SUMMARY_PATH = OUTPUT_DIR / "subgenre_split_tree_downward_summary.md"
...
def downward_bend(token: str, parents: dict[str, str | None]) -> float:
...
def write_summary(stats: pd.DataFrame, parents: dict[str, str | None], base) -> None:
    ...
    with SUMMARY_PATH.open("w", encoding="utf-8") as handle:
        handle.write("# Downward subgenre split tree summary\n\n")
        handle.write(
            "This variant keeps the same lexical parent logic as `subgenre_split_tree`, but forces a "
            "top-down paper layout: `heavy metal` sits at the top and all descendants flow downward. "
            "Within each sibling group, earlier and larger branches are placed closer to the parent, "
            "while later or smaller offshoots appear farther down.\n\n"
        )
...
def plot(stats: pd.DataFrame, parents: dict[str, str | None], base) -> None:
    ...
    children, x_map, y_map = build_downward_positions(stats, parents, root_order, base)
    ...
    for token in stats["token"]:
        ...
        x0 = x_map[token]
        y = y_map[token]
        base.curve_run(ax, x0, x_max, y, downward_bend(token, parents), color=color, lw=lw, alpha=alpha)
    ...
    ax.set_title("Downward split tree of metal subgenre labels over time", fontsize=15, weight="bold")
    ...
    note = (
        "Illustrative label-based tree rather than a literal genealogy. Vertical position is layout only:\n"
        "the root sits at the top, and later or smaller offshoots are placed farther down within each subtree."
    )
