"""
Build scene network pilot from Wikipedia-sourced musician-band edge list.
Produces:
...
EDGE_FILE = os.path.join(DATA_DIR, "norwegian_bm_musician_band_edges.csv")
OUT_BAND_EDGES = os.path.join(DATA_DIR, "norwegian_bm_band_band_edges.csv")
OUT_STATS = os.path.join(DATA_DIR, "norwegian_bm_network_stats.md")
OUT_MUSICIAN_STATS = os.path.join(DATA_DIR, "norwegian_bm_musician_degree.csv")
...
band_band_edges = defaultdict(set)  # (band_a, band_b) -> set of shared musicians
...
# For each band, fraction of its neighbours that are also connected to each other
clustering_coeffs = {}
...
with open(OUT_STATS, "w", encoding="utf-8") as f:
    f.write("# Norwegian Black Metal Scene: Network Statistics (Pilot)\n\n")
    ...
    f.write(f"\n## Interpretation\n\n")
    f.write(f"This is a pilot network built from Wikipedia sources for the early Norwegian black metal scene.\n")
    f.write(f"The bipartite musician-band graph is projected onto band-band space via shared members.\n")
    f.write(f"Bridge musicians (those playing in multiple bands) are the key conduits for style transfer.\n\n")
    f.write(f"**Key observations to verify:**\n")
    f.write(f"- Which bands are most central (highest degree)?\n")
    f.write(f"- Do bridge musicians connect bands across cities or within cities?\n")
    f.write(f"- Is the clustering coefficient consistent with a small-world structure?\n")
    f.write(f"- Does the dense core of the network correspond to the bands that defined the genre?\n")
