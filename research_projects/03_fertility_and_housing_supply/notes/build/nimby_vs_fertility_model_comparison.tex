...
The more direct question for this project is not whether exogenous aging alone changes the sign of
the fertility channel. It is whether persistent housing-supply restriction can itself generate
lower future fertility. Figure 7 therefore replaces the earlier aging-only bridge with a simple
supply-tightening counterfactual: starting from a shared baseline, baseline political tightness is
raised permanently by `0.05` from the first post-baseline period onward.

This object is much closer to the intended mechanism. The top row shows a stylized endogenous
transition with no baby-boom shock. The bottom row feeds the medium-immigration age-share path
into a forward recursive bridge, but now the NIMBY shift starts after the shared 2020 baseline.
In the stylized transition, the house-price index rises from about `0.549` in baseline to about
`0.761` under the NIMBY shock by horizon `t = 40`, while the fertility rate falls from about
`0.248` to about `0.232`. By horizon `t = 79`, the same comparison is about `0.747` versus
`1.192` for house prices and about `0.233` versus `0.212` for fertility.

The forward bridge tells the same story in calendar time. In the medium-immigration scenario, the
projected house-price index rises from about `1.066` in baseline to about `1.163` under the NIMBY
shock by `2050`, while the fertility rate falls from about `0.194` to about `0.190`. By `2100`,
the same comparison is about `1.254` versus `1.767` for house prices and about `0.188` versus
`0.175` for fertility. So once housing scarcity is made persistent, the model does exactly what
the intended mechanism says: households still value children, but realized fertility falls because
high housing costs and crowding make family formation harder.

The result is also monotone in the size of the NIMBY shift. In the medium-immigration forward
bridge, moving from `theta0 + 0.03` to `theta0 + 0.10` lowers the projected 2100 fertility rate
from about `0.180` to about `0.167`, while the projected 2100 house-price index rises from about
`1.562` to about `2.281`, against a baseline of about `1.254`. Nearby changes in the price
sensitivity of fertility and the family-demand block leave the sign of that long-run result
unchanged.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{nimby_shock_fertility_counterfactual.png}
\caption{Persistent NIMBY shock. Starting from a shared baseline, a permanent increase in
...
aggregate US data, although the fit improvement is modest. The persistent NIMBY-shock
counterfactual then shows the other side of the mechanism: when housing scarcity itself is made
more persistent, future house prices rise and realized fertility falls. The
...
transitions, historical validation, bounded robustness, and a forward supply-tightening
counterfactual.
...
Seventh, the forward NIMBY-shock counterfactual uses the upstream forecast age weights from
\path{C:/Users/Dave_/Dropbox/Zac and David/Code/SteadyState/Mod_DynamicForecast/loopoutput_forecast.mat}
but applies a permanent increase in baseline political tightness from the first post-baseline year
onward. The reported comparison is still a bridge rather than the full project-02 forecast solver,
but it starts both lines from the same baseline and therefore speaks directly to the question of
whether persistent housing scarcity can lower future fertility.
