function run_nimby_annual_owner_grid_compromise_screen(mode)
% run_nimby_annual_owner_grid_compromise_screen.m -- Annual NIMBY screen
% that reopens the owner-entry grid after the low-entry broad screen
% exposed a clean vote-versus-balance-sheet tradeoff.
%
% This workflow starts from the low-entry broad-screen closeout:
%   - `housingmax = 10` improved debt / liquid-asset fit
%   - but it pushed the annual vote object further negative
% The next branch therefore fixes the low-leverage blocks:
%   1. uniform cohorts
%   2. benchmark CC
% and jointly screens:
%   1. housingmax
...
%
% Usage:
%   run_nimby_annual_owner_grid_compromise_screen
%   run_nimby_annual_owner_grid_compromise_screen('full')
%   run_nimby_annual_owner_grid_compromise_screen('smoke')
%
% Outputs:
...
writetable(target_blocks, fullfile(out_dir, 'nimby_annual_owner_grid_compromise_screen_target_blocks.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual owner-grid compromise candidate %d: %s ===\n', spec.candidate_id, spec.label);
    ...
    tmp = table( ...
        ...
        repmat(string(spec.housingmax_code), height(grid), 1), ...
        repmat(spec.housingmax_value, height(grid), 1), ...
        repmat(string(spec.ka_code), height(grid), 1), ...
        ...
        grid.debt_per_mass, ...
        grid.mass, ...
        'VariableNames', {'candidate_id', 'label', 'housingmax_code', 'housingmax_value', 'ka_code', 'theta_r_code', ...
        'rent_markup_code', 'a_price', 'vote_per_mass', 'debt_per_mass', 'mass'});
    ...
    tmp_blocks = table( ...
        ...
        repmat(spec.housingmax_value, height(candidate_blocks), 1), ...
        ...
        'VariableNames', {'candidate_id', 'label', 'housingmax_value', 'age_block', ...
    ...
    checkpoint_outputs(out_dir, settings, target_grid, target_crossing, target_eval, target_blocks, ...
        candidate_rows(1:i), path_table, block_table);
end

checkpoint_outputs(out_dir, settings, target_grid, target_crossing, target_eval, target_blocks, ...
    candidate_rows, path_table, block_table);
end
...
settings.housingmax_reference = 15.0;
...
switch char(settings.mode)
    case 'full'
        ...
        settings.housingmax_values = 10:15;
    case 'fast'
        ...
        settings.housingmax_values = [10, 12, 15];
    case 'smoke'
        solver_I = 16;
        solver_J = 8;
        settings.compare_price_grid = [1.50, 2.00, 2.50];
        settings.crossing_price_grid = 1.00:0.50:3.00;
        settings.housingmax_values = [10, 12, 15];
    otherwise
...
settings.five_year_overrides.housingmax = settings.housingmax_reference;
...
switch char(settings.mode)
    ...
    case 'fast'
        ka_specs = [ ...
            struct('code', "zero", 'label', "zero annual ka", 'value', settings.ka_zero), ...
            struct('code', "annualized", 'label', "annualized ka", 'value', settings.ka_annual), ...
            struct('code', "hold_5y", 'label', "hold 5y ka", 'value', settings.ka_five_year)];
        theta_specs = [ ...
            struct('code', "low", 'label', "low theta_r", 'value', settings.theta_r_low), ...
...
housing_specs = repmat(struct(), numel(settings.housingmax_values), 1);
for ih = 1:numel(settings.housingmax_values)
    hval = settings.housingmax_values(ih);
    housing_specs(ih).code = string(sprintf('h%02d', round(hval)));
    housing_specs(ih).label = string(sprintf('housingmax %.0f', hval));
    housing_specs(ih).value = hval;
end

specs = repmat(struct(), numel(housing_specs) * numel(ka_specs) * numel(theta_specs) * numel(rent_specs), 1);
idx = 0;
for ih = 1:numel(housing_specs)
    for ik = 1:numel(ka_specs)
        for it = 1:numel(theta_specs)
            for ir = 1:numel(rent_specs)
                idx = idx + 1;
                specs(idx).candidate_id = idx;
                specs(idx).housingmax_code = housing_specs(ih).code;
                specs(idx).housingmax_label = housing_specs(ih).label;
                specs(idx).housingmax_value = housing_specs(ih).value;
                specs(idx).ka_code = ka_specs(ik).code;
                specs(idx).ka_label = ka_specs(ik).label;
                ...
                specs(idx).label = sprintf('%s + %s + %s + %s', ...
                    housing_specs(ih).label, ka_specs(ik).label, theta_specs(it).label, rent_specs(ir).label);
            end
        end
    end
end
...
ov.housingmin = 0;
ov.housingmax = spec.housingmax_value;
ov.bmin = -20;
ov.bmax = 20;
...
row.housingmax_code = string(spec.housingmax_code);
...
function frontier_table = build_housingmax_frontier(candidates_table)
hvals = unique(candidates_table.housingmax_value(:));
frontier_table = table();
for i = 1:numel(hvals)
    idx = find(candidates_table.housingmax_value == hvals(i), 1, 'first');
    if ~isempty(idx)
        frontier_table = [frontier_table; candidates_table(idx, :)]; %#ok<AGROW>
    end
end
if ~isempty(frontier_table)
    frontier_table = sortrows(frontier_table, {'housingmax_value'}, {'ascend'});
end
end

function checkpoint_outputs(out_dir, settings, target_grid, target_crossing, target_eval, target_blocks, candidate_rows, path_table, block_table)
if isempty(candidate_rows)
    return;
end

candidates_table = sort_candidate_rows(candidate_rows);
writetable(candidates_table, fullfile(out_dir, 'nimby_annual_owner_grid_compromise_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'nimby_annual_owner_grid_compromise_screen_candidate_paths.csv'));
writetable(block_table, fullfile(out_dir, 'nimby_annual_owner_grid_compromise_screen_candidate_blocks.csv'));
write_report(fullfile(out_dir, 'nimby_annual_owner_grid_compromise_screen.md'), ...
...
fprintf(fid, '# Annual NIMBY owner-grid compromise screen\n\n');
fprintf(fid, ['This workflow reopens the annual owner-entry grid after the low-entry broad screen showed a clean tradeoff: ', ...
    'lower `housingmax` values improve debt / liquid-asset fit, while higher `housingmax` values make early-life price votes less negative. ', ...
    'It fixes uniform cohorts and benchmark `CC`, then jointly screens `housingmax`, `ka`, `theta_r`, and `rent_markup`.\n\n']);
...
fprintf(fid, '- fixed annual baseline: uniform cohorts, benchmark `CC`\n');
fprintf(fid, '- screened annual axes: `housingmax`, `ka`, `theta_r`, `rent_markup`\n');
fprintf(fid, '- housingmax grid in this run: `%s`\n', strjoin(string(settings.housingmax_values), ", "));
...
frontier_table = build_housingmax_frontier(candidates_table);
if ~isempty(frontier_table)
    fprintf(fid, '## Best candidate by owner grid\n\n');
    fprintf(fid, '| Housingmax | Candidate | Eval vote/mass | Eval debt/mass | Early vote RMSE | Early owner RMSE | Early liquid-asset RMSE | Score |\n');
    fprintf(fid, '|---:|---|---:|---:|---:|---:|---:|---:|\n');
    for i = 1:height(frontier_table)
        fprintf(fid, '| %.0f | %s | `%.6f` | `%.6f` | `%.6f` | `%.6f` | `%.6f` | `%.6f` |\n', ...
            frontier_table.housingmax_value(i), char(frontier_table.label(i)), ...
            frontier_table.eval_vote_per_mass(i), frontier_table.eval_debt_per_mass(i), ...
            frontier_table.early_vote_rmse(i), frontier_table.early_owner_share_rmse(i), ...
            frontier_table.early_liquid_asset_rmse(i), frontier_table.score(i));
    end
    fprintf(fid, '\n');
end
...
fprintf(fid, ['The full candidate ranking is saved in `nimby_annual_owner_grid_compromise_screen_candidates.csv`, ', ...
    'and the full age-block panel is saved in `nimby_annual_owner_grid_compromise_screen_candidate_blocks.csv`.\n']);
fclose(fid);
end
