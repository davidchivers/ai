function run_fertility_annual_relaxed_borrowing_screen(mode)
% run_fertility_annual_relaxed_borrowing_screen.m
%
% Small annual screen for relaxed owner borrowing constraints.
% Keeps the current annual no-mix working point fixed and varies only:
%   - owner_mortgage_CC (max owner LTV proxy)
%   - owner_entry_payment_to_income_cap (PTI underwriting)
%
% Note: values above 0.97 LTV are treated as stress tests rather than
% benchmark-realistic calibration points.
...
for i = 1:numel(candidates)
    fprintf('\n=== annual relaxed-borrowing candidate %d / %d ===\n', i, numel(candidates));
...
writetable(candidate_table, fullfile(out_dir, 'fertility_annual_relaxed_borrowing_screen_candidates.csv'));
write_summary_note(fullfile(out_dir, 'fertility_annual_relaxed_borrowing_screen.md'), candidate_table, settings, cfg);
end
...
switch char(settings.mode)
    case 'smoke'
        ...
        settings.owner_mortgage_CC_grid = [0.97, 0.985, 1.00];
        settings.owner_entry_payment_to_income_cap_grid = [0.40, 0.45, 0.50];
    case 'confirm'
        base_overrides.I = 20;
        base_overrides.J = 6;
        settings.crossing_price_grid = [1.50, 1.75, 2.00, 2.25, 2.50, 3.00, 5.00];
        settings.owner_mortgage_CC_grid = [0.97, 0.98, 0.985, 0.99, 1.00];
        settings.owner_entry_payment_to_income_cap_grid = [0.40, 0.425, 0.45, 0.475, 0.50];
    otherwise
...
for icc = 1:numel(settings.owner_mortgage_CC_grid)
    for ipti = 1:numel(settings.owner_entry_payment_to_income_cap_grid)
        idx = idx + 1;
        candidates(idx).candidate_id = idx; %#ok<AGROW>
        candidates(idx).owner_mortgage_CC = settings.owner_mortgage_CC_grid(icc);
        candidates(idx).owner_entry_payment_to_income_cap = settings.owner_entry_payment_to_income_cap_grid(ipti);
        candidates(idx).label = string(sprintf('owner mCC %.3f | PTI %.3f', ...
            candidates(idx).owner_mortgage_CC, candidates(idx).owner_entry_payment_to_income_cap));
    end
end
end
...
fprintf(fid, '# annual relaxed borrowing screen\n\n');
fprintf(fid, '- mode: `%s`\n', char(settings.mode));
fprintf(fid, '- fixed annual anchor and finance:\n');
fprintf(fid, '  - entry mortgage floor `%.2f`\n', settings.fixed_finance.owner_entry_mortgage_floor);
fprintf(fid, '  - owner mortgage spread `%.4f`\n', settings.fixed_finance.owner_mortgage_spread);
fprintf(fid, '  - amortization `%.2f`\n', settings.fixed_finance.owner_mortgage_amortization);
fprintf(fid, '  - current realistic benchmark LTV region is already around `0.965-0.97`\n');
fprintf(fid, '  - values above `0.97` here are stress tests\n\n');
...
fprintf(fid, '- label: `%s`\n', char(best.label(1)));
fprintf(fid, '- owner mortgage CC: `%.3f`\n', best.owner_mortgage_CC);
fprintf(fid, '- entry PTI cap: `%.3f`\n', best.owner_entry_payment_to_income_cap);
