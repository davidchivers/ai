...
SUMMARY_MD = BUILD_DIR / "nimby_annual_owner_grid_compromise_screen_handoff.md"
CANDIDATES_CSV = BUILD_DIR / "nimby_annual_owner_grid_compromise_screen_candidates.csv"
        ...
        out.append(
            {
                ...
                "housingmax_value": to_float(row.get("housingmax_value")),
            }
        )
    return out
...
def build_summary(args, candidates):
    ...
    lines = [
        "# Hamilton owner-grid compromise handoff",
        "",
        f"Generated: {now}",
        f"Job: `{args.job_id}`",
        f"State: `{args.state}`",
    ...
    if args.state == "COMPLETED" and args.exit_code == "0:0" and candidates:
        ...
        lines.extend(
            [
                f"- Completed owner-grid run with `{len(candidates)}` scored candidates.",
                ...
                f"- Best candidate housingmax: `{format_optional(best['housingmax_value'], 0)}`.",
            ]
        )
        if best["crossing_exists"] == 1 and best["crossing_is_unique"] == 1 and not math.isnan(best["crossing_refined_price"]):
            lines.append(f"- Unique crossing price: `{format_optional(best['crossing_refined_price'])}`.")
        else:
            lines.append("- No unique crossing appears in the scored owner-grid set.")
        lines.extend(
            [
                "",
                "## Interpretation",
                "",
                "- This handoff packet stops after collecting and summarizing the owner-grid result.",
                "- It does not auto-launch a second annual design branch while you are away.",
            ]
        )
    else:
        lines.extend(
            [
                "- The owner-grid run did not finish cleanly inside this handoff packet.",
...
def prepend_status_entry(text, args, candidates):
    ...
    lines = [
        f"- Last updated: {date_tag} (away handoff collected Hamilton owner-grid result)",
        ...
        "    - `notes/build/nimby_annual_owner_grid_compromise_screen_handoff.md`",
    ]
    if args.remote_run_dir:
        lines.append(f"  - remote run dir: `{args.remote_run_dir}`")
    if args.state == "COMPLETED" and args.exit_code == "0:0" and candidates:
        ...
        lines.extend(
            [
                "  - stopping rule hit:",
                "    - collected and summarized one clean owner-grid milestone",
        ...
        lines.extend(
            [
                "  - blocker:",
                "    - the owner-grid run did not finish cleanly, so the handoff packet stopped after syncing the available logs and outputs",
            ]
        )
...
def replace_next_tasks(text, args, candidates):
    if args.state == "COMPLETED" and args.exit_code == "0:0" and candidates:
        ...
        if best["crossing_exists"] == 1 and best["crossing_is_unique"] == 1 and not math.isnan(best["crossing_refined_price"]):
            replacement = (
                "## Next 3 Tasks\n\n"
                f"1. Read `notes/build/nimby_annual_owner_grid_compromise_screen_handoff.md` and confirm whether promoted candidate `{best['label']}` is strong enough to treat as the new annual leader.\n"
            ...
            replacement = (
                "## Next 3 Tasks\n\n"
                "1. Read `notes/build/nimby_annual_owner_grid_compromise_screen_handoff.md` and the full `nimby_annual_owner_grid_compromise_screen.md` as the canonical closeout for this owner-grid screen.\n"
                "2. Decide the next annual design branch after reviewing the full owner-grid result; do not auto-extend to a second branch from this handoff packet.\n"
                "3. Keep annual fertility and RE / transition work off the table until the next steady-state branch is chosen from the collected owner-grid evidence.\n\n"
                "## Blockers"
            )
    else:
        replacement = (
            "## Next 3 Tasks\n\n"
            "1. Read `notes/build/nimby_annual_owner_grid_compromise_screen_handoff.md` and the synced Hamilton logs to diagnose why the owner-grid run did not finish cleanly.\n"
            "2. Decide whether to rerun the same owner-grid screen or narrow the failure before launching another annual job.\n"
...
def prepend_memory_entry(text, args, candidates):
    date_tag = datetime.now().strftime("%Y-%m-%d")
    lines = [
        f"### Session: {date_tag} (away handoff collected Hamilton owner-grid result)",
    ...
    if args.state == "COMPLETED" and args.exit_code == "0:0" and candidates:
        ...
        lines.extend(
            [
                "- Main read from the completed owner-grid handoff:",
                f"  - scored candidates collected: `{len(candidates)}`",
                f"  - best candidate: `{best['label']}`",
                f"  - eval vote-per-mass: `{format_optional(best['eval_vote_per_mass'])}`",
                f"  - eval debt-per-mass: `{format_optional(best['eval_debt_per_mass'])}`",
        ...
        lines.extend(
            [
                "- Blocker read:",
                "  - the owner-grid run did not finish cleanly, so the away packet stopped after syncing the available logs and outputs",
            ]
        )
