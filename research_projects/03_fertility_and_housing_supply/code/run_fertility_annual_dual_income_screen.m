function run_fertility_annual_dual_income_screen(mode)
% run_fertility_annual_dual_income_screen.m
%
% Reduced-form annual test for dual-income mortgage qualification.
% This keeps the current no-mix annual working point fixed and only changes
% the income multiple used in the owner-entry PTI underwriting rule.
...
for i = 1:numel(candidates)
    fprintf('\n=== annual dual-income candidate %d / %d ===\n', i, numel(candidates));
...
writetable(candidate_table, fullfile(out_dir, 'fertility_annual_dual_income_screen_candidates.csv'));
write_summary_note(fullfile(out_dir, 'fertility_annual_dual_income_screen.md'), candidate_table, settings, cfg);
end
...
switch char(settings.mode)
    case 'smoke'
        ...
        settings.pre_birth_multiplier_grid = [1.00, 1.25, 1.50, 1.75];
        settings.after_birth_multiplier_grid = [1.00, 1.10, 1.20, 1.30];
    case 'confirm'
        base_overrides.I = 20;
        base_overrides.J = 6;
        settings.crossing_price_grid = [1.50, 1.75, 2.00, 2.25, 2.50, 3.00, 5.00];
        settings.pre_birth_multiplier_grid = [1.00, 1.15, 1.25, 1.40, 1.50, 1.75];
        settings.after_birth_multiplier_grid = [1.00, 1.10, 1.20, 1.30, 1.40];
    otherwise
...
candidates = repmat(struct( ...
    'candidate_id', NaN, ...
    'owner_entry_income_multiplier', NaN, ...
    'owner_entry_income_multiplier_after_birth', NaN, ...
    'label', ""), 0, 1);

for ipre = 1:numel(settings.pre_birth_multiplier_grid)
    for ipost = 1:numel(settings.after_birth_multiplier_grid)
        pre_mult = settings.pre_birth_multiplier_grid(ipre);
        post_mult = settings.after_birth_multiplier_grid(ipost);
        if post_mult > pre_mult
            continue;
        end
        idx = idx + 1;
        candidates(idx).candidate_id = idx; %#ok<AGROW>
        candidates(idx).owner_entry_income_multiplier = pre_mult;
        candidates(idx).owner_entry_income_multiplier_after_birth = post_mult;
        candidates(idx).label = string(sprintf('pre-birth PTI income x%.2f | after-birth x%.2f', pre_mult, post_mult));
    end
end
end
...
row.owner_entry_income_multiplier = candidate.owner_entry_income_multiplier;
row.owner_entry_income_multiplier_after_birth = candidate.owner_entry_income_multiplier_after_birth;
...
overrides.owner_entry_income_multiplier = candidate.owner_entry_income_multiplier;
overrides.owner_entry_income_multiplier_after_birth = candidate.owner_entry_income_multiplier_after_birth;
...
vote_sign = sign(vote_grid);
vote_sign(vote_sign == 0) = NaN;
...
fprintf(fid, '# annual dual-income qualification screen\n\n');
fprintf(fid, '- mode: `%s`\n', char(settings.mode));
...
fprintf(fid, '- label: `%s`\n', char(best.label(1)));
fprintf(fid, '- pre-birth qualifying income multiplier: `%.2f`\n', best.owner_entry_income_multiplier);
fprintf(fid, '- after-birth qualifying income multiplier: `%.2f`\n', best.owner_entry_income_multiplier_after_birth);
