...
$scriptPath = Join-Path $PSScriptRoot "hamilton_fertility_benchmark_handoff.ps1"
$launcherStamp = Get-Date -Format "yyyyMMdd_HHmmss"
$launcherStdout = Join-Path $logsRoot "hamilton_fertility_benchmark_handoff_launcher_$launcherStamp.stdout.log"
$launcherStderr = Join-Path $logsRoot "hamilton_fertility_benchmark_handoff_launcher_$launcherStamp.stderr.log"
$activeRunPath = Join-Path $logsRoot "active_hamilton_fertility_benchmark_handoff.txt"
...
@(
    ...
    "latest_pointer=$(Join-Path $logsRoot 'latest_hamilton_fertility_benchmark_handoff.txt')"
) | Set-Content -Path $activeRunPath

Write-Output "Started Hamilton fertility benchmark handoff."
