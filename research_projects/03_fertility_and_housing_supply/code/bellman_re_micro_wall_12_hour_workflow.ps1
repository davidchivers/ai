param(
    [string]$RunLabel = "bellman_re_micro_wall_12_hour_workflow",
...
$canonicalNotePath = Join-Path $projectRoot "notes/build/compiled_sidecar_bellman_re_micro_wall_packet.md"
...
try {
    ...
    $microWallSeed = [double[]]@(1.7303548376, 1.7483599506, 1.7526877876, 2.1532712772)
    $microVariantBaseArgs = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "6")

    @(
        "Objective: run a bounded 12-hour compiled-sidecar Bellman RE packet around the improved micro-wall near q3=1.7526878.",
        "Run label: $RunLabel",
        "Scope lock:",
        " - project 03 only",
        ...
        "Deliverables:",
        " - exact micro-wall probe CSVs around the improved local endpoint",
        " - bounded continuation sweep under the best wall-variant controls",
        " - one canonical packet note at notes/build/compiled_sidecar_bellman_re_micro_wall_packet.md",
        "Stopping rule: stop after the packet note exists, or at the first timeout/error."
    ) | Set-Content -Path $manifestPath

    Write-Status ("RUN_DIR {0}" -f $runDir)

    $exactBaseline = Invoke-ExactEval -QPath $microWallSeed
    Write-Status ("BASELINE_EXACT maxres={0:F10}" -f $exactBaseline.maxres)

    $q2Micro = [double[]]@(-0.00125, -0.000625, -0.0003125, 0.0, 0.0003125, 0.000625, 0.00125)
    $q3Ultra = [double[]]@(-0.000625, -0.0003125, -0.00015625, 0.0, 0.00015625, 0.0003125, 0.000625)
    $q4Ultra = [double[]]@(-0.000625, -0.0003125, -0.00015625, 0.0, 0.00015625, 0.0003125, 0.000625)
    $q2Micro3D = [double[]]@(-0.000625, 0.0, 0.000625)
    $q3Micro3D = [double[]]@(-0.0003125, 0.0, 0.0003125)
    $q4Micro3D = [double[]]@(-0.0003125, 0.0, 0.0003125)

    $lineQ3 = Run-ExactLine1D -StepName "01_micro_wall_exact_line_q3" `
        -CenterQ $microWallSeed `
        -IndexA 2 `
        -DeltasA $q3Ultra

    $grid23 = Run-ExactGrid2D -StepName "02_micro_wall_exact_grid_q23_fine" `
        -CenterQ $microWallSeed `
        -IndexA 1 `
        -IndexB 2 `
        -DeltasA $q2Micro `
    ...
    $grid34 = Run-ExactGrid2D -StepName "03_micro_wall_exact_grid_q34_fine" `
        -CenterQ $microWallSeed `
        -IndexA 2 `
        -IndexB 3 `
        -DeltasA $q3Ultra `
    ...
    $grid234 = Run-ExactGrid3D -StepName "04_micro_wall_exact_grid_q234_micro" `
        -CenterQ $microWallSeed `
        -IndexA 1 `
        -IndexB 2 `
        -IndexC 3 `
    ...
    if ([double]$bestExactRow.maxres -lt ($exactBaseline.maxres - 1.0e-6)) {
        $bestExactContinuation = Run-StagedCase -CaseName "05_best_micro_exact_candidate" `
            -InitialQPath $bestExactSeed `
            -StageIters @(4, 4) `
            -ExtraArgs $microVariantBaseArgs
    }

    $microVariants = @(
        [pscustomobject]@{ name = "micro_base_p3_r6"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "6") },
        [pscustomobject]@{ name = "micro_p3_r6_step00625"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "6", "--stall-probe-initial-step-fraction", "0.0625") },
        [pscustomobject]@{ name = "micro_p3_r6_step0125"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "6", "--stall-probe-initial-step-fraction", "0.125") },
        [pscustomobject]@{ name = "micro_p3_r8"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "8") },
        [pscustomobject]@{ name = "micro_p4_r6"; extra = @("--max-stall-probe-periods", "4", "--max-stall-probe-rounds", "6") },
        [pscustomobject]@{ name = "micro_p4_r8"; extra = @("--max-stall-probe-periods", "4", "--max-stall-probe-rounds", "8") }
    )

    $variantResults = New-Object System.Collections.Generic.List[object]
    foreach ($variant in $microVariants) {
        $variantCase = Run-StagedCase -CaseName ("06_variant_{0}" -f $variant.name) `
            -InitialQPath $microWallSeed `
            -StageIters @(4) `
    ...
    $variantCsvPath = Join-Path $runDir "06_micro_variant_sweep.csv"
    $variantResults | Sort-Object maxres, name | Export-Csv -Path $variantCsvPath -NoTypeInformation
    Write-Status ("DONE 06_micro_variant_sweep rows={0}" -f $variantResults.Count)
    ...
    if ([double]$bestVariant.maxres -lt ($exactBaseline.maxres - 1.0e-6)) {
        ...
        $bestVariantContinuation = Run-StagedCase -CaseName "07_best_micro_variant_continuation" `
    ...
    if (($null -ne $bestVariantContinuation) -and ($bestVariantContinuation.final_maxres -lt ($bestVariant.maxres - 1.0e-6))) {
        $continuationGrid = Run-ExactGrid2D -StepName "08_best_micro_variant_exact_grid_q34_followup" `
    ...
    $noteLines.Add("# Compiled sidecar Bellman RE micro-wall 12-hour packet")
    $noteLines.Add("")
    ...
    $noteLines.Add("- improved micro-wall seed: [1.730355, 1.748360, 1.752688, 2.153271]")
    $noteLines.Add("- no MATLAB, no annual branch, no benchmark-grid RE push")
    $noteLines.Add("")
    $noteLines.Add("## Exact micro-wall baseline")
    $noteLines.Add("")
    ...
    $noteLines.Add("")
    $noteLines.Add("## Exact micro-wall probes")
    $noteLines.Add("")
    ...
    if ($null -ne $bestExactContinuation) {
        ...
        $noteLines.Add("- no exact micro-wall candidate beat the baseline by more than the acceptance tolerance")
    }
    $noteLines.Add("")
    $noteLines.Add("## Micro-wall variant sweep")
    $noteLines.Add("")
    ...
    $noteLines.Add("")
    $noteLines.Add("## Best micro-wall variant continuation")
    $noteLines.Add("")
    if ($null -ne $bestVariantContinuation) {
        $noteLines.Add(("- best micro-wall variant: {0}" -f $bestVariant.name))
        ...
        $noteLines.Add("- no micro-wall variant beat the baseline enough to justify a continuation run")
    }
    $noteLines.Add("")
    ...
    $noteLines.Add("## Read")
    $noteLines.Add("")
    $noteLines.Add("- This packet is meant to decide whether the improved q3 micro-step opens a real local continuation beyond the old ~0.04744 wall.")
    ...
    @(
        "Bellman RE micro-wall 12-hour workflow stopped with an error.",
