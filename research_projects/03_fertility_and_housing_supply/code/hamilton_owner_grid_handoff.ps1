...
function Find-RemoteRunDir {
    ...
    $cmd = "find $RemoteRoot/annual_nimby_bundle_*/03_fertility_and_housing_supply/notes/build/logs -maxdepth 1 -type d -name 'annual_nimby_owner_grid_compromise_screen_hpc_*' | sort | tail -n 1"
...
function Sync-RemoteOutputs {
    ...
    & scp -o BatchMode=yes "hamilton8:$remoteProjectRoot/notes/build/nimby_annual_owner_grid_compromise_screen*" "$localBuild\"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to sync owner-grid output files from Hamilton."
    }

    if (-not [string]::IsNullOrWhiteSpace($RemoteRunDir)) {
        & scp -o BatchMode=yes -r "hamilton8:$RemoteRunDir" "$localLogs\"
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to sync remote owner-grid run directory from Hamilton."
        }
    }
}
...
$runDir = Join-Path $logsRoot "hamilton_owner_grid_handoff_$runStamp"
New-Item -ItemType Directory -Path $runDir | Out-Null

$statusPath = Join-Path $runDir "status.txt"
$summaryPath = Join-Path $runDir "summary.txt"
$manifestPath = Join-Path $runDir "manifest.txt"
$latestPointerPath = Join-Path $logsRoot "latest_hamilton_owner_grid_handoff.txt"
$activePointerPath = Join-Path $logsRoot "active_hamilton_owner_grid_handoff.txt"

@(
    "Objective: monitor Hamilton owner-grid job and stop after one clean milestone."
...
while ((Get-Date) -lt $deadline) {
    ...
    if ($record.State -in @("COMPLETED", "FAILED", "CANCELLED", "TIMEOUT", "OUT_OF_MEMORY", "BOOT_FAIL", "NODE_FAIL", "PREEMPTED")) {
        ...
        $completionScript = Join-Path $PSScriptRoot "complete_owner_grid_handoff.py"
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Completion script failed for owner-grid handoff."
        }

        @(
            "Hamilton owner-grid handoff completed."
            "Job id: $JobId"
            "State: $($record.State)"
            ...
            "Summary note: $(Join-Path $projectRoot 'notes/build/nimby_annual_owner_grid_compromise_screen_handoff.md')"
...
@(
    "# Hamilton owner-grid handoff monitor timeout"
    ""
    "Job id: $JobId"
...
) | Set-Content -Path (Join-Path $projectRoot "notes/build/nimby_annual_owner_grid_compromise_screen_handoff.md")

@(
    "Hamilton owner-grid handoff monitor timed out."
    "Job id: $JobId"
    "Run dir: $runDir"
    "Summary note: $(Join-Path $projectRoot 'notes/build/nimby_annual_owner_grid_compromise_screen_handoff.md')"
) | Set-Content -Path $summaryPath
