function run_fertility_annual_no_crowding_test()
% run_fertility_annual_no_crowding_test.m
%
% Clean falsification test on the live annual no-mix baseline:
% keep child utility and the direct price-sensitive birth cost, but remove
% the crowding block from effective housing services.
...
for i = 1:numel(candidates)
    fprintf('\n=== annual no-crowding test candidate %d / %d ===\n', i, numel(candidates));
...
writetable(candidate_table, fullfile(out_dir, 'fertility_annual_no_crowding_test_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'fertility_annual_no_crowding_test_path.csv'));
write_summary_note(fullfile(out_dir, 'fertility_annual_no_crowding_test.md'), candidate_table, path_table, settings, cfg);
end
...
settings.baseline_psi_crowd = 0.70;
end
...
candidates(1).label = "baseline crowding";
candidates(1).lambda_crowd = settings.anchor.lambda_crowd;
candidates(1).psi_crowd = settings.baseline_psi_crowd;

candidates(2).candidate_id = 2;
candidates(2).label = "no crowding";
candidates(2).lambda_crowd = 0.0;
candidates(2).psi_crowd = 0.0;
end
...
row.psi_crowd = candidate.psi_crowd;
...
overrides.psi_crowd = candidate.psi_crowd;
...
fprintf(fid, '# annual no-crowding test\n\n');
fprintf(fid, 'This is a clean falsification test on the live annual no-mix baseline. It keeps child utility, the direct price-sensitive birth cost, and the finance block fixed, and only removes the crowding term from effective housing services.\n\n');
...
fprintf(fid, '## Compared cases\n\n');
fprintf(fid, '| Case | lambda crowd | psi crowd |\n');
fprintf(fid, '|---|---:|---:|\n');
for i = 1:height(candidate_table)
    row = candidate_table(i, :);
    fprintf(fid, '| `%s` | %.3f | %.3f |\n', char(row.label(1)), row.lambda_crowd, row.psi_crowd);
end
fprintf(fid, '\n');
...
fprintf(fid, '| Case | House price | Mean age | Share 30+ | Childless @50 | Owner 25-34 | Vote/mass |\n');
fprintf(fid, '|---|---:|---:|---:|---:|---:|---:|\n');
for i = 1:height(path_table)
    row = path_table(i, :);
    fprintf(fid, '| `%s` | %.2f | %.2f | %.3f | %.3f | %.3f | %.3f |\n', ...
        char(row.label(1)), row.house_price, row.mean_age_first_birth, row.share_first_birth_30_plus, ...
        row.childless_share_at_50, row.owner_share_25_34, row.vote_per_mass);
end
fprintf(fid, '\n');

baseline = candidate_table(candidate_table.candidate_id == 1, :);
nocrowd = candidate_table(candidate_table.candidate_id == 2, :);
fprintf(fid, '## Read\n\n');
fprintf(fid, '- Removing crowding keeps child utility and the direct price-sensitive birth cost alive, but removes the anticipatory space penalty.\n');
fprintf(fid, '- Baseline mortgaged-owner share under `35`: `%.3f`.\n', baseline.eval_mortgaged_owner_share_under_35);
fprintf(fid, '- No-crowding mortgaged-owner share under `35`: `%.3f`.\n', nocrowd.eval_mortgaged_owner_share_under_35);
fprintf(fid, '- Baseline unique crossing: `%d` at about `%.3f`.\n', baseline.crossing_is_unique, baseline.crossing_refined_price);
fprintf(fid, '- No-crowding unique crossing: `%d` at about `%.3f`.\n', nocrowd.crossing_is_unique, nocrowd.crossing_refined_price);
fprintf(fid, '- If the leverage miss remains after crowding is removed, then the annual problem is not mainly excessive family crowding.\n');
end
