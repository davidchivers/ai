function run_fertility_annual_birth_housing_mismatch_screen(mode)
% run_fertility_annual_birth_housing_mismatch_screen.m
%
% Fast annual prototype for a different missing mechanism: once children
% are present, staying in too little housing creates a direct mismatch
% penalty. The penalty depends on chosen housing services, so it pushes the
% post-birth housing adjustment margin rather than acting as a scalar birth
% tax.
%
% Outputs:
...
%   notes/build/fertility_annual_birth_housing_mismatch_screen_target_ranges.csv
...
writetable(target_table, fullfile(out_dir, 'fertility_annual_birth_housing_mismatch_screen_target_ranges.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual birth-housing-mismatch candidate %d: %s ===\n', spec.candidate_id, char(spec.label));
...
switch char(settings.mode)
    case 'fast'
        ...
        settings.family_space_floor_grid = [0.25, 0.50, 0.75, 1.00, 1.25, 1.50];
        settings.family_space_penalty_grid = [0.05, 0.10, 0.20, 0.30];
    case 'confirm'
        base_overrides.I = 32;
        base_overrides.J = 12;
        settings.price_grid = [1.50, 1.75, 2.00, 2.25, 2.50];
        settings.crossing_price_grid = [1.50, 1.75, 2.00, 2.25, 2.50, 2.75, 3.00, 3.50, 4.00, 5.00, 6.00];
        settings.family_space_floor_grid = [0.25, 0.50, 0.75, 1.00, 1.25];
        settings.family_space_penalty_grid = [0.05, 0.10, 0.20, 0.30];
    otherwise
        base_overrides.I = 16;
        base_overrides.J = 4;
        ...
        settings.family_space_floor_grid = [0.50, 1.00, 1.50];
        settings.family_space_penalty_grid = [0.10, 0.20, 0.40];
end
...
baseline = struct( ...
    'family_space_floor', 0.00, ...
    'family_space_penalty', 0.00, ...
    'is_baseline', 1);

idx = idx + 1;
specs(idx) = build_spec( ...
    idx, ...
    "baseline annual housing-mismatch block", ...
    baseline.family_space_floor, ...
    baseline.family_space_penalty, ...
    baseline.is_baseline);

for ifloor = 1:numel(settings.family_space_floor_grid)
    for ipen = 1:numel(settings.family_space_penalty_grid)
        floor_value = settings.family_space_floor_grid(ifloor);
        penalty_value = settings.family_space_penalty_grid(ipen);
        idx = idx + 1;
        label = sprintf('family space floor %.2f | mismatch penalty %.2f', floor_value, penalty_value);
        specs(idx) = build_spec(idx, label, floor_value, penalty_value, 0);
    end
end
end

function spec = build_spec(candidate_id, label, family_space_floor, family_space_penalty, is_baseline)
spec = struct();
spec.candidate_id = candidate_id;
spec.label = string(label);
spec.family_space_floor = family_space_floor;
spec.family_space_penalty = family_space_penalty;
...
overrides.family_space_floor = spec.family_space_floor;
overrides.family_space_penalty = spec.family_space_penalty;
...
row.family_space_floor = spec.family_space_floor;
row.family_space_penalty = spec.family_space_penalty;
...
path_rows = table( ...
    ...
    repmat(spec.family_space_floor, n, 1), ...
    repmat(spec.family_space_penalty, n, 1), ...
    settings.price_grid(:), ...
    avg_birth_rate, ...
    ...
    debtstock, ...
    mass_error, ...
    'VariableNames', {'candidate_id', 'label', 'family_space_floor', 'family_space_penalty', ...
...
crossing_path_rows = table( ...
    ...
    repmat(spec.family_space_floor, height(crossing_results), 1), ...
    repmat(spec.family_space_penalty, height(crossing_results), 1), ...
...
row = struct( ...
    'candidate_id', NaN, ...
    'label', "", ...
    'family_space_floor', NaN, ...
    'family_space_penalty', NaN, ...
    'is_baseline', NaN, ...
...
writetable(candidates_table, fullfile(out_dir, 'fertility_annual_birth_housing_mismatch_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'fertility_annual_birth_housing_mismatch_screen_candidate_paths.csv'));
writetable(crossing_path_table, fullfile(out_dir, 'fertility_annual_birth_housing_mismatch_screen_crossing_paths.csv'));
write_report(fullfile(out_dir, 'fertility_annual_birth_housing_mismatch_screen.md'), ...
...
fprintf(fid, '# Annual birth-triggered housing-mismatch screen\n\n');
fprintf(fid, ['This workflow holds the usable annual fertility anchor fixed and adds a child-dependent housing mismatch term. ', ...
    'The penalty depends on chosen housing services, so the mechanism pushes households to adjust housing after children arrive rather than simply adding another direct birth cost.\n\n']);
...
fprintf(fid, '- the older annual rescue attempts were still mostly scalar owner-access or renter-wedge tweaks\n');
fprintf(fid, '- this screen adds a different channel: if children are present and the chosen housing bundle is too small, continuation utility falls\n');
fprintf(fid, '- if vote moves toward zero without blowing up timing, that would support the idea that annual time is missing a state-contingent housing adjustment margin rather than just a scalar price deterrence term\n');
fclose(fid);
end
