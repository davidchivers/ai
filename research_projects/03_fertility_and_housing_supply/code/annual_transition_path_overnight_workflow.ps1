param(
    ...
    [ValidateRange(1, 3)]
    [int]$EndAtStep = 3,
    [string]$RunLabel = "annual_transition_path_overnight",
...
try {
    ...
    @(
        "Objective: run one bounded overnight transition-path preparation chain for the annual branch.",
        "Run label: $RunLabel",
        "Scope lock:",
        " - stay inside the annual transition-path branch only",
        " - do not rerun steady-state finance, entrant-mixture, or mortgage-state screens",
        ...
        "Stopping rule: stop after the three-step transition chain finishes once, or after the first logged blocker/timeout."
    ) | Set-Content -Path $manifestPath

    $steps = @(
        @{ Index = 1; Name = "01_transition_prep"; Command = "run_fertility_annual_transition_path_prep"; TimeoutHours = $Step1TimeoutHours },
        @{ Index = 2; Name = "02_transition_bucket_mapping"; Command = "run_fertility_annual_transition_path_bucket_mapping"; TimeoutHours = $Step2TimeoutHours },
        @{ Index = 3; Name = "03_transition_state_grid_mapping"; Command = "run_fertility_annual_transition_path_state_grid_mapping"; TimeoutHours = $Step3TimeoutHours }
    )
    ...
    @(
        "Annual transition-path overnight workflow completed successfully.",
        "Run directory: $runDir",
        "Latest pointer: $latestPointer",
        "Outputs refreshed: transition-path prep, bucket mapping, and state-grid mapping files."
    ...
    @(
        "Annual transition-path overnight workflow stopped with an error.",
        "Run directory: $runDir",
        "Error: $message"
    ) | Set-Content -Path $summaryPath
