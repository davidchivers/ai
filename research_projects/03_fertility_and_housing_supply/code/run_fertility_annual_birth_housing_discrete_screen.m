function run_fertility_annual_birth_housing_discrete_screen(mode)
% run_fertility_annual_birth_housing_discrete_screen.m
%
% Sharper annual prototype for a missing mechanism: once children are
% present, staying below a family-space threshold triggers a fixed utility
% loss. This is meant to approximate a discrete birth-triggered upgrade
% pressure rather than a smooth linear services penalty.
%
% Outputs:
...
%   notes/build/fertility_annual_birth_housing_discrete_screen_target_ranges.csv
...
writetable(target_table, fullfile(out_dir, 'fertility_annual_birth_housing_discrete_screen_target_ranges.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual birth-housing-discrete candidate %d: %s ===\n', spec.candidate_id, char(spec.label));
...
switch char(settings.mode)
    case 'fast'
        ...
        settings.family_space_floor_grid = [2.0, 3.0, 4.0];
        settings.family_space_fixed_penalty_grid = [0.10, 0.25, 0.50, 1.00];
    case 'confirm'
        base_overrides.I = 32;
        base_overrides.J = 12;
        settings.price_grid = [1.50, 1.75, 2.00, 2.25, 2.50];
        ...
        settings.family_space_floor_grid = [2.0, 3.0, 4.0];
        settings.family_space_fixed_penalty_grid = [0.10, 0.25, 0.50, 0.75];
    otherwise
        base_overrides.I = 16;
        base_overrides.J = 4;
        ...
        settings.family_space_floor_grid = [2.0, 3.0, 4.0];
        settings.family_space_fixed_penalty_grid = [0.10, 0.30, 0.60];
end
...
idx = idx + 1;
specs(idx) = build_spec(idx, "baseline annual discrete housing block", 0.0, 0.0, 1);

for ifloor = 1:numel(settings.family_space_floor_grid)
    for ipen = 1:numel(settings.family_space_fixed_penalty_grid)
        floor_value = settings.family_space_floor_grid(ifloor);
        penalty_value = settings.family_space_fixed_penalty_grid(ipen);
        idx = idx + 1;
        label = sprintf('family space floor %.2f | fixed mismatch penalty %.2f', floor_value, penalty_value);
        specs(idx) = build_spec(idx, label, floor_value, penalty_value, 0);
    end
end
end

function spec = build_spec(candidate_id, label, family_space_floor, family_space_fixed_penalty, is_baseline)
spec = struct();
spec.candidate_id = candidate_id;
spec.label = string(label);
spec.family_space_floor = family_space_floor;
spec.family_space_fixed_penalty = family_space_fixed_penalty;
...
overrides.family_space_penalty = 0.0;
overrides.family_space_fixed_penalty = spec.family_space_fixed_penalty;
...
row.family_space_fixed_penalty = spec.family_space_fixed_penalty;
...
path_rows = table( ...
    ...
    repmat(spec.family_space_fixed_penalty, n, 1), ...
    settings.price_grid(:), ...
    avg_birth_rate, ...
    ...
    debtstock, ...
    mass_error, ...
    'VariableNames', {'candidate_id', 'label', 'family_space_floor', 'family_space_fixed_penalty', ...
...
crossing_path_rows = table( ...
    ...
    repmat(spec.family_space_fixed_penalty, height(crossing_results), 1), ...
...
row = struct( ...
    ...
    'family_space_fixed_penalty', NaN, ...
    'is_baseline', NaN, ...
...
writetable(candidates_table, fullfile(out_dir, 'fertility_annual_birth_housing_discrete_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'fertility_annual_birth_housing_discrete_screen_candidate_paths.csv'));
writetable(crossing_path_table, fullfile(out_dir, 'fertility_annual_birth_housing_discrete_screen_crossing_paths.csv'));
write_report(fullfile(out_dir, 'fertility_annual_birth_housing_discrete_screen.md'), ...
...
fprintf(fid, '# Annual birth-triggered discrete housing screen\n\n');
fprintf(fid, ['This workflow holds the usable annual fertility anchor fixed and adds a discrete family-space penalty. ', ...
    'If housing services stay below a child-dependent threshold, the household takes a fixed utility hit. ', ...
    'This is a sharper proxy for birth-triggered upgrade pressure than the smooth linear mismatch term.\n\n']);
...
fprintf(fid, '- the smooth linear mismatch term was effectively inactive in the annual baseline region\n');
fprintf(fid, '- this screen sharpens the same mechanism family into a discrete upgrade-pressure penalty\n');
fprintf(fid, '- if this still fails, the next step should probably be a tenure-linked or balance-sheet mechanism rather than another housing-floor variant\n');
fclose(fid);
end
