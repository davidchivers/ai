function run_nimby_annual_low_entry_broad_screen(mode)
% run_nimby_annual_low_entry_broad_screen.m -- Broader annual NIMBY screen
% that keeps the best access-side improvement in place while reopening the
% structural axes that the narrow low-entry vote-side rerun held fixed.
%
% This workflow starts from the current best direct access-side read:
%   benchmark CC + housingmax = 10
% and then jointly screens:
%   1. cohort weights
%   2. ka
%   3. theta_r
%   4. rent_markup
%
% Usage:
%   run_nimby_annual_low_entry_broad_screen
%   run_nimby_annual_low_entry_broad_screen('full')
%   run_nimby_annual_low_entry_broad_screen('smoke')
%
% Outputs:
...
writetable(target_blocks, fullfile(out_dir, 'nimby_annual_low_entry_broad_screen_target_blocks.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual low-entry broad candidate %d: %s ===\n', spec.candidate_id, spec.label);
...
writetable(candidates_table, fullfile(out_dir, 'nimby_annual_low_entry_broad_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'nimby_annual_low_entry_broad_screen_candidate_paths.csv'));
writetable(block_table, fullfile(out_dir, 'nimby_annual_low_entry_broad_screen_candidate_blocks.csv'));
write_report(fullfile(out_dir, 'nimby_annual_low_entry_broad_screen.md'), ...
...
fprintf(fid, '# Annual NIMBY low-entry broad screen\n\n');
fprintf(fid, ['This workflow keeps the best direct access-side improvement in play, fixes benchmark `CC` and low-entry owner grid `housingmax = %.0f`, ', ...
    'and then broadens the annual re-screen beyond the narrow vote-side follow-up by reopening cohort weights, `ka`, `theta_r`, and `rent_markup`.\n\n'], ...
...
fprintf(fid, '- fixed annual baseline: benchmark `CC`, low-entry owner grid `housingmax = %.0f`\n', settings.housingmax_low_entry);
...
fprintf(fid, 'The full candidate ranking is saved in `nimby_annual_low_entry_broad_screen_candidates.csv`, and the full age-block panel is saved in `nimby_annual_low_entry_broad_screen_candidate_blocks.csv`.\n');
fclose(fid);
end
