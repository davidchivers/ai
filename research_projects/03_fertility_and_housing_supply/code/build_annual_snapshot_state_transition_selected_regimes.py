...
OUTPUT_STEM = "annual_snapshot_state_transition_selected_regimes"
T = 80
    ...
    for scenario, frame in combined.groupby("scenario", sort=False):
        for row in frame.itertuples(index=False):
            ...
            young_mortgage_ratio = (
                float(row.young_mortgaged_owner_share_25_34) / max(float(row.young_owner_share_25_34), 1.0e-12)
            )
            mid_mortgage_ratio = (
                float(row.mortgaged_owner_share_35_44) / max(float(row.owner_share_35_44), 1.0e-12)
            )
            rows.append(
                {
                    "scenario": scenario,
                    ...
                    "young_mortgaged_owner_share_25_34": float(row.young_mortgaged_owner_share_25_34),
                    "young_mortgage_share_among_owners": young_mortgage_ratio,
                    "young_renter_with_debt_share_25_34": float(row.young_renter_with_debt_share_25_34),
                    ...
                    "young_owner_gap_vs_drift": float(row.young_owner_share_25_34) - float(base["young_owner_share_25_34"]),
                    "young_mortgage_gap_vs_drift": float(row.young_mortgaged_owner_share_25_34)
                    - float(base["young_mortgaged_owner_share_25_34"]),
                    "owner_35_44_gap_vs_drift": float(row.owner_share_35_44) - float(base["owner_share_35_44"]),
...
def write_note(path: Path, summary: pd.DataFrame) -> None:
    ...
    peaks = (
        top.groupby("scenario", as_index=False)[
            ["young_mortgage_gap_vs_drift", "young_owner_gap_vs_drift", "mortgage_35_44_gap_vs_drift", "owner_35_44_gap_vs_drift"]
        ]
        .max()
    )
    lines = [
        "# Annual selected transition regimes",
        "",
        "This note freezes the best calibrated targeted persistent eligibility rule and compares only three annual transition scenarios under permanent price drift.",
        "",
        "## Frozen calibration",
        "",
        ...
        "- family deposit-help weight: `0.50` when active",
        "- all support is standing, not temporary",
        "",
        "## Scenarios",
        "",
        "- `drift_baseline`: permanent `+5%` price path, no support",
        "- `drift_targeted_help_to_buy`: permanent `+5%` price path, standing targeted qualification only",
        "- `drift_targeted_help_to_buy_plus_family_help`: permanent `+5%` price path, standing targeted qualification plus family deposit help",
        "",
    ...
    for row in peaks.itertuples(index=False):
        lines.append(
            f"| `{row.scenario}` | `{row.young_mortgage_gap_vs_drift:.3f}` | `{row.young_owner_gap_vs_drift:.3f}` | "
            f"`{row.mortgage_35_44_gap_vs_drift:.3f}` | `{row.owner_35_44_gap_vs_drift:.3f}` |"
        )
    ...
    for scenario in scenarios:
        ...
        for row in rows.itertuples(index=False):
            lines.append(
                f"| `{row.scenario}` | `{row.t}` | `{row.young_owner_share_25_34:.3f}` | `{row.young_mortgaged_owner_share_25_34:.3f}` | "
                f"`{row.young_mortgage_share_among_owners:.3f}` | `{row.owner_share_35_44:.3f}` | `{row.mortgaged_owner_share_35_44:.3f}` | "
                f"`{row.mortgage_share_among_owners_35_44:.3f}` | `{row.young_mortgage_gap_vs_drift:.3f}` | `{row.young_owner_gap_vs_drift:.3f}` |"
            )
    ...
    lines.extend(
        [
            ...
            "## Read",
            "",
            "- The annual transition mechanism is now fixed and calibrated, not still searching over new wedges.",
            "- The question is no longer persistence. It is whether the ownership lift is acceptable relative to the mortgage gain.",
            "- The family-help layer should be read as an augmentation of help-to-buy, not the main driver.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
