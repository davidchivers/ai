function run_fertility_annual_anticipated_price_drift_screen(mode)
% run_fertility_annual_anticipated_price_drift_screen.m
%
% RE-style smoke test for the annual branch. This is not a full transition
% model. It lets households place some weight on the existing higher-price
% continuation values when choosing today, so we can test whether
% anticipated upward house-price drift is directionally powerful enough to
% matter.
%
% Outputs:
...
%   notes/build/fertility_annual_anticipated_price_drift_screen_target_ranges.csv
...
target_table = build_target_table(cfg.target_ranges, cfg.homeownership_target_ranges);
writetable(target_table, fullfile(out_dir, 'fertility_annual_anticipated_price_drift_screen_target_ranges.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual anticipated-price-drift candidate %d: %s ===\n', spec.candidate_id, char(spec.label));
...
switch char(settings.mode)
    case 'fast'
        ...
        settings.drift_weight_grid = [0.10, 0.30, 0.50, 0.70];
        settings.drift_factor_grid = [1.01, 1.03, 1.05];
    case 'confirm'
        base_overrides.I = 32;
        base_overrides.J = 12;
        settings.price_grid = [1.50, 1.75, 2.00, 2.25, 2.50];
        settings.crossing_price_grid = [1.50, 1.75, 2.00, 2.25, 2.50, 2.75, 3.00, 3.50, 4.00, 5.00, 6.00];
        settings.drift_weight_grid = [0.10, 0.30, 0.50];
        settings.drift_factor_grid = [1.01, 1.03, 1.05];
    otherwise
        base_overrides.I = 16;
        base_overrides.J = 4;
...
spec_template = build_spec(NaN, "", NaN, NaN, 0);
specs = repmat(spec_template, 0, 1);
idx = 0;

idx = idx + 1;
specs(idx) = build_spec(idx, "baseline anticipated-drift block", 0.0, 1.01, 1);

for iw = 1:numel(settings.drift_weight_grid)
    for id = 1:numel(settings.drift_factor_grid)
        weight_value = settings.drift_weight_grid(iw);
        factor_value = settings.drift_factor_grid(id);
        idx = idx + 1;
        label = sprintf('drift weight %.2f | future price factor %.3f', weight_value, factor_value);
        specs(idx) = build_spec(idx, label, weight_value, factor_value, 0);
    end
end
end

function spec = build_spec(candidate_id, label, drift_weight, drift_factor, is_baseline)
spec = struct();
spec.candidate_id = candidate_id;
spec.label = string(label);
spec.drift_weight = drift_weight;
spec.drift_factor = drift_factor;
...
overrides.anticipated_price_drift_weight = spec.drift_weight;
overrides.d_a_price = spec.drift_factor;
...
owner_shares = summarize_owner_bins(eval_diagnostics);
...
homeownership_scores = score_annual_homeownership_support_targets(cfg.homeownership_target_ranges, owner_shares);
homeownership_support_score = homeownership_scores.score;
...
score = 4.0 * primary_score + 0.50 * homeownership_support_score + sign_penalty + ...
...
row.drift_weight = spec.drift_weight;
row.drift_factor = spec.drift_factor;
...
row.eval_owner_share_25_34 = owner_shares(1);
row.eval_owner_share_35_44 = owner_shares(2);
row.eval_owner_share_45_54 = owner_shares(3);
...
row.homeownership_support_band_excess = homeownership_support_score;
...
path_rows = table( ...
    ...
    repmat(spec.drift_weight, n, 1), ...
    repmat(spec.drift_factor, n, 1), ...
    settings.price_grid(:), ...
    avg_birth_rate, ...
    ...
    debtstock, ...
    mass_error, ...
    'VariableNames', {'candidate_id', 'label', 'drift_weight', 'drift_factor', ...
...
crossing_path_rows = table( ...
    ...
    repmat(spec.drift_weight, height(crossing_results), 1), ...
    repmat(spec.drift_factor, height(crossing_results), 1), ...
...
function owner_shares = summarize_owner_bins(diagnostics)
owner_shares = NaN(1, 3);
if isempty(diagnostics)
    return;
end
...
owner_share_by_age = NaN(numel(ages), 1);
age_mass = NaN(numel(ages), 1);

for i = 1:numel(ages)
    age_slice = dens4(:, :, :, i);
    age_mass(i) = sum(age_slice, 'all');
    renter_mass = sum(age_slice(:, 1, :), 'all');
    owner_share_by_age(i) = 1 - renter_mass / max(age_mass(i), 1e-12);
end

owner_shares(1) = weighted_bin_share(owner_share_by_age, age_mass, ages >= 25 & ages <= 34);
owner_shares(2) = weighted_bin_share(owner_share_by_age, age_mass, ages >= 35 & ages <= 44);
owner_shares(3) = weighted_bin_share(owner_share_by_age, age_mass, ages >= 45 & ages <= 54);
end

function value = weighted_bin_share(series, weights, mask)
bin_weights = weights(mask);
if isempty(bin_weights) || sum(bin_weights) <= 0
    value = NaN;
    return;
end
value = sum(series(mask) .* bin_weights) / sum(bin_weights);
end

function target_table = build_target_table(ranges, varargin)
all_targets = [ranges.primary(:); ranges.support(:); ranges.validation];
if nargin >= 2 && ~isempty(varargin{1})
    extra_ranges = varargin{1};
    all_targets = [all_targets; extra_ranges.support(:); extra_ranges.validation(:)];
end
...
row = struct( ...
    ...
    'homeownership_support_band_excess', NaN, ...
...
writetable(candidates_table, fullfile(out_dir, 'fertility_annual_anticipated_price_drift_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'fertility_annual_anticipated_price_drift_screen_candidate_paths.csv'));
writetable(crossing_path_table, fullfile(out_dir, 'fertility_annual_anticipated_price_drift_screen_crossing_paths.csv'));
write_report(fullfile(out_dir, 'fertility_annual_anticipated_price_drift_screen.md'), ...
...
fprintf(fid, '# Annual anticipated-price-drift screen\n\n');
fprintf(fid, ['This workflow holds the usable annual fertility anchor fixed and adds an RE-style price-drift expectation wedge. ', ...
    'Households place some weight on higher-price continuation values when choosing today. This is a smoke test, not a full transition-path equilibrium.\n\n']);
...
fprintf(fid, '- `theta_r`: `%.2f`\n', settings.anchor.theta_r);
fprintf(fid, '- `housingmax`: `%.2f`\n\n', settings.anchor.housingmax);
...
fprintf(fid, '## Homeownership support strictness\n\n');
fprintf(fid, '- source: official U.S. Census ACS 1-year table `B25007` pooled over `2019, 2021-2024`\n');
fprintf(fid, '- scored bins: `25-34`, `35-44`, `45-54`\n');
fprintf(fid, '- validation-only bins: `55-64`, `65-74`\n');
fprintf(fid, '- scoring weight in objective: `0.50` versus `4.00` on the primary fertility score\n');
fprintf(fid, '- interpretation: this is a loose support layer because ACS is age of householder, while the model state is female age\n\n');
...
fprintf(fid, '- Candidates inside loose homeownership support bands: `%d`\n', ...
    sum(candidates_table.homeownership_support_band_excess == 0));
fprintf(fid, '- Candidates with a unique crossing: `%d`\n\n', sum(candidates_table.crossing_is_unique == 1));

fprintf(fid, '## Top candidates\n\n');
fprintf(fid, '| Rank | Candidate | Mean age | Share 30+ | Childless @50 | Own score | Eval vote | Max crossing-grid vote | Best vote price | Crossing | Score |\n');
fprintf(fid, '|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n');
...
for i = 1:max_rows
    ...
    fprintf(fid, '| %d | %s | `%.2f` | `%.3f` | `%.3f` | `%.3f` | `%.3f` | `%.3f` | `%.2f` | %s | `%.4f` |\n', ...
        ...
        candidates_table.homeownership_support_band_excess(i), candidates_table.eval_totalvote(i), candidates_table.max_crossing_totalvote(i), ...
...
if height(candidates_table) >= 1
    ...
    fprintf(fid, '## Best candidate owner shares at eval price\n\n');
    fprintf(fid, '| Bin | Model | Loose target band |\n');
    fprintf(fid, '|---|---:|---:|\n');
    fprintf(fid, '| 25-34 | `%.3f` | `[0.280, 0.540]` |\n', candidates_table.eval_owner_share_25_34(1));
    fprintf(fid, '| 35-44 | `%.3f` | `[0.480, 0.720]` |\n', candidates_table.eval_owner_share_35_44(1));
    fprintf(fid, '| 45-54 | `%.3f` | `[0.580, 0.800]` |\n\n', candidates_table.eval_owner_share_45_54(1));
...
fprintf(fid, '- this is not full RE; it is a bounded smoke that loads actual choices toward the higher-price continuation values already in the solver\n');
fprintf(fid, '- if this barely moves the annual row, then full RE over expected price drift is unlikely to be a simple benchmark rescue by itself\n');
fprintf(fid, '- if it moves a lot, then a richer transition-path branch would become worth building\n');
fclose(fid);
end
