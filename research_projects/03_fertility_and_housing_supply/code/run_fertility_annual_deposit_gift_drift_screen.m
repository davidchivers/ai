function run_fertility_annual_deposit_gift_drift_screen(mode)
% run_fertility_annual_deposit_gift_drift_screen.m
%
% Annual screen combining data-based deposit-help incidence with the
% bounded anticipated price-drift wedge.
...
for i = 1:numel(candidates)
    fprintf('\n=== annual deposit-gift-drift candidate %d / %d ===\n', i, numel(candidates));
...
writetable(candidate_table, fullfile(out_dir, 'fertility_annual_deposit_gift_drift_screen_candidates.csv'));
write_summary_note(fullfile(out_dir, 'fertility_annual_deposit_gift_drift_screen.md'), candidate_table, settings, cfg);
end
...
switch char(settings.mode)
    case 'smoke'
        ...
        settings.gift_share_grid = [0.20, 0.33];
        settings.deposit_gift_grid = [0.75, 1.00];
        settings.deposit_age_max_grid = [44];
        settings.drift_weight_grid = [0.30, 0.50];
        settings.drift_factor_grid = [1.03, 1.05];
    case 'confirm'
        base_overrides.I = 20;
        base_overrides.J = 6;
...
settings.data_basis = "data-based deposit-help incidence plus bounded anticipated house-price drift";
end
...
candidates(idx).drift_weight = 0.0;
candidates(idx).drift_factor = 1.01;
candidates(idx).label = "baseline no gift | no drift";
        ...
        for iage = 1:numel(settings.deposit_age_max_grid)
            for iw = 1:numel(settings.drift_weight_grid)
                for id = 1:numel(settings.drift_factor_grid)
                    ...
                    drift_weight = settings.drift_weight_grid(iw);
                    drift_factor = settings.drift_factor_grid(id);
                    candidates(idx).candidate_id = idx; %#ok<AGROW>
                    candidates(idx).gift_share = share;
                    candidates(idx).deposit_gift = gift;
                    candidates(idx).deposit_age_max = age_max;
                    candidates(idx).drift_weight = drift_weight;
                    candidates(idx).drift_factor = drift_factor;
                    candidates(idx).label = string(sprintf( ...
                        'gift share %.2f | gift %.2f | drift %.2f x %.2f | age max %d', ...
                        share, gift, drift_weight, drift_factor, age_max));
                end
            end
        end
    end
...
row.drift_weight = candidate.drift_weight;
row.drift_factor = candidate.drift_factor;
...
baseline = build_anchor_overrides(settings, cfg, candidate);
...
function overrides = build_anchor_overrides(settings, cfg, candidate)
...
overrides.anticipated_price_drift_weight = candidate.drift_weight;
overrides.d_a_price = candidate.drift_factor;
...
fprintf(fid, '# annual deposit-gift plus drift screen\n\n');
fprintf(fid, '- mode: `%s`\n', char(settings.mode));
...
fprintf(fid, '\n');
fprintf(fid, 'This branch combines data-based first-home deposit help with the bounded anticipated-price-drift wedge.\n\n');
...
fprintf(fid, '- drift weight: `%.2f`\n', best.drift_weight);
fprintf(fid, '- future price factor: `%.2f`\n', best.drift_factor);
...
fprintf(fid, '| label | gift share | gift | drift weight | future factor | mortgaged-owner <35 | owner 25-34 | vote @ eval | unique crossing |\n');
fprintf(fid, '|---|---:|---:|---:|---:|---:|---:|---:|---:|\n');
top_n = min(8, height(candidate_table));
for i = 1:top_n
    row = candidate_table(i, :);
    fprintf(fid, '| `%s` | %.2f | %.2f | %.2f | %.2f | %.3f | %.3f | %.3f | %d |\n', ...
        char(row.label(1)), row.gift_share, row.owner_entry_deposit_gift, row.drift_weight, ...
        row.drift_factor, row.eval_mortgaged_owner_share_under_35, ...
        row.eval_owner_share_25_34, row.eval_vote_per_mass, row.crossing_is_unique);
end
end
