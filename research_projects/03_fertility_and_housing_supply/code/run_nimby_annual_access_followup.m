function run_nimby_annual_access_followup(mode)
% run_nimby_annual_access_followup.m -- Follow-up annual NIMBY screen for
% early-life housing-access mechanics after the broader housing-access pass.
%
% This workflow holds the current best annual baseline fixed:
%   uniform cohorts + hold-5y ka + mid theta_r + hold-5y rent markup
% and then screens two direct access mechanisms that were not in the
% earlier grid:
%   1. borrowing constraint CC
%   2. annual owner-house grid via housingmax (which changes the minimum
%      positive owned-house size at a fixed J)
%
% Usage:
%   run_nimby_annual_access_followup
%   run_nimby_annual_access_followup('full')
%
% Outputs:
...
writetable(target_blocks, fullfile(out_dir, 'nimby_annual_access_followup_target_blocks.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual access follow-up candidate %d: %s ===\n', spec.candidate_id, spec.label);
    ...
    tmp = table( ...
        ...
        repmat(string(spec.cc_code), height(grid), 1), ...
        repmat(string(spec.housingmax_code), height(grid), 1), ...
        grid.a_price, ...
        grid.vote_per_mass, ...
        grid.debt_per_mass, ...
        grid.mass, ...
        'VariableNames', {'candidate_id', 'label', 'cc_code', 'housingmax_code', ...
...
writetable(candidates_table, fullfile(out_dir, 'nimby_annual_access_followup_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'nimby_annual_access_followup_candidate_paths.csv'));
writetable(block_table, fullfile(out_dir, 'nimby_annual_access_followup_candidate_blocks.csv'));
write_report(fullfile(out_dir, 'nimby_annual_access_followup.md'), ...
...
settings.cc_relaxed = 0.95;
settings.cc_full = 1.00;
settings.housingmax_benchmark = 15.0;
settings.housingmax_mid = 12.0;
...
cc_specs = [ ...
    struct('code', "benchmark_cc", 'label', "benchmark CC", 'value', settings.cc_benchmark), ...
    struct('code', "relaxed_cc", 'label', "relaxed CC", 'value', settings.cc_relaxed), ...
    struct('code', "full_cc", 'label', "full CC", 'value', settings.cc_full)];
housing_specs = [ ...
    struct('code', "benchmark_grid", 'label', "benchmark owner grid", 'value', settings.housingmax_benchmark), ...
    struct('code', "mid_entry_grid", 'label', "mid-entry owner grid", 'value', settings.housingmax_mid), ...
    struct('code', "low_entry_grid", 'label', "low-entry owner grid", 'value', settings.housingmax_low_entry)];

specs = repmat(struct(), numel(cc_specs) * numel(housing_specs), 1);
idx = 0;
for ic = 1:numel(cc_specs)
    for ih = 1:numel(housing_specs)
        idx = idx + 1;
        specs(idx).candidate_id = idx;
        specs(idx).cc_code = cc_specs(ic).code;
        specs(idx).cc_label = cc_specs(ic).label;
        specs(idx).cc_value = cc_specs(ic).value;
        ...
        specs(idx).label = sprintf('%s + %s', cc_specs(ic).label, housing_specs(ih).label);
    end
end
end
...
ov.rent_markup = settings.rent_markup_five_year;
ov.ka = settings.ka_five_year;
ov.theta_r = settings.theta_r_mid;
...
row.cc_code = string(spec.cc_code);
row.cc_value = spec.cc_value;
...
fprintf(fid, '# Annual NIMBY access follow-up screen\n\n');
fprintf(fid, 'This workflow starts from the current best annual housing-access baseline and screens only direct early-life access mechanisms: the borrowing constraint `CC` and the annual owner-house grid via `housingmax` at fixed `J`.\n\n');
...
fprintf(fid, '- fixed annual baseline: uniform cohorts, hold-5y `ka`, mid `theta_r`, hold-5y `rent_markup`\n');
fprintf(fid, '- screened annual axes: `CC`, `housingmax`\n');
...
fprintf(fid, 'Score is heuristic: overall vote RMSE + 0.25 * overall debt RMSE + early-life vote RMSE + 0.5 * early-life owner-share RMSE + 0.5 * early-life liquid-asset RMSE + crossing penalties. Lower is better.\n\n');
...
fprintf(fid, 'The full candidate ranking is saved in `nimby_annual_access_followup_candidates.csv`, and the full age-block panel is saved in `nimby_annual_access_followup_candidate_blocks.csv`.\n');
fclose(fid);
end
