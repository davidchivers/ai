function run_nimby_annual_housing_access_screen(mode)
% run_nimby_annual_housing_access_screen.m -- Screen annual NIMBY
% parameterizations that target the early-life housing-access failure mode.
%
% The stage-2 screen showed that the annual mismatch is concentrated in the
% 25-54 age blocks, where the annual model has much lower owner shares and
% much more negative price votes than the 5-year benchmark. This screen
% therefore fixes the best stage-2 baseline block and varies only the
% mechanisms most likely to move early-life ownership support.
%
% Usage:
%   run_nimby_annual_housing_access_screen
...
%   notes/build/nimby_annual_housing_access_screen_target_blocks.csv
...
writetable(target_blocks, fullfile(out_dir, 'nimby_annual_housing_access_screen_target_blocks.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual housing-access candidate %d: %s ===\n', spec.candidate_id, spec.label);
    ...
    tmp = table( ...
        ...
        grid.debt_per_mass, ...
        grid.mass, ...
        'VariableNames', {'candidate_id', 'label', 'cohort_code', 'ka_code', 'theta_r_code', ...
...
specs = repmat(struct(), numel(cohort_specs) * numel(ka_specs) * numel(theta_specs) * numel(rent_specs), 1);
idx = 0;
        ...
        for it = 1:numel(theta_specs)
            for ir = 1:numel(rent_specs)
                ...
                specs(idx).label = sprintf('%s + %s + %s + %s', ...
                    cohort_specs(ic).label, ka_specs(ik).label, theta_specs(it).label, rent_specs(ir).label);
            end
        end
    end
end
...
block_table = table(age_block, five_year_vote_per_mass, annual_vote_per_mass, vote_gap, ...
    five_year_owner_share, annual_owner_share, owner_share_gap);
end
...
score = vote_rmse + 0.25 * debt_rmse + early_vote_rmse + 0.5 * early_owner_share_rmse + crossing_penalty + no_cross_penalty;
...
candidates_table = sortrows(candidates_table, {'score', 'early_vote_rmse', 'early_owner_share_rmse', 'vote_rmse'}, ...
...
writetable(candidates_table, fullfile(out_dir, 'nimby_annual_housing_access_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'nimby_annual_housing_access_screen_candidate_paths.csv'));
writetable(block_table, fullfile(out_dir, 'nimby_annual_housing_access_screen_candidate_blocks.csv'));
write_report(fullfile(out_dir, 'nimby_annual_housing_access_screen.md'), ...
...
fprintf(fid, '# Annual NIMBY housing-access screen\n\n');
fprintf(fid, 'This workflow targets the early-life annual failure mode directly. It fixes the best stage-2 baseline block and screens annual cohort, purchase-friction, rent-utility, and rent-markup choices against both the overall 5-year benchmark and the early-life `25-54` age blocks.\n\n');
...
fprintf(fid, '- fixed annual baseline: annualized discount block, benchmark annual bounds, hold-5y vote shock\n');
fprintf(fid, '- screened annual axes: cohorts, `ka`, `theta_r`, `rent_markup`\n');
...
fprintf(fid, '| Age block | 5-year vote/mass | 5-year owner share |\n');
fprintf(fid, '|---|---:|---:|\n');
for i = 1:height(target_blocks)
    fprintf(fid, '| %s | `%.6f` | `%.6f` |\n', ...
        char(target_blocks.age_block(i)), target_blocks.five_year_vote_per_mass(i), target_blocks.five_year_owner_share(i));
end
fprintf(fid, '\n');

fprintf(fid, '## Candidate ranking\n\n');
fprintf(fid, 'Score is heuristic: overall vote RMSE + 0.25 * overall debt RMSE + early-life vote RMSE + 0.5 * early-life owner-share RMSE + crossing penalties. Lower is better.\n\n');
fprintf(fid, '| Rank | Candidate | Overall vote RMSE | Early vote RMSE | Early owner RMSE | Eval vote/mass | Eval debt/mass | 25-29 owner gap | 50-54 owner gap | Crossing | Score |\n');
fprintf(fid, '|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n');
...
for i = 1:max_rows
    ...
    fprintf(fid, '| %d | %s | `%.6f` | `%.6f` | `%.6f` | `%.6f` | `%.6f` | `%.6f` | `%.6f` | %s | `%.6f` |\n', ...
        i, char(candidates_table.label(i)), candidates_table.vote_rmse(i), ...
        candidates_table.early_vote_rmse(i), candidates_table.early_owner_share_rmse(i), ...
        candidates_table.eval_vote_per_mass(i), candidates_table.eval_debt_per_mass(i), ...
        candidates_table.eval_owner_share_gap_25_29(i), candidates_table.eval_owner_share_gap_50_54(i), ...
...
if height(candidates_table) >= 1
    ...
    fprintf(fid, '| Age block | 5-year vote/mass | Annual vote/mass | Gap | 5-year owner share | Annual owner share | Gap |\n');
    fprintf(fid, '|---|---:|---:|---:|---:|---:|---:|\n');
    for i = 1:height(best_blocks)
        fprintf(fid, '| %s | `%.6f` | `%.6f` | `%.6f` | `%.6f` | `%.6f` | `%.6f` |\n', ...
            ...
            best_blocks.owner_share_gap(i));
    end
    fprintf(fid, '\n');
end

fprintf(fid, 'The full candidate ranking is saved in `nimby_annual_housing_access_screen_candidates.csv`, and the full age-block panel is saved in `nimby_annual_housing_access_screen_candidate_blocks.csv`.\n');
fclose(fid);
end
