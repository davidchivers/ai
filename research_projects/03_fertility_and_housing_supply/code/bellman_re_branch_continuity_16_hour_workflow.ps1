param(
    [string]$RunLabel = "bellman_re_branch_continuity_16_hour_workflow",
...
$canonicalNotePath = Join-Path $projectRoot "notes/build/compiled_sidecar_bellman_re_branch_continuity_16_hour_packet.md"
...
try {
    ...
    $stage2WallSeed = [double[]]@(1.7299384716, 1.7542903475, 1.8750762607, 2.3960221075)
    ...
    @(
        "Objective: validate whether period-3 branch continuity can beat the staged enriched Bellman RE wall near 0.0039779."
        "Run label: $RunLabel"
        "Scope lock:"
        " - project 03 only"
        ...
        " - canonical packet note at notes/build/compiled_sidecar_bellman_re_branch_continuity_16_hour_packet.md"
    ...
    $caseSpecs = @(
        ...
        [pscustomobject]@{
            name = "continuity_p3_stage2"
            stage1_extra = @()
            stage2_extra = @(
                "--branch-continuity-mask", "0,0,1,0",
                "--branch-continuity-vote-slack", "0.02"
            )
        },
        [pscustomobject]@{
            name = "continuity_p34_stage2"
            stage1_extra = @()
            stage2_extra = @(
                "--branch-continuity-mask", "0,0,1,1",
                "--branch-continuity-vote-slack", "0.02"
            )
        },
        [pscustomobject]@{
            name = "continuity_p3_prev34_stage2"
            stage1_extra = @()
            stage2_extra = @(
                "--branch-continuity-mask", "0,0,1,0",
                "--branch-continuity-vote-slack", "0.02",
        ...
        [pscustomobject]@{
            name = "continuity_p3_slack10_prev34_stage2"
            stage1_extra = @()
            stage2_extra = @(
                "--branch-continuity-mask", "0,0,1,0",
                "--branch-continuity-vote-slack", "0.10",
                "--root-selection-anchor", "previous_implied",
                "--root-selection-previous-implied-mask", "0,0,1,1",
                "--branch-hysteresis-mask", "0,0,1,1",
                "--branch-tie-break-mask", "0,0,1,1"
        ...
        [pscustomobject]@{
            name = "continuity_p3_lookahead_prev34_stage2"
            stage1_extra = @()
            stage2_extra = @(
                "--branch-continuity-mask", "0,0,1,0",
                "--branch-continuity-vote-slack", "0.02",
    ...
    $continuityCliffArgs = $baselineCliffArgs + @(
        "--branch-continuity-mask", "0,0,1,0",
        "--branch-continuity-vote-slack", "0.02"
    )
    $continuityLookaheadCliffArgs = $continuityCliffArgs + @(
        "--root-selection-lookahead",
        "--root-selection-lookahead-min-brackets", "2"
    )

    foreach ($row in (Run-CliffGrid -Label "cliff_baseline" -BaseQPath $stage2WallSeed -Q4Values $cliffQ4Values -ExtraArgs $baselineCliffArgs)) {
        $cliffTable.Add($row)
    }
    foreach ($row in (Run-CliffGrid -Label "cliff_continuity_p3" -BaseQPath $stage2WallSeed -Q4Values $cliffQ4Values -ExtraArgs $continuityCliffArgs)) {
        $cliffTable.Add($row)
    }
    foreach ($row in (Run-CliffGrid -Label "cliff_continuity_p3_lookahead" -BaseQPath $stage2WallSeed -Q4Values $cliffQ4Values -ExtraArgs $continuityLookaheadCliffArgs)) {
        $cliffTable.Add($row)
    }
    ...
    $cliffContinuityWorst = $cliffTable | Where-Object { $_.label -eq "cliff_continuity_p3" } | Sort-Object q4 | Select-Object -First 1
    $cliffContinuityLookaheadWorst = $cliffTable | Where-Object { $_.label -eq "cliff_continuity_p3_lookahead" } | Sort-Object q4 | Select-Object -First 1

    $noteLines = @(
        "# Compiled sidecar Bellman RE branch continuity 16-hour packet"
        ""
        "Date: $(Get-Date -Format 'yyyy-MM-dd')"
        ""
        "## Objective"
        ""
        "Test whether explicit period-3 branch continuity can beat the staged enriched Bellman RE wall near 0.0039778925 on the standalone compiled branch."
        ""
        "## Run"
        ""
    ...
    $noteLines += ""
    $noteLines += "- The cliff grid scans q4 values from 2.3960221075 to 2.3962000000 at the staged wall seed."
    $noteLines += "- Baseline, continuity, and continuity-plus-lookahead all use root-selection-anchor = previous_implied with masks on periods 3-4."
    $noteLines += "- See the CSV for the full path table; this packet's purpose is to test whether period-3 continuity changes the local q4 cliff in a way that survives the staged continuation."
    $noteLines += ('- Baseline first row: q4 = {0:F10}, maxres = {1:F10}, qhat = [{2}]' -f [double]$cliffBaselineWorst.q4, [double]$cliffBaselineWorst.maxres, $cliffBaselineWorst.implied_q_path)
    $noteLines += ('- Continuity first row: q4 = {0:F10}, maxres = {1:F10}, qhat = [{2}]' -f [double]$cliffContinuityWorst.q4, [double]$cliffContinuityWorst.maxres, $cliffContinuityWorst.implied_q_path)
    $noteLines += ('- Continuity plus lookahead first row: q4 = {0:F10}, maxres = {1:F10}, qhat = [{2}]' -f [double]$cliffContinuityLookaheadWorst.q4, [double]$cliffContinuityLookaheadWorst.maxres, $cliffContinuityLookaheadWorst.implied_q_path)
    $noteLines += ""
    ...
    $noteLines += "- If one continuity case beats the wall, the next live task is to promote that rule into the canonical staged workflow."
    $noteLines += "- If none do, the next live task is a stronger period-3 predictor-corrector or branch-continuation rule rather than more anchor-only tests."
