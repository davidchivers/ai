function run_fertility_annual_parental_transfer_screen(mode)
% run_fertility_annual_parental_transfer_screen.m
%
% Fast annual prototype for an early-life balance-sheet mechanism: a small
% share of new entrants receives a parental-transfer asset boost that can
% operate like down-payment help. The goal is to see whether earlier
% ownership access can move annual vote and market clearing without
% reopening the fertility anchor itself.
%
% Outputs:
...
%   notes/build/fertility_annual_parental_transfer_screen_target_ranges.csv
...
writetable(target_table, fullfile(out_dir, 'fertility_annual_parental_transfer_screen_target_ranges.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual parental-transfer candidate %d: %s ===\n', spec.candidate_id, char(spec.label));
...
switch char(settings.mode)
    case 'fast'
        ...
        settings.transfer_share_grid = [0.05, 0.10, 0.20, 0.30];
        settings.transfer_b_boost_grid = [2.5, 5.0, 7.5, 10.0];
    case 'confirm'
        base_overrides.I = 32;
        base_overrides.J = 12;
        settings.price_grid = [1.50, 1.75, 2.00, 2.25, 2.50];
        settings.crossing_price_grid = [1.50, 1.75, 2.00, 2.25, 2.50, 2.75, 3.00, 3.50, 4.00, 5.00, 6.00];
        settings.transfer_share_grid = [0.05, 0.10, 0.20];
        settings.transfer_b_boost_grid = [2.5, 5.0, 7.5, 10.0];
    otherwise
        base_overrides.I = 16;
        base_overrides.J = 4;
        ...
        settings.transfer_share_grid = [0.10, 0.20, 0.30];
        settings.transfer_b_boost_grid = [2.5, 5.0, 10.0];
end
...
idx = idx + 1;
specs(idx) = build_spec(idx, "baseline annual parental-transfer block", 0.0, 0.0, 1);

for ishare = 1:numel(settings.transfer_share_grid)
    for ib = 1:numel(settings.transfer_b_boost_grid)
        share_value = settings.transfer_share_grid(ishare);
        b_boost_value = settings.transfer_b_boost_grid(ib);
        idx = idx + 1;
        label = sprintf('transfer share %.2f | parental asset boost %.2f', share_value, b_boost_value);
        specs(idx) = build_spec(idx, label, share_value, b_boost_value, 0);
    end
end
end

function spec = build_spec(candidate_id, label, transfer_share, transfer_b_boost, is_baseline)
spec = struct();
spec.candidate_id = candidate_id;
spec.label = string(label);
spec.transfer_share = transfer_share;
spec.transfer_b_boost = transfer_b_boost;
...
overrides.parental_transfer_share = spec.transfer_share;
overrides.parental_transfer_b_boost = spec.transfer_b_boost;
...
row.transfer_share = spec.transfer_share;
row.transfer_b_boost = spec.transfer_b_boost;
...
path_rows = table( ...
    ...
    repmat(spec.transfer_share, n, 1), ...
    repmat(spec.transfer_b_boost, n, 1), ...
    settings.price_grid(:), ...
    avg_birth_rate, ...
    ...
    debtstock, ...
    mass_error, ...
    'VariableNames', {'candidate_id', 'label', 'transfer_share', 'transfer_b_boost', ...
...
crossing_path_rows = table( ...
    ...
    repmat(spec.transfer_share, height(crossing_results), 1), ...
    repmat(spec.transfer_b_boost, height(crossing_results), 1), ...
...
writetable(candidates_table, fullfile(out_dir, 'fertility_annual_parental_transfer_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'fertility_annual_parental_transfer_screen_candidate_paths.csv'));
writetable(crossing_path_table, fullfile(out_dir, 'fertility_annual_parental_transfer_screen_crossing_paths.csv'));
write_report(fullfile(out_dir, 'fertility_annual_parental_transfer_screen.md'), ...
...
fprintf(fid, '# Annual parental-transfer screen\n\n');
fprintf(fid, ['This workflow holds the usable annual fertility anchor fixed and adds an early-life parental-transfer mechanism. ', ...
    'A subset of new entrants starts with an asset boost, intended to proxy for down-payment help or similar family balance-sheet support.\n\n']);
...
fprintf(fid, '- this screen tests whether early-life parental balance-sheet help can rebuild ownership exposure without reopening the fertility block\n');
fprintf(fid, '- unlike the housing-floor experiments, this branch targets entry conditions for young households directly\n');
fprintf(fid, '- if this moves vote materially, it would support a real savings/downpayment interpretation of the annual miss\n');
fclose(fid);
end
