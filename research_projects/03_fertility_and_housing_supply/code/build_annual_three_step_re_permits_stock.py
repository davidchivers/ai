...
OUTPUT_STEM = "annual_three_step_re_permits_stock"
T = 21


@dataclass(frozen=True)
class ThreeStepFixedPoint:
    q1_re: float
    q2_re: float
    q3_re: float
    theta0: float
...
def implied_q_next(q_current: float, demand_t: float, stock_next: float, params: SupplyStockParams) -> float:
    return float(
        np.clip(
...
def backlog_stock_update(backlog_t: float, stock_t: float, permits_next: float, params: SupplyStockParams) -> tuple[float, float, float]:
    ...
    return completions_next, backlog_next, stock_next


def three_step_implied(summary: pd.DataFrame, scenario, params: SupplyStockParams, q1_guess: float, q2_guess: float, q3_guess: float) -> ThreeStepFixedPoint:
    ...
    completions[1], backlog[1], stock[1] = backlog_stock_update(backlog[0], stock[0], permits[1], params)
    ...
    completions[2], backlog[2], stock[2] = backlog_stock_update(backlog[1], stock[1], permits[2], params)
    pop2 = update_population(summary, pop1, scenario, q2_guess, 1)
    metrics2 = extract_metrics(pop2)
    theta2 = theta_from_metrics(metrics2, params)
    ...
    # t = 2 -> 3
    permits[3] = permit_update(permits[2], theta2, demand2, stock[2], q3_guess, params)
    completions[3], backlog[3], stock[3] = backlog_stock_update(backlog[2], stock[2], permits[3], params)
    pop3 = update_population(summary, pop2, scenario, q3_guess, 2)
    metrics3 = extract_metrics(pop3)
    demand3 = demand_index(metrics3, refs, scenario.price_ratio, 3, params)
    q3_implied = implied_q_next(q2_implied, demand3, stock[3], params)

    return ThreeStepFixedPoint(
        q1_re=q1_implied,
        q2_re=q2_implied,
        q3_re=q3_implied,
        theta0=theta0,
        ...
        permits3=float(permits[3]),
        stock1=float(stock[1]),
        stock2=float(stock[2]),
        stock3=float(stock[3]),
        young_mortgage_25_34_t1=float(metrics1["young_mortgage_25_34"]),
        young_mortgage_25_34_t2=float(metrics2["young_mortgage_25_34"]),
        young_mortgage_25_34_t3=float(metrics3["young_mortgage_25_34"]),
    )


def solve_q123_fixed_point(summary: pd.DataFrame, scenario, params: SupplyStockParams, q1_0: float, q2_0: float, q3_0: float) -> ThreeStepFixedPoint:
    q1_guess = q1_0
    q2_guess = q2_0
    q3_guess = q3_0
    result = three_step_implied(summary, scenario, params, q1_guess, q2_guess, q3_guess)
    for _ in range(400):
        result = three_step_implied(summary, scenario, params, q1_guess, q2_guess, q3_guess)
        q1_new = 0.5 * q1_guess + 0.5 * result.q1_re
        q2_new = 0.5 * q2_guess + 0.5 * result.q2_re
        q3_new = 0.5 * q3_guess + 0.5 * result.q3_re
        if (
            abs(q1_new - q1_guess) < 1.0e-10
            and abs(q2_new - q2_guess) < 1.0e-10
            and abs(q3_new - q3_guess) < 1.0e-10
        ):
            q1_guess = q1_new
            q2_guess = q2_new
            q3_guess = q3_new
...
def simulate_re_path(summary: pd.DataFrame, scenario, params: SupplyStockParams, q1_re: float, q2_re: float, q3_re: float, T: int) -> pd.DataFrame:
    pop0, state, refs = initial_state(summary, params)
    q, permits, completions, stock, backlog = state
    ...
    for t in range(T):
        ...
        records.append(
            {
                "scenario": f"{scenario.name}_three_step_re",
                "t": t,
                "q": float(q[t]),
                "theta": float(theta[t]),
        ...
        if t == 2:
            permits[3] = permit_update(permits[2], theta[t], demand[t], stock[t], q3_re, params)
            completions[3], backlog[3], stock[3] = backlog_stock_update(backlog[2], stock[2], permits[3], params)
            pop = update_population(summary, pop, scenario, q3_re, 2)
            q[3] = q3_re
            continue

        permits[t + 1] = permit_update(permits[t], theta[t], demand[t], stock[t], q[t], params)
        completions[t + 1], backlog[t + 1], stock[t + 1] = backlog_stock_update(backlog[t], stock[t], permits[t + 1], params)
...
def build_checkpoint_table(paths: pd.DataFrame, anchors: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in paths.loc[paths["t"].isin(CHECKPOINTS)].iterrows():
        base_name = row["scenario"].replace("_three_step_re", "")
...
def write_note(path: Path, fixed_points: pd.DataFrame, checkpoints: pd.DataFrame) -> None:
    lines = [
        "# Annual three-step RE on permits-stock transition",
        "",
        "This note extends bounded RE on the upgraded annual permits/backlog/stock price block to an explicit three-step price horizon.",
        "",
        "## Setup",
        "",
        "- time-zero object: observed annual age-by-state snapshot",
        "- price block: politics -> permits -> completions -> stock -> prices",
        "- RE horizon: jointly solve `q_1`, `q_2`, and `q_3`",
        "- `q_1` affects year-1 entry decisions and permit incentives",
        "- `q_2` affects year-2 entry decisions and permit incentives",
        "- `q_3` affects year-3 entry decisions and permit incentives",
        "- after `t = 3`, the path reverts to the upgraded stock-flow transition law",
        "",
        "## Solved three-step fixed points",
        "",
        "| Scenario | q1 | q2 | q3 | Permits1 | Permits2 | Permits3 | Stock1 | Stock2 | Stock3 | Young mortgage t=1 | Young mortgage t=2 | Young mortgage t=3 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in fixed_points.itertuples(index=False):
        lines.append(
            f"| `{row.scenario}` | `{row.q1_re:.3f}` | `{row.q2_re:.3f}` | `{row.q3_re:.3f}` | "
            f"`{row.permits1:.3f}` | `{row.permits2:.3f}` | `{row.permits3:.3f}` | "
            f"`{row.stock1:.3f}` | `{row.stock2:.3f}` | `{row.stock3:.3f}` | "
            f"`{row.young_mortgage_25_34_t1:.3f}` | `{row.young_mortgage_25_34_t2:.3f}` | `{row.young_mortgage_25_34_t3:.3f}` |"
        )
    ...
    lines.extend(
        [
            ...
            "## Read",
            "",
            "- This is the first explicit three-period RE pass on the upgraded permits/stock block.",
            "- If `q_1`, `q_2`, and `q_3` stay moderate and the paths remain close to the stock-flow anchors, the upgraded block is passing the bounded RE workflow cleanly.",
            "- If this also remains tame, the next step is no longer deeper RE by default; it is to write the permits/stock RE summary note and decide whether a more data-driven permit/completion calibration is needed before going further.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    for scenario in scenarios:
        fixed = solve_q123_fixed_point(
            summary,
            scenario,
            params,
            q1_0=scenario.price_ratio,
            q2_0=scenario.price_ratio**2,
            q3_0=scenario.price_ratio**3,
        )
        fixed_rows.append(
            {
                ...
                "permits3": fixed.permits3,
                "stock1": fixed.stock1,
                "stock2": fixed.stock2,
                "stock3": fixed.stock3,
                "young_mortgage_25_34_t1": fixed.young_mortgage_25_34_t1,
                "young_mortgage_25_34_t2": fixed.young_mortgage_25_34_t2,
                "young_mortgage_25_34_t3": fixed.young_mortgage_25_34_t3,
            }
        )
        re_frames.append(simulate_re_path(summary, scenario, params, fixed.q1_re, fixed.q2_re, fixed.q3_re, T=T))
