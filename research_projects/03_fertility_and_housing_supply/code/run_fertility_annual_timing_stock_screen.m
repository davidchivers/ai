function run_fertility_annual_timing_stock_screen(mode)
% run_fertility_annual_timing_stock_screen.m
%
% Theory-led annual fertility recalibration screen that follows the broad
% survivor box with a narrower search over the margins that actually moved
% in local diagnostics:
%   1. first-birth timing profiles
%   2. stock-fertility deterrence bundles
%
% The goal is not to re-run the flat broad box. The goal is to test whether
% stronger deterrence can recover a realistic childlessness band once the
% timing profile is allowed to move toward the CDC-style annual shape.
%
% Usage:
%   run_fertility_annual_timing_stock_screen
%   run_fertility_annual_timing_stock_screen('smoke')
%   run_fertility_annual_timing_stock_screen('fast')
%   run_fertility_annual_timing_stock_screen('full')
%
% Outputs:
...
writetable(target_table, fullfile(out_dir, 'fertility_annual_timing_stock_screen_target_ranges.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual timing-stock candidate %d: %s ===\n', spec.candidate_id, char(spec.label));
...
settings.balanced_realized = repelem([1.00, 0.45, 0.15, 0.04], 5);
settings.early_realized = repelem([1.00, 0.40, 0.12, 0.03], 5);
...
switch char(settings.mode)
    case 'smoke'
        ...
        settings.phi0_grid = [1.05, 1.45];
        settings.bundle_names = ["intermediate_deterrence", "benchmark_deterrence"];
    case 'fast'
        base_overrides.I = 32;
        base_overrides.J = 10;
        settings.price_grid = [1.50, 2.00, 2.50, 3.00];
        settings.market_price_grid = 1.25:0.25:3.25;
        settings.phi0_grid = [0.85, 1.05, 1.25, 1.45];
        settings.bundle_names = ["intermediate_deterrence", "benchmark_deterrence"];
    case 'full'
        ...
        settings.phi0_grid = [0.75, 0.95, 1.15, 1.35, 1.55, 1.75];
        settings.bundle_names = ["intermediate_deterrence", "benchmark_deterrence", "price_heavy_deterrence"];
    otherwise
...
profiles(1) = struct('name', "balanced_timing", 'realized_weights', settings.balanced_realized);
profiles(2) = struct('name', "early_timing", 'realized_weights', settings.early_realized);
profiles(3) = struct('name', "front_loaded_timing", 'realized_weights', settings.front_loaded_realized);
end

function bundles = build_policy_bundles()
bundles = struct();
bundles.intermediate_deterrence = struct( ...
    'child_utility', 0.02, ...
    'birth_cost', 0.05, ...
...
bundles.benchmark_deterrence = struct( ...
    ...
    'lambda_crowd', 0.18);
bundles.price_heavy_deterrence = struct( ...
    'child_utility', 0.00, ...
    'birth_cost', 0.06, ...
    'birth_price_coeff', 0.30, ...
    'lambda_crowd', 0.12);
end
...
writetable(candidates_table, fullfile(out_dir, 'fertility_annual_timing_stock_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'fertility_annual_timing_stock_screen_candidate_paths.csv'));
write_report(fullfile(out_dir, 'fertility_annual_timing_stock_screen.md'), ...
...
fprintf(fid, '# Annual fertility timing-plus-stock screen\n\n');
fprintf(fid, ['This workflow follows the cancelled broad survivor box with a narrower theory-led search. ', ...
    'It varies the timing profile and the stock-fertility deterrence bundle together, because the broad box showed that ', ...
    'generic `phi0` / `child_utility` / `birth_cost` / `kappa` / `lambda` moves on a fixed timing profile do not move the annual targets enough.\n\n']);
...
fprintf(fid, '  - earlier realized-birth profiles should pull timing earlier\n');
fprintf(fid, '  - stronger `birth_cost` / `birth_price_coeff` / `lambda_crowd` should raise stock childlessness and lower birth rates\n');
fprintf(fid, '  - the useful region is where stronger deterrence improves childlessness without sending timing back to the 36-year benchmark failure\n');
fclose(fid);
end
