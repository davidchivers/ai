function run_fertility_annual_deposit_gift_share_screen(mode)
% run_fertility_annual_deposit_gift_share_screen.m
%
% Data-motivated annual screen for first-home deposit gifts. The gift is
% still a first-time owner-entry transfer, but only a subset of households
% receives it. The share grid is centered on observed family-help / DPA
% ranges rather than universal take-up.
...
for i = 1:numel(candidates)
    fprintf('\n=== annual deposit-gift-share candidate %d / %d ===\n', i, numel(candidates));
...
writetable(candidate_table, fullfile(out_dir, 'fertility_annual_deposit_gift_share_screen_candidates.csv'));
write_summary_note(fullfile(out_dir, 'fertility_annual_deposit_gift_share_screen.md'), candidate_table, settings, cfg);
end
...
switch char(settings.mode)
    ...
    case 'confirm'
        ...
        settings.gift_share_grid = [0.20, 0.22, 0.25, 0.30, 0.33];
        settings.deposit_gift_grid = [0.50, 0.75, 1.00];
...
settings.data_basis = "share range centered on FHA family-gift / DPA style take-up";
end
...
for ishare = 1:numel(settings.gift_share_grid)
    for igift = 1:numel(settings.deposit_gift_grid)
        for iage = 1:numel(settings.deposit_age_max_grid)
            ...
            candidates(idx).label = string(sprintf('gift share %.2f | gift %.2f | age max %d', share, gift, age_max));
        end
    end
end
end
...
fprintf(fid, '# annual deposit-gift share screen\n\n');
fprintf(fid, '- mode: `%s`\n', char(settings.mode));
...
fprintf(fid, '\n');
fprintf(fid, 'This branch keeps the deposit-only owner-entry gift, but only a subset of households receives it. It is a reduced-form proxy for observed family-help / DPA take-up, not universal parental support.\n\n');
