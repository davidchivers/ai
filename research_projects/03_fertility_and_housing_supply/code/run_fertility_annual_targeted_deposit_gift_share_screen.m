function run_fertility_annual_targeted_deposit_gift_share_screen(mode)
% run_fertility_annual_targeted_deposit_gift_share_screen.m
%
% Data-motivated annual screen for targeted first-home deposit gifts.
% Only a subset of households is gift-eligible, and the gift only applies
% to low-cash renter households near zero liquid assets when they buy.
...
for i = 1:numel(candidates)
    fprintf('\n=== annual targeted-deposit-gift candidate %d / %d ===\n', i, numel(candidates));
...
writetable(candidate_table, fullfile(out_dir, 'fertility_annual_targeted_deposit_gift_share_screen_candidates.csv'));
write_summary_note(fullfile(out_dir, 'fertility_annual_targeted_deposit_gift_share_screen.md'), candidate_table, settings, cfg);
end
...
settings.data_basis = "gift incidence around FHA family-gift / DPA take-up, targeted to low-cash near-margin households";
end
...
candidates(idx).deposit_b_min = -inf;
candidates(idx).deposit_b_max = inf;
...
for ishare = 1:numel(settings.gift_share_grid)
    for igift = 1:numel(settings.deposit_gift_grid)
        for iage = 1:numel(settings.deposit_age_max_grid)
            for iw = 1:size(settings.asset_windows, 1)
                idx = idx + 1;
                share = settings.gift_share_grid(ishare);
                gift = settings.deposit_gift_grid(igift);
                age_max = settings.deposit_age_max_grid(iage);
                bmin = settings.asset_windows(iw, 1);
                bmax = settings.asset_windows(iw, 2);
                candidates(idx).candidate_id = idx; %#ok<AGROW>
                candidates(idx).gift_share = share;
                candidates(idx).deposit_gift = gift;
                candidates(idx).deposit_age_max = age_max;
                candidates(idx).deposit_b_min = bmin;
                candidates(idx).deposit_b_max = bmax;
                candidates(idx).label = string(sprintf('gift share %.2f | gift %.2f | b in [%.1f, %.1f] | age max %d', ...
                    share, gift, bmin, bmax, age_max));
            end
        end
    end
end
...
row.deposit_b_min = candidate.deposit_b_min;
row.deposit_b_max = candidate.deposit_b_max;
...
baseline.owner_entry_deposit_b_min = -inf;
baseline.owner_entry_deposit_b_max = inf;
...
gifted.owner_entry_deposit_b_min = candidate.deposit_b_min;
gifted.owner_entry_deposit_b_max = candidate.deposit_b_max;
...
fprintf(fid, '# annual targeted deposit-gift share screen\n\n');
fprintf(fid, '- mode: `%s`\n', char(settings.mode));
...
fprintf(fid, '\n');
fprintf(fid, 'This branch keeps data-based gift incidence but only lets the gift operate for low-cash renter households near zero liquid assets when they buy.\n\n');
...
fprintf(fid, '- asset window: `[%.1f, %.1f]`\n', best.deposit_b_min, best.deposit_b_max);
...
fprintf(fid, '| label | gift share | gift | asset window | mortgaged-owner <35 | owner 25-34 | vote @ eval | unique crossing |\n');
fprintf(fid, '|---|---:|---:|---|---:|---:|---:|---:|\n');
top_n = min(8, height(candidate_table));
for i = 1:top_n
    row = candidate_table(i, :);
    fprintf(fid, '| `%s` | %.2f | %.2f | `[%.1f, %.1f]` | %.3f | %.3f | %.3f | %d |\n', ...
        char(row.label(1)), row.gift_share, row.owner_entry_deposit_gift, ...
        row.deposit_b_min, row.deposit_b_max, row.eval_mortgaged_owner_share_under_35, ...
