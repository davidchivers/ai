function run_fertility_annual_timing_stock_bridge_screen(mode)
% run_fertility_annual_timing_stock_bridge_screen.m
%
% Theory-led frontier-bridge follow-up to the completed timing-stock
% survivor box. The goal is to bridge the newly exposed tradeoff:
%   1. more front-loaded realized first-birth timing profiles
%   2. intermediate phi0 values between the timing winner and the
%      childlessness winner
%   3. mild stock-fertility deterrence that is weaker than the old
%      benchmark bundle
%
% The goal is not to re-run the broad or timing-stock screens. The goal is
% to test whether a more front-loaded timing profile can preserve
% acceptable annual timing even when phi0 is lowered enough to move the
% childlessness margin upward.
%
% Usage:
%   run_fertility_annual_timing_stock_bridge_screen
%   run_fertility_annual_timing_stock_bridge_screen('smoke')
%   run_fertility_annual_timing_stock_bridge_screen('fast')
%   run_fertility_annual_timing_stock_bridge_screen('full')
%
% Outputs:
...
%   notes/build/fertility_annual_timing_stock_bridge_screen_target_ranges.csv
...
writetable(target_table, fullfile(out_dir, 'fertility_annual_timing_stock_bridge_screen_target_ranges.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual timing-stock bridge candidate %d: %s ===\n', spec.candidate_id, char(spec.label));
...
settings.front_loaded_realized = repelem([1.00, 0.38, 0.10, 0.025], 5);
settings.pivot_front_loaded_realized = repelem([1.00, 0.35, 0.08, 0.020], 5);
...
switch char(settings.mode)
    case 'smoke'
        ...
        settings.phi0_grid = [1.05, 1.25];
        settings.profile_names = ["front_loaded_timing", "pivot_front_loaded", "ultra_front_loaded", "very_front_loaded"];
        settings.bundle_names = ["survivor_deterrence", "bridge_deterrence"];
    case 'fast'
        base_overrides.I = 32;
        base_overrides.J = 10;
        settings.price_grid = [1.50, 2.00, 2.50, 3.00];
        settings.market_price_grid = 1.25:0.25:3.25;
        settings.phi0_grid = [1.05, 1.10, 1.15, 1.20, 1.25];
        settings.profile_names = ["pivot_front_loaded", "ultra_front_loaded", "very_front_loaded"];
        settings.bundle_names = ["survivor_deterrence"];
    case 'full'
        ...
        settings.phi0_grid = [1.00, 1.05, 1.10, 1.15, 1.20, 1.25, 1.30];
        settings.profile_names = ["pivot_front_loaded", "ultra_front_loaded", "very_front_loaded"];
        settings.bundle_names = ["survivor_deterrence", "bridge_deterrence"];
    otherwise
...
spec_template = build_spec(NaN, "", "", [NaN, NaN, NaN], NaN, NaN, NaN, NaN, []);
specs = repmat(spec_template, 0, 1);
idx = 0;

idx = idx + 1;
specs(idx) = build_spec( ...
    idx, "current_annualized_benchmark", "current", ...
    ...
    cfg.overrides.first_birth_realized_weights);
...
for it = 1:numel(timing_profiles)
    ...
    specs(idx) = build_spec( ...
        idx, ...
        sprintf('%s | benchmark params', char(profile.name)), ...
        char(profile.name), ...
        cfg.overrides.birth_utility_by_parity, ...
        cfg.overrides.child_utility, ...
        cfg.overrides.birth_cost, ...
    ...
    for ib = 1:numel(settings.bundle_names)
        ...
        for ip = 1:numel(settings.phi0_grid)
            ...
            specs(idx) = build_spec( ...
                idx, ...
                sprintf('%s | phi0 %.2f | child %.2f | cost %.2f | kappa %.2f | lambda %.2f', ...
...
available = struct();
available.front_loaded_timing = struct('name', "front_loaded_timing", 'realized_weights', settings.front_loaded_realized);
available.pivot_front_loaded = struct('name', "pivot_front_loaded", 'realized_weights', settings.pivot_front_loaded_realized);
...
profiles = repmat(available.front_loaded_timing, 0, 1);
...
bundles = struct();
bundles.survivor_deterrence = struct( ...
    'child_utility', 0.02, ...
    'birth_cost', 0.05, ...
    'birth_price_coeff', 0.16, ...
    'lambda_crowd', 0.10);
bundles.bridge_deterrence = struct( ...
    'child_utility', 0.015, ...
    'birth_cost', 0.05, ...
    'birth_price_coeff', 0.18, ...
    'lambda_crowd', 0.12);
bundles.benchmark_deterrence = struct( ...
    'child_utility', 0.02, ...
    'birth_cost', 0.06, ...
...
function spec = build_spec(candidate_id, label, timing_profile, birth_utility_by_parity, child_utility, birth_cost, birth_price_coeff, lambda_crowd, first_birth_realized_weights)
spec = struct();
...
row.timing_profile = spec.timing_profile;
...
path_rows = table( ...
    ...
    repmat(spec.timing_profile, n, 1), ...
    settings.price_grid(:), ...
    avg_birth_rate, ...
    ...
    debtstock, ...
    mass_error, ...
    'VariableNames', {'candidate_id', 'label', 'timing_profile', 'a_price', 'avg_birth_rate', ...
...
candidates_table = sortrows(candidates_table, ...
    {'primary_pass', 'sign_pass', 'score', 'support_pass', 'crossing_is_unique'}, ...
    {'descend', 'descend', 'ascend', 'descend', 'descend'});
end
...
candidates_table = sort_candidate_rows(candidate_rows);
    writetable(candidates_table, fullfile(out_dir, 'fertility_annual_timing_stock_bridge_screen_candidates.csv'));
    writetable(path_table, fullfile(out_dir, 'fertility_annual_timing_stock_bridge_screen_candidate_paths.csv'));
    write_report(fullfile(out_dir, 'fertility_annual_timing_stock_bridge_screen.md'), ...
...
fprintf(fid, '# Annual fertility timing-stock frontier bridge screen\n\n');
fprintf(fid, ['This workflow follows the completed timing-plus-stock survivor box with a narrower frontier bridge. ', ...
    'It focuses only on more front-loaded timing profiles and intermediate stock-fertility deterrence, because the previous Hamilton batch showed a clear tradeoff between acceptable timing and acceptable childlessness.\n\n']);
...
fprintf(fid, '- anchor targets: mean age first birth and share first births at age `30+`\n');
fprintf(fid, '- loose target: childless share at age `50` inside a broad band\n');
fprintf(fid, '- plausibility diagnostics: sign checks, market-crossing sanity, mass conservation\n');
fprintf(fid, '- expected sign for the new box:\n');
fprintf(fid, '  - more front-loaded realized-birth profiles should pull timing earlier for a given `phi0`\n');
fprintf(fid, '  - lower `phi0` should raise childlessness but push timing later\n');
fprintf(fid, '  - the useful bridge region is where extra front-loading offsets the timing delay from lower `phi0`\n');
fprintf(fid, '  - mild deterrence should help the stock margin without reproducing the very-late timing failure of the old benchmark bundle\n');
fclose(fid);
end
