param(
    [string]$RunLabel = "bellman_re_branch_corrector_16_hour_workflow",
...
$canonicalNotePath = Join-Path $projectRoot "notes/build/compiled_sidecar_bellman_re_branch_corrector_16_hour_packet.md"
...
try {
    ...
    @(
        "Objective: validate whether the branch-switch-triggered corrector refresh can beat the staged enriched Bellman RE wall near 0.0039779."
        "Run label: $RunLabel"
        "Scope lock:"
        " - project 03 only"
        ...
        " - canonical packet note at notes/build/compiled_sidecar_bellman_re_branch_corrector_16_hour_packet.md"
    ...
    $caseSpecs = @(
        ...
        [pscustomobject]@{
            name = "corrector_p3_lookahead_slack100_prev34_stage2"
            stage1_extra = @()
            stage2_extra = @(
        ...
        [pscustomobject]@{
            name = "corrector_p34_lookahead_slack100_prev34_stage2"
            stage1_extra = @()
            stage2_extra = @(
        ...
        [pscustomobject]@{
            name = "corrector_p3_lookahead_slack100_prev234_stage2"
            stage1_extra = @()
            stage2_extra = @(
                "--branch-continuity-mask", "0,0,1,0",
                "--branch-continuity-vote-slack", "1.0",
    ...
    $correctorCliffArgs = $baselineCliffArgs + @(
        "--branch-continuity-mask", "0,0,1,0",
        "--branch-continuity-vote-slack", "1.0",
        "--root-selection-lookahead",
        "--root-selection-lookahead-min-brackets", "2"
    ...
    foreach ($row in (Run-CliffGrid -Label "cliff_corrector_p3" -BaseQPath $stage2WallSeed -Q4Values $cliffQ4Values -ExtraArgs $correctorCliffArgs)) {
        $cliffTable.Add($row)
    }
    ...
    $cliffCorrectorWorst = $cliffTable | Where-Object { $_.label -eq "cliff_corrector_p3" } | Sort-Object q4 | Select-Object -First 1

    $noteLines = @(
        "# Compiled sidecar Bellman RE branch corrector 16-hour packet"
        ""
        "Date: $(Get-Date -Format 'yyyy-MM-dd')"
        ""
        "## Objective"
        ""
        "Test whether the branch-switch-triggered corrector refresh can beat the staged enriched Bellman RE wall near 0.0039778925 on the standalone compiled branch."
        ""
        "## Run"
        ""
    ...
    $noteLines += "- The aggressive corrector case uses continuity on period 3, lookahead, very wide continuity vote slack, and previous-implied anchoring."
    $noteLines += "- See the CSV for the full path table; this packet's purpose is to test whether the new branch-corrector refresh can turn that aggressive selector into a better staged continuation."
    ...
    $noteLines += ('- Corrector first row: q4 = {0:F10}, maxres = {1:F10}, qhat = [{2}]' -f [double]$cliffCorrectorWorst.q4, [double]$cliffCorrectorWorst.maxres, $cliffCorrectorWorst.implied_q_path)
    $noteLines += ""
    ...
    $noteLines += ""
    $noteLines += "- This packet tests the first explicit branch-corrector refresh, not another same-period selector tie-break."
    $noteLines += "- If one aggressive case beats the wall, the next live task is to promote that rule into the canonical staged workflow."
    $noteLines += "- If none do, the next live task is a stronger predictor-corrector or repaired-prefix branch continuation rather than more selection tweaks."
