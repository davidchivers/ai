param(
    [string]$RunLabel = "bellman_re_ultra_micro_wall_12_hour_workflow",
...
$canonicalNotePath = Join-Path $projectRoot "notes/build/compiled_sidecar_bellman_re_ultra_micro_wall_packet.md"
...
try {
    ...
    $ultraSeed = [double[]]@(1.7303548376, 1.7483599506, 1.7526096626, 2.1532712772)

    @(
        "Objective: run a bounded 12-hour compiled-sidecar Bellman RE packet around the improved ultra-micro wall near q3=1.7526097.",
        "Run label: $RunLabel",
        "Scope lock:",
        " - project 03 only",
        ...
        "Deliverables:",
        " - exact ultra-micro probe CSVs around the improved endpoint",
        " - bounded continuation sweep under smaller stall-probe steps",
        " - one canonical packet note at notes/build/compiled_sidecar_bellman_re_ultra_micro_wall_packet.md",
        "Stopping rule: stop after the packet note exists, or at the first timeout/error."
    ) | Set-Content -Path $manifestPath

    Write-Status ("RUN_DIR {0}" -f $runDir)

    $exactBaseline = Invoke-ExactEval -QPath $ultraSeed
    Write-Status ("BASELINE_EXACT maxres={0:F10}" -f $exactBaseline.maxres)

    $q2Tiny = [double[]]@(-0.000625, -0.0003125, -0.00015625, 0.0, 0.00015625, 0.0003125, 0.000625)
    $q3Ultra = [double[]]@(-0.00015625, -0.000078125, -0.0000390625, 0.0, 0.0000390625, 0.000078125, 0.00015625)
    $q4Tiny = [double[]]@(-0.0003125, -0.00015625, -0.000078125, 0.0, 0.000078125, 0.00015625, 0.0003125)
    $q2Tiny3D = [double[]]@(-0.0003125, 0.0, 0.0003125)
    $q3Tiny3D = [double[]]@(-0.000078125, 0.0, 0.000078125)
    $q4Tiny3D = [double[]]@(-0.00015625, 0.0, 0.00015625)

    $lineQ3 = Run-ExactLine1D -StepName "01_ultra_micro_exact_line_q3" `
        -CenterQ $ultraSeed `
        -IndexA 2 `
        -DeltasA $q3Ultra

    $grid23 = Run-ExactGrid2D -StepName "02_ultra_micro_exact_grid_q23" `
        -CenterQ $ultraSeed `
        -IndexA 1 `
        -IndexB 2 `
        -DeltasA $q2Tiny `
    ...
    $grid34 = Run-ExactGrid2D -StepName "03_ultra_micro_exact_grid_q34" `
        -CenterQ $ultraSeed `
        -IndexA 2 `
        -IndexB 3 `
        -DeltasA $q3Ultra `
    ...
    $grid234 = Run-ExactGrid3D -StepName "04_ultra_micro_exact_grid_q234" `
        -CenterQ $ultraSeed `
        -IndexA 1 `
        -IndexB 2 `
        -IndexC 3 `
    ...
    if ([double]$bestExactRow.maxres -lt ($exactBaseline.maxres - 1.0e-6)) {
        $bestExactContinuation = Run-StagedCase -CaseName "05_best_ultra_exact_candidate" `
            -InitialQPath $bestExactSeed `
            -StageIters @(4, 4) `
            -ExtraArgs @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "6", "--stall-probe-initial-step-fraction", "0.03125")
    }

    $ultraVariants = @(
        [pscustomobject]@{ name = "ultra_p3_r6_step003125"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "6", "--stall-probe-initial-step-fraction", "0.03125") },
        [pscustomobject]@{ name = "ultra_p3_r8_step003125"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "8", "--stall-probe-initial-step-fraction", "0.03125") },
        [pscustomobject]@{ name = "ultra_p3_r6_step0046875"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "6", "--stall-probe-initial-step-fraction", "0.046875") },
        [pscustomobject]@{ name = "ultra_p3_r8_step0046875"; extra = @("--max-stall-probe-periods", "3", "--max-stall-probe-rounds", "8", "--stall-probe-initial-step-fraction", "0.046875") },
        [pscustomobject]@{ name = "ultra_p4_r6_step00625"; extra = @("--max-stall-probe-periods", "4", "--max-stall-probe-rounds", "6", "--stall-probe-initial-step-fraction", "0.0625") },
        [pscustomobject]@{ name = "ultra_p4_r8_step00625"; extra = @("--max-stall-probe-periods", "4", "--max-stall-probe-rounds", "8", "--stall-probe-initial-step-fraction", "0.0625") }
    )

    $variantResults = New-Object System.Collections.Generic.List[object]
    foreach ($variant in $ultraVariants) {
        $variantCase = Run-StagedCase -CaseName ("06_variant_{0}" -f $variant.name) `
            -InitialQPath $ultraSeed `
            -StageIters @(4) `
    ...
    $variantCsvPath = Join-Path $runDir "06_ultra_micro_variant_sweep.csv"
    $variantResults | Sort-Object maxres, name | Export-Csv -Path $variantCsvPath -NoTypeInformation
    Write-Status ("DONE 06_ultra_micro_variant_sweep rows={0}" -f $variantResults.Count)
    ...
    if ([double]$bestVariant.maxres -lt ($exactBaseline.maxres - 1.0e-6)) {
        ...
        $bestVariantContinuation = Run-StagedCase -CaseName "07_best_ultra_variant_continuation" `
    ...
    if (($null -ne $bestVariantContinuation) -and ($bestVariantContinuation.final_maxres -lt ($bestVariant.maxres - 1.0e-6))) {
        $continuationGrid = Run-ExactGrid2D -StepName "08_best_ultra_variant_exact_grid_q34_followup" `
    ...
    $noteLines.Add("# Compiled sidecar Bellman RE ultra-micro wall 12-hour packet")
    $noteLines.Add("")
    ...
    $noteLines.Add("- improved ultra-micro wall seed: [1.730355, 1.748360, 1.752610, 2.153271]")
    $noteLines.Add("- no MATLAB, no annual branch, no benchmark-grid RE push")
    $noteLines.Add("")
    $noteLines.Add("## Exact ultra-micro baseline")
    $noteLines.Add("")
    ...
    $noteLines.Add("")
    $noteLines.Add("## Exact ultra-micro probes")
    $noteLines.Add("")
    ...
    if ($null -ne $bestExactContinuation) {
        ...
        $noteLines.Add("- no exact ultra-micro candidate beat the baseline by more than the acceptance tolerance")
    }
    $noteLines.Add("")
    $noteLines.Add("## Ultra-micro variant sweep")
    $noteLines.Add("")
    ...
    $noteLines.Add("")
    $noteLines.Add("## Best ultra-micro variant continuation")
    $noteLines.Add("")
    if ($null -ne $bestVariantContinuation) {
        $noteLines.Add(("- best ultra-micro variant: {0}" -f $bestVariant.name))
        ...
        $noteLines.Add("- no ultra-micro variant beat the baseline enough to justify a continuation run")
    }
    $noteLines.Add("")
    ...
    $noteLines.Add("## Read")
    $noteLines.Add("")
    $noteLines.Add("- This packet is meant to decide whether the improved q3 ultra-micro step opens a real local continuation beyond the micro-wall.")
    ...
    @(
        "Bellman RE ultra-micro wall 12-hour workflow stopped with an error.",
