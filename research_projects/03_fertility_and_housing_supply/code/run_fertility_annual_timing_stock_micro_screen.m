function run_fertility_annual_timing_stock_micro_screen(mode)
% run_fertility_annual_timing_stock_micro_screen.m
%
% Post-bridge micro continuation for the annual fertility calibration.
% The bridge survivor is already close on timing, so this box avoids any
% stronger price/crowding deterrence and instead tests:
%   1. slightly more front-loaded first-birth realized-weight profiles
%   2. finer phi0 steps centered on the bridge winner
%   3. flat stock nudges through child utility / birth cost only
%
% The goal is to lift childlessness into band without pushing mean first
% birth age and the 30+ first-birth share back out of the broad annual
% timing bands.
%
% Usage:
%   run_fertility_annual_timing_stock_micro_screen
%   run_fertility_annual_timing_stock_micro_screen('confirm')
%   run_fertility_annual_timing_stock_micro_screen('smoke')
%   run_fertility_annual_timing_stock_micro_screen('fast')
%   run_fertility_annual_timing_stock_micro_screen('full')
%
% Outputs:
...
%   notes/build/fertility_annual_timing_stock_micro_screen_target_ranges.csv
...
writetable(target_table, fullfile(out_dir, 'fertility_annual_timing_stock_micro_screen_target_ranges.csv'));
...
for i = 1:nc
    ...
    fprintf('\n=== annual timing-stock micro candidate %d: %s ===\n', spec.candidate_id, char(spec.label));
...
switch char(settings.mode)
    case 'confirm'
        ...
        settings.phi0_grid = [1.120, 1.125, 1.115, 1.130];
    ...
    case 'smoke'
        ...
        settings.phi0_grid = [1.12, 1.125, 1.15];
        settings.profile_names = ["max_front_loaded", "extra_front_loaded", "very_front_loaded"];
        settings.bundle_names = ["survivor_deterrence"];
    case 'fast'
        base_overrides.I = 32;
        ...
        settings.phi0_grid = [1.10, 1.12, 1.125, 1.15, 1.175, 1.20];
        settings.profile_names = ["max_front_loaded", "extra_front_loaded", "very_front_loaded", "ultra_front_loaded"];
        settings.bundle_names = ["survivor_deterrence", "flat_child_nudge", "flat_cost_nudge"];
    case 'full'
        base_overrides.I = cfg.overrides.I;
        base_overrides.J = cfg.overrides.J;
        settings.price_grid = [1.50, 1.75, 2.00, 2.50, 3.00];
        settings.market_price_grid = cfg.market_price_grid;
        settings.phi0_grid = [1.08, 1.10, 1.12, 1.125, 1.15, 1.175, 1.20, 1.225];
        settings.profile_names = ["max_front_loaded", "extra_front_loaded", "very_front_loaded", "ultra_front_loaded"];
        settings.bundle_names = ["survivor_deterrence", "flat_child_nudge", "flat_cost_nudge", "flat_dual_nudge"];
    otherwise
        error('Unknown mode "%s". Use "confirm", "smoke", "fast", or "full".', mode);
end
    ...
    for ib = 1:numel(settings.bundle_names)
        ...
        for ip = 1:numel(settings.phi0_grid)
            ...
            specs(idx) = build_spec( ...
                idx, ...
                sprintf('%s | phi0 %.3f | child %.3f | cost %.3f | kappa %.2f | lambda %.2f', ...
...
profiles = repmat(available.very_front_loaded, 0, 1);
...
bundles.survivor_deterrence = struct( ...
    ...
    'lambda_crowd', 0.10);
bundles.flat_child_nudge = struct( ...
    'child_utility', 0.018, ...
    'birth_cost', 0.050, ...
    'birth_price_coeff', 0.16, ...
    'lambda_crowd', 0.10);
bundles.flat_cost_nudge = struct( ...
    'child_utility', 0.020, ...
    'birth_cost', 0.052, ...
    'birth_price_coeff', 0.16, ...
    'lambda_crowd', 0.10);
bundles.flat_dual_nudge = struct( ...
    'child_utility', 0.018, ...
    'birth_cost', 0.052, ...
    'birth_price_coeff', 0.16, ...
    'lambda_crowd', 0.10);
...
writetable(candidates_table, fullfile(out_dir, 'fertility_annual_timing_stock_micro_screen_candidates.csv'));
writetable(path_table, fullfile(out_dir, 'fertility_annual_timing_stock_micro_screen_candidate_paths.csv'));
write_report(fullfile(out_dir, 'fertility_annual_timing_stock_micro_screen.md'), ...
...
fprintf(fid, '# Annual fertility timing-stock micro screen\n\n');
fprintf(fid, ['This workflow follows the near-usable bridge survivor with a deliberately narrow continuation. ', ...
    'It keeps price and crowding deterrence fixed at the survivor level, then tests whether more front-loaded first-birth timing ', ...
    'and small flat stock nudges can lift childlessness without undoing the timing progress.\n\n']);
...
fprintf(fid, '  - more front-loaded realized first-birth weights should pull timing earlier at a given `phi0`\n');
fprintf(fid, '  - lower `phi0` should raise childlessness but delay timing, so it must be paired with extra front-loading\n');
fprintf(fid, '  - small flat `child_utility` / `birth_cost` nudges should lift childlessness with less timing damage than higher `birth_price_coeff` or `lambda_crowd`\n');
fprintf(fid, '  - the useful micro region is where childlessness reaches the band floor while mean age stays at or below `31.0` and share `30+` stays inside band\n');
fclose(fid);
end
