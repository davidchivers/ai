...
double ubi_transfer_level = 0.0;           // UBI: universal lump-sum transfer (fraction of mean wage, manna-from-heaven)
double var_pE;
...
const int Ni = 4; // dim of current occupational state
const int Ni_hi = 3;
...
void apply_runtime_overrides()
{
	...
	double ubi_transfer_env = 0.0;
	if (parse_env_double(getenv("CFV_UBI_TRANSFER"), ubi_transfer_env)) {
		ubi_transfer_level = ubi_transfer_env;
		cout << "[override] ubi_transfer_level (env) = " << ubi_transfer_level << endl;
	}
...
{
	return occp_state == OCCP_ENTR || occp_state == OCCP_UNEMP_UI || occp_state == OCCP_UNEMP_NO_UI;
}
	...
	double& k_opt_in, double n_opt_in, double& rev_opt_in, double prob_rho, double prob_theta_u)
{
	...
	if (i_occp1 == 1) { // choose to be worker
		...
		const double asset_income = (1 + r0 - dep_k) * a0 - t_lumpsum_hhd + ubi_transfer_level * w0_tmp;
		...
		if (i_occp0 == OCCP_EMP) // currently employed
		{
			...
			income_tmp_un = asset_income + lowerincome_b * w0_tmp;
		...
		else if (i_occp0 == OCCP_ENTR || i_occp0 == OCCP_UNEMP_NO_UI) // exiting entrepreneurship or no-UI unemployed
		{
			cx = income_tmp;
			prob_work_rate = prob_theta_u;
			i_occp_through[0] = OCCP_EMP;
	...
	//entrepreneur's decision
		...
		const double asset_income = (1 + r0 - dep_k) * a0 - t_lumpsum_hhd + ubi_transfer_level * w0_tmp;
		income_tmp = asset_income + rev_opt - tau_pi;
		...
		if (use_entrepreneur_closure_risk) {
			...
			i_occp_through[0] = OCCP_ENTR;
			i_occp_through[1] = OCCP_UNEMP_NO_UI;
		}
		else {
			prob_work_rate = 1.0;
			i_occp_through[0] = OCCP_ENTR;
			i_occp_through[1] = OCCP_ENTR;
		}
...
void simulation()
{
        ...
        for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ppl_sim++) {
            ...
            if (i_occp0_sim_tmp == OCCP_EMP) {
                if (occp_prev_state != OCCP_EMP) {
                    z_agg += (1 + edu_z[ppl_edu]) * theta_ut_new;
                }
                else {
                    z_agg += (1 + edu_z[ppl_edu]) * (1 - edu_rho[ppl_edu]);
                }
            }
            else if (i_occp0_sim_tmp == OCCP_ENTR) {
            ...
            w0_tmp = w0;
            bool entrepreneur_operated_this_period = (i_occp0_sim_tmp == OCCP_ENTR);
            ...
            if (i_occp0_sim_tmp == OCCP_ENTR) {// to be an entrepreneur
                if (use_entrepreneur_closure_risk) {
            ...
            else if (i_occp0_sim_tmp == OCCP_ENTR) {//en
                if (entrepreneur_operated_this_period) {
    ...
    if (use_self_employment_root) {
        ...
        for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
            if (i_occp0_sim[ppl_sim] != OCCP_ENTR) {
                continue;
            }
            if (!entrepreneur_closure_preserve_current_income && occp0_sim[ppl_sim] != OCCP_ENTR) {
                continue;
            }
...
void assign_value()
{
	...
	//========================================================================
	// PART UBI: UNIVERSAL BASIC INCOME SCAN (manna-from-heaven, unfunded)
	// Cases 148-151: UBI at 0.2, 0.4, 0.6, 0.8 of mean wage
	// Baseline is case 147 (no UBI). All other parameters identical.
	// Transfer added to asset_income in all states (employed, unemployed, entrepreneur).
	// Not revenue-neutral: isolates outside-option channel only.
	//========================================================================

	// Case 148: UBI = 0.2 * mean wage
	if (i_case == 148) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		for (int i_loop = 0; i_loop < 3; i_loop++) { x_share_edu[i_loop] = x_share_dims[i_loop]; }
		bar_m = 0.9; mu = 0.68; kappa = 0.035;
		lowerincome_b = 0.4; underline_c = 0.3;
		use_consumption_penalty = false; penalty_threshold = 0.0; penalty_weight = 0.0;
		use_fixed_tax_rate = false; tau_y_baseline = 0.0; t_lumpsum_endogenous = 0.0;
		use_self_employment_root = true; self_employment_owner_labor = 1.0;
		self_employment_hired_scale = 1.0; self_employment_n_threshold = 1e-4;
		self_employment_x_scale = 1.0; self_employment_hiring_fixed_cost = 0.25;
		chi_self_emp = 0.0; chi_employer = 0.0;
		use_entrepreneur_closure_risk = true;
		entrepreneur_closure_preserve_current_income = true;
		entrepreneur_capital_loss_share = 0.25;
		baseline_total_tax = 0.0; baseline_UI_spending = 0.0;
		baseline_cfloor_spending = 0.0; baseline_govt_spending = 0.0;
		for (int i_rho = 0; i_rho < Ne; i_rho++) {
			edu_rho[i_rho] = base_edu_rho[i_rho];
			edu_rho_entr[i_rho] = base_edu_rho_entr[i_rho];
		}
		ubi_transfer_level = 0.2; // UBI = 20% of mean wage
	}

	// Case 149: UBI = 0.4 * mean wage (equal to current UI replacement rate)
	if (i_case == 149) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		for (int i_loop = 0; i_loop < 3; i_loop++) { x_share_edu[i_loop] = x_share_dims[i_loop]; }
		...
		ubi_transfer_level = 0.4; // UBI = 40% of mean wage (benchmark: same rate as UI)
	}

	// Case 150: UBI = 0.6 * mean wage
	if (i_case == 150) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		for (int i_loop = 0; i_loop < 3; i_loop++) { x_share_edu[i_loop] = x_share_dims[i_loop]; }
		...
		ubi_transfer_level = 0.6; // UBI = 60% of mean wage
	}

	// Case 151: UBI = 0.8 * mean wage
	if (i_case == 151) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		for (int i_loop = 0; i_loop < 3; i_loop++) { x_share_edu[i_loop] = x_share_dims[i_loop]; }
		...
		ubi_transfer_level = 0.8; // UBI = 80% of mean wage
	}
