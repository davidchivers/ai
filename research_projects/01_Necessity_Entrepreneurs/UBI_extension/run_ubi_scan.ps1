...
$exe = Join-Path $root 'cfv_ubi_extension.exe'
$src = $null
# UBI extension: default source is the self-contained UBI source in this folder
$ubiSource = Join-Path $root 'source\main_2025_v1_ubi_extension.cpp'
$canonicalMain = if (Test-Path $ubiSource) { $ubiSource } else { $null }
...
if (-not [string]::IsNullOrWhiteSpace($SourceFile)) {
    ...
    $srcCandidates = @(
        $canonicalMain,
        (Join-Path $root 'source\main_2025_v1_ubi_extension.cpp')
...
try {
    ...
    Set-OrClearEnv -Name 'CFV_UBI_TRANSFER' -Value $UBITransfer
    ...
    $meta = @(
        ...
        "[ABLT] ubi_transfer=$($env:CFV_UBI_TRANSFER)",
