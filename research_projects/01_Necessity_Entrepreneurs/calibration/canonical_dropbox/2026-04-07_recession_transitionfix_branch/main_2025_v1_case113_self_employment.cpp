...
// ============================================================================
// COAUTHOR ANNOTATION: what changed in this file relative to the benchmark
// `main_2025_v1_case113.cpp`
//
// 1. This branch adds a genuine self-employment corner:
//    entrepreneurs can operate with zero hired workers.
//    In the benchmark employer-only file, `n = 0` implied zero output.
//
// 2. The static entrepreneurial problem is no longer the old closed-form
//    employer-only span-of-control block. Instead, this file solves a
//    numerical static problem over hired labor and capital using owner labor.
//
// 3. Simulation diagnostics now split entrepreneurs into:
//    - self-employed / nonemployer
//    - employer entrepreneurs
//    This was added because the whole point of this branch is to see whether
//    the model generates one-person firms rather than only relabeling employers.
//
// 4. Cases 141-144 are experimental self-employment cases layered on top of
//    the benchmark environment.
//
// 5. Entrepreneur closure risk is implemented in benchmark form:
//    when switched on, failed entrepreneurs move into a separate
//    non-UI unemployment state rather than the regular UI branch.
//    This keeps the institutional assumption explicit in the code.
//
// 6. Important debugging background:
//    the zero-UI convergence problems in the benchmark were partly numerical
//    (interpolation/indexing/output mapping), not purely economic. This file
//    inherits those guardrails, but its main new contribution is the
//    self-employment mechanism rather than a fresh convergence fix.
// ============================================================================
...
double best_agg_residual_l1 = std::numeric_limits<double>::infinity();
int best_agg_iter = 0;
double underline_c;
...
//++++++++++++++++++++++++++++++
//experiments: self-employment root
//++++++++++++++++++++++++++++++
// These controls are the main "switchboard" for the self-employment branch.
// If `use_self_employment_root` is false, the code falls back to the benchmark
// employer-only behavior.
bool use_self_employment_root;   // allow entrepreneurs to operate with zero hired workers
double self_employment_owner_labor;   // owner labor input in effective-labor units
double self_employment_hired_scale;   // scale on hired labor in production
double self_employment_n_threshold;   // threshold for classifying self-employed vs employer
double self_employment_x_scale;       // productivity scale applied to the no-hire self-employed corner
double self_employment_hiring_fixed_cost; // fixed cost paid whenever hired labor is strictly positive
double self_employment_entry_fixed_cost; // one-time cost paid when entering entrepreneurship from outside
double self_employment_employer_entry_fixed_cost; // extra one-time cost paid when entering directly as an employer
double chi_self_emp;                  // utility shifter for self-employed entrepreneurs
double chi_employer;                  // utility shifter for employer entrepreneurs
bool use_entrepreneur_closure_risk;   // benchmark entrepreneur closure risk switch
bool entrepreneur_closure_preserve_current_income; // if true, failure hits next period only
double entrepreneur_capital_loss_share; // fraction of installed capital lost on closure
...
double z_agg;
// Legacy naming note:
// `theta_ut` / `theta_ut_new` store the unemployed worker's job-finding
// probability p(Theta), not labor-market tightness Theta = v/u itself.
// This is why the update below uses bar_m * (v/u)^(1-mu) and why the
// simulation uses `theta_ut_new` directly as a match probability.
...
int ui_benefit_mode = 0;                    // 0 = flat reduced-form object, 1 = prior-wage replacement with direct benefit cap
double ui_benefit_replacement_rate = 0.50; // off-the-shelf Popp-style replacement rate
double ui_benefit_cap_mult = 2.0 / 3.0;   // weekly cap share, mapped to average worker wage
double ui_benefit_duration_share = 0.50;  // 26 weeks in an annual model period
bool one_shot_ui = false;                  // benchmark timing flag: UI eligibility lasts one annual period
...
bool env_override_underline_c = false;
double env_underline_c = 0.0;
bool env_override_ui_benefit_mode = false;
int env_ui_benefit_mode = 0;
bool env_override_ui_benefit_replacement_rate = false;
double env_ui_benefit_replacement_rate = 0.50;
bool env_override_ui_benefit_cap_mult = false;
double env_ui_benefit_cap_mult = 2.0 / 3.0;
bool env_override_ui_benefit_duration_share = false;
double env_ui_benefit_duration_share = 0.50;
...
bool env_override_fixed_r = false;
double env_fixed_r = 0.0;
bool env_override_fixed_w = false;
double env_fixed_w = 0.0;
bool env_override_init_theta = false;
double env_init_theta = 0.0;
bool env_override_init_r = false;
double env_init_r = 0.0;
bool env_override_init_w = false;
double env_init_w = 0.0;
bool env_override_init_tau = false;
double env_init_tau = 0.0;
...
bool override_self_employment_entry_fixed_cost = false;
double self_employment_entry_fixed_cost_override = 0.0;
bool override_self_employment_employer_entry_fixed_cost = false;
double self_employment_employer_entry_fixed_cost_override = 0.0;
bool override_self_employment_x_scale = false;
double self_employment_x_scale_override = 1.0;
...
bool env_override_output_wedge = false;
double env_output_wedge = 1.0;
bool env_override_one_shot_ui = false;
bool env_one_shot_ui = false;
bool env_export_transition_init = false;
bool env_export_transition_path = false;
std::string env_welfare_save_file;
std::string env_welfare_load_file;
std::string env_transition_init_state_file;
std::string env_transition_init_occp_file;
std::string env_transition_output_tag;
int env_transition_export_anchor_periods = 2;
bool welfare_benchmark_loaded = false;
int env_max_iter_agg = -1;
int env_single_case = -1;
int env_rng_seed = -1;
bool rng_seed_initialized = false;
int env_vacancy_mode = 2;
int env_vacancy_source_rule = -1;
double env_vacancy_floor = 1.0;
double env_vacancy_scale = 1e-3;
double env_update_step1 = -1.0;
double env_wage_update_step = -1.0;
double env_theta_update_step = -1.0;
double active_theta_update_step = 0.0;
double active_update_step1 = 0.0;
double active_wage_update_step = 0.0;
...
const int Nx_iid = 1;
const int Nz = 5;
const int OCCP_ENTR = 0;
const int OCCP_EMP = 1;
const int OCCP_UNEMP_UI = 2;
const int OCCP_UNEMP_NO_UI = 3;
const int OCCP_ENTR_EMP = 4;
const int CHOICE_ENTR = 0;
const int CHOICE_WORK = 1;
const int BUSINESS_NONE = 0;
const int BUSINESS_SELF_EMP = 1;
const int BUSINESS_EMP_1_4 = 2;
const int BUSINESS_EMP_5_19 = 3;
const int BUSINESS_EMP_20_PLUS = 4;
const int NbusinessTypes = 5;
const int Nchoice = 2; // entrepreneur vs worker
const int Ni = 5; // dim of current occupational state
const int Ni_hi = 3;
...
double v0[Na][Nx][Ni][Ne], v1[Na][Nx][Ni][Ne];                                     //value functions: (a, x, occp_state, edu)-->Value function.   
double vx[Na][Nx_iid][Ni][Ne];
double opt_occp_0[Na][Nx][Ni][Ne], opt_occp_1[Na][Nx][Ni][Ne];                     //policy function: (a, x, edu)-->occp choice.
double opt_a_0[Ni][Nchoice][Na][Nx][Ne], opt_a_1[Ni][Nchoice][Na][Nx][Ne];           //policy function: (I, i_E, a, x, edu)-->asset.
double opt_k_0[Ni][Nchoice][Na][Nx][Ne], opt_k_1[Ni][Nchoice][Na][Nx][Ne];           //policy function: (I, i_E, a, x, edu)-->loan.
double opt_n_0[Ni][Nchoice][Na][Nx][Ne], opt_n_1[Ni][Nchoice][Na][Nx][Ne];           //policy function: (I, i_E, a, x, edu)-->n demand.
double opt_rev_0[Ni][Nchoice][Na][Nx][Ne], opt_rev_1[Ni][Nchoice][Na][Nx][Ne];
double opt_c_0[Ni][Nchoice][Na][Nx][Ne], opt_c_1[Ni][Nchoice][Na][Nx][Ne];
//welfare
double v_ben[Na][Nx][Ni][Ne], cev[Na][Nx][Ni][Ne];

//pay consumption by goverment
double c_gov_pay[Ni][Nchoice][Na][Nx][Ne];
...
double opt_a_in[Ni][Nchoice][Na_in][Nx_in][Ne];           //policy function: (I, i_E, a, x, edu)-->asset.
double opt_k_in[Ni][Nchoice][Na_in][Nx_in][Ne];           //policy function: (I, i_E, a, x, edu)-->loan.
double opt_n_in[Ni][Nchoice][Na_in][Nx_in][Ne];           //policy function: (I, i_E, a, x, edu)-->n demand.
...
// 
//Educations:
// `edu_z` is the worker wage ladder used in employee earnings and UI.
// `edu_efficiency_z` is the separate efficiency-units ladder used in
// aggregate labor supply / matching objects. Keep them equal in the benchmark;
// separate them in wage-premium counterfactuals.
double edu_z[Ne] = { 0.0, 0.31, 0.61 };// worker wage premium
double edu_efficiency_z[Ne] = { 0.0, 0.31, 0.61 };// labor-efficiency units
double edu_rho[Ne] = { 0.0383,0.0288,0.0136 };//separation rate
double edu_rho_entr[Ne] = { 0.0,0.0,0.0 };// entrepreneur closure risk
...
double intp_2d_in_6arg(double& x_a0, double& x_x0, int& edu_occp,
	...
	double policy[Ni][Nchoice][Na_in][Nx_in][Ne]);
double update_step1;  //update r0, w0;

double prob_work_rate;
double branch2_asset_loss_amount;
...
namespace {
	...
	inline const char* vacancy_source_rule_label(const int rule)
	{
		switch (rule) {
		case 0: return "legacy";
		case 1: return "market_gap";
		case 2: return "incumbent_proto";
		case 3: return "replacement_offset";
		case 4: return "incumbent_scaled";
		default: return "modern_mode";
		}
	}

	inline double build_vacancy_mass_from_source_rule(
		const int rule,
		const double v_legacy,
		const double v_market_gap,
		const double v_incumbent,
		const double v_shrink_offset)
	{
		const double vacancy_floor = std::max(0.0, env_vacancy_floor);
		const double incumbent_scaled = std::max(0.0, env_vacancy_scale) * std::max(0.0, v_incumbent);
		double v_out = 0.0;
		switch (rule) {
		case 0:
			v_out = std::max(std::max(0.0, v_legacy), vacancy_floor);
			break;
		case 1:
			v_out = std::max(std::max(0.0, v_market_gap), vacancy_floor);
			break;
		case 2:
			v_out = std::max(std::max(0.0, v_incumbent), vacancy_floor);
			break;
		case 3:
			v_out = std::max(std::max(0.0, v_legacy - std::max(0.0, v_shrink_offset)), vacancy_floor);
			break;
		case 4:
			v_out = std::max(incumbent_scaled, vacancy_floor);
			break;
		default:
			v_out = std::max(std::max(0.0, v_legacy), vacancy_floor);
			break;
	...
	inline double incumbent_vacancy_flow_from_current_and_carried_n(
		const double n_current,
		const double n_prev)
	{
		return std::max(0.0, std::max(0.0, n_current) - std::max(0.0, n_prev));
	}
	...
	inline void create_directory_if_missing(const std::string& path)
	{
		if (path.empty()) {
			return;
		}
...
void apply_runtime_overrides()
{
	...
	double consumption_floor_env = 0.0;
	if (parse_env_double(getenv("CFV_CONSUMPTION_FLOOR"), consumption_floor_env) &&
		consumption_floor_env >= 0.0) {
		env_override_underline_c = true;
		env_underline_c = consumption_floor_env;
		cout << "[override] underline_c = " << env_underline_c << endl;
	}

	int ui_benefit_mode_env = 0;
	if (parse_env_int(getenv("CFV_UI_BENEFIT_MODE"), ui_benefit_mode_env) &&
		ui_benefit_mode_env >= 0) {
		env_override_ui_benefit_mode = true;
		env_ui_benefit_mode = ui_benefit_mode_env;
		cout << "[override] ui_benefit_mode = " << env_ui_benefit_mode << endl;
	}

	double ui_benefit_replacement_env = 0.0;
	if (parse_env_double(getenv("CFV_UI_BENEFIT_REPLACEMENT"), ui_benefit_replacement_env) &&
		ui_benefit_replacement_env >= 0.0) {
		env_override_ui_benefit_replacement_rate = true;
		env_ui_benefit_replacement_rate = ui_benefit_replacement_env;
		cout << "[override] ui_benefit_replacement_rate = " << env_ui_benefit_replacement_rate << endl;
	}

	double ui_benefit_cap_env = 0.0;
	const char* ui_benefit_cap_raw = getenv("CFV_UI_BENEFIT_CAP");
	if (ui_benefit_cap_raw == nullptr || *ui_benefit_cap_raw == '\0') {
		ui_benefit_cap_raw = getenv("CFV_UI_BENEFIT_EARNINGS_CAP");
	}
	if (parse_env_double(ui_benefit_cap_raw, ui_benefit_cap_env) &&
		ui_benefit_cap_env >= 0.0) {
		env_override_ui_benefit_cap_mult = true;
		env_ui_benefit_cap_mult = ui_benefit_cap_env;
		cout << "[override] ui_benefit_cap_mult = " << env_ui_benefit_cap_mult << endl;
	}

	double ui_benefit_duration_env = 0.0;
	if (parse_env_double(getenv("CFV_UI_BENEFIT_DURATION_SHARE"), ui_benefit_duration_env) &&
		ui_benefit_duration_env >= 0.0 && ui_benefit_duration_env <= 1.0) {
		env_override_ui_benefit_duration_share = true;
		env_ui_benefit_duration_share = ui_benefit_duration_env;
		cout << "[override] ui_benefit_duration_share = "
			 << env_ui_benefit_duration_share << endl;
	}

	int one_shot_ui_env = 0;
	if (parse_env_int(getenv("CFV_ONE_SHOT_UI"), one_shot_ui_env)) {
		env_override_one_shot_ui = true;
		env_one_shot_ui = (one_shot_ui_env > 0);
		cout << "[override] one_shot_ui = " << (env_one_shot_ui ? 1 : 0) << endl;
	}

	int export_transition_init_env = 0;
	if (parse_env_int(getenv("CFV_EXPORT_TRANSITION_INIT"), export_transition_init_env)) {
		env_export_transition_init = (export_transition_init_env > 0);
		cout << "[override] export_transition_init = " << (env_export_transition_init ? 1 : 0) << endl;
	}

	int export_transition_path_env = 0;
	if (parse_env_int(getenv("CFV_EXPORT_TRANSITION_PATH"), export_transition_path_env)) {
		env_export_transition_path = (export_transition_path_env > 0);
		cout << "[override] export_transition_path = " << (env_export_transition_path ? 1 : 0) << endl;
	}

	const char* transition_init_state_env = getenv("CFV_TRANSITION_INIT_STATE_FILE");
	if (transition_init_state_env != nullptr && *transition_init_state_env != '\0') {
		env_transition_init_state_file = transition_init_state_env;
		cout << "[override] transition_init_state_file = " << env_transition_init_state_file << endl;
	}

	const char* transition_init_occp_env = getenv("CFV_TRANSITION_INIT_OCCP_FILE");
	if (transition_init_occp_env != nullptr && *transition_init_occp_env != '\0') {
		env_transition_init_occp_file = transition_init_occp_env;
		cout << "[override] transition_init_occp_file = " << env_transition_init_occp_file << endl;
	}

	const char* transition_output_tag_env = getenv("CFV_TRANSITION_OUTPUT_TAG");
	if (transition_output_tag_env != nullptr && *transition_output_tag_env != '\0') {
		env_transition_output_tag = transition_output_tag_env;
		cout << "[override] transition_output_tag = " << env_transition_output_tag << endl;
	}

	int export_transition_anchor_periods_env = 0;
	if (parse_env_int(getenv("CFV_EXPORT_TRANSITION_ANCHOR_PERIODS"), export_transition_anchor_periods_env) &&
		export_transition_anchor_periods_env >= 0) {
		env_transition_export_anchor_periods = export_transition_anchor_periods_env;
		cout << "[override] export_transition_anchor_periods = "
			 << env_transition_export_anchor_periods << endl;
	}

	const char* welfare_save_env = getenv("CFV_WELFARE_SAVE_FILE");
	if (welfare_save_env != nullptr && *welfare_save_env != '\0') {
		env_welfare_save_file = welfare_save_env;
		cout << "[override] welfare_save_file = " << env_welfare_save_file << endl;
	}

	const char* welfare_load_env = getenv("CFV_WELFARE_LOAD_FILE");
	if (welfare_load_env != nullptr && *welfare_load_env != '\0') {
		env_welfare_load_file = welfare_load_env;
		cout << "[override] welfare_load_file = " << env_welfare_load_file << endl;
	}
	...
	double self_employment_entry_fixed_cost_env = 0.0;
	if (parse_env_double(getenv("CFV_SE_ENTRY_FIXED_COST"), self_employment_entry_fixed_cost_env) &&
		self_employment_entry_fixed_cost_env >= 0.0) {
		override_self_employment_entry_fixed_cost = true;
		self_employment_entry_fixed_cost_override = self_employment_entry_fixed_cost_env;
		cout << "[override] self_employment_entry_fixed_cost = "
			 << self_employment_entry_fixed_cost_override << endl;
	}

	double self_employment_employer_entry_fixed_cost_env = 0.0;
	if (parse_env_double(getenv("CFV_SE_EMPLOYER_ENTRY_FIXED_COST"), self_employment_employer_entry_fixed_cost_env) &&
		self_employment_employer_entry_fixed_cost_env >= 0.0) {
		override_self_employment_employer_entry_fixed_cost = true;
		self_employment_employer_entry_fixed_cost_override = self_employment_employer_entry_fixed_cost_env;
		cout << "[override] self_employment_employer_entry_fixed_cost = "
			 << self_employment_employer_entry_fixed_cost_override << endl;
	}

	double self_employment_x_scale_env = 1.0;
	if (parse_env_double(getenv("CFV_SE_X_SCALE"), self_employment_x_scale_env) &&
		self_employment_x_scale_env > 0.0) {
		override_self_employment_x_scale = true;
		self_employment_x_scale_override = self_employment_x_scale_env;
		cout << "[override] self_employment_x_scale = "
			 << self_employment_x_scale_override << endl;
	}
	...
	double fixed_r_override_value = 0.0;
	if (parse_env_double(getenv("CFV_FIXED_R"), fixed_r_override_value) &&
		fixed_r_override_value > 0.0) {
		env_override_fixed_r = true;
		env_fixed_r = fixed_r_override_value;
		cout << "[override] fixed_r = " << env_fixed_r << endl;
	}

	double fixed_w_override_value = 0.0;
	if (parse_env_double(getenv("CFV_FIXED_W"), fixed_w_override_value) &&
		fixed_w_override_value > 0.0) {
		env_override_fixed_w = true;
		env_fixed_w = fixed_w_override_value;
		cout << "[override] fixed_w = " << env_fixed_w << endl;
	}

	double init_theta_override_value = 0.0;
	if (parse_env_double(getenv("CFV_INIT_THETA"), init_theta_override_value) &&
		init_theta_override_value >= 0.0 && init_theta_override_value <= 1.0) {
		env_override_init_theta = true;
		env_init_theta = init_theta_override_value;
		cout << "[override] init_theta = " << env_init_theta << endl;
	}

	double init_r_override_value = 0.0;
	if (parse_env_double(getenv("CFV_INIT_R"), init_r_override_value) &&
		init_r_override_value > 0.0) {
		env_override_init_r = true;
		env_init_r = init_r_override_value;
		cout << "[override] init_r = " << env_init_r << endl;
	}

	double init_w_override_value = 0.0;
	if (parse_env_double(getenv("CFV_INIT_W"), init_w_override_value) &&
		init_w_override_value > 0.0) {
		env_override_init_w = true;
		env_init_w = init_w_override_value;
		cout << "[override] init_w = " << env_init_w << endl;
	}

	double init_tau_override_value = 0.0;
	if (parse_env_double(getenv("CFV_INIT_TAU"), init_tau_override_value) &&
		init_tau_override_value >= 0.0) {
		env_override_init_tau = true;
		env_init_tau = init_tau_override_value;
		cout << "[override] init_tau = " << env_init_tau << endl;
	}
	...
	int vacancy_source_rule_override_value = -1;
	if (parse_env_int(getenv("CFV_VACANCY_SOURCE_RULE"), vacancy_source_rule_override_value) && vacancy_source_rule_override_value >= 0) {
		env_vacancy_source_rule = vacancy_source_rule_override_value;
		cout << "[override] vacancy_source_rule = " << env_vacancy_source_rule
			 << " (" << vacancy_source_rule_label(env_vacancy_source_rule) << ")" << endl;
	}
	...
	double update_step_override_value = 0.0;
	if (parse_env_double(getenv("CFV_UPDATE_STEP1"), update_step_override_value) &&
		update_step_override_value >= 0.0 && update_step_override_value < 1.0) {
		env_update_step1 = update_step_override_value;
		cout << "[override] update_step1 = " << env_update_step1 << endl;
	}

	double wage_update_step_override_value = 0.0;
	if (parse_env_double(getenv("CFV_WAGE_UPDATE_STEP"), wage_update_step_override_value) &&
		wage_update_step_override_value >= 0.0 && wage_update_step_override_value < 1.0) {
		env_wage_update_step = wage_update_step_override_value;
		cout << "[override] wage_update_step = " << env_wage_update_step << endl;
	}

	double theta_update_step_override_value = 0.0;
	if (parse_env_double(getenv("CFV_THETA_UPDATE_STEP"), theta_update_step_override_value) &&
		theta_update_step_override_value >= 0.0 && theta_update_step_override_value < 1.0) {
		env_theta_update_step = theta_update_step_override_value;
		cout << "[override] theta_update_step = " << env_theta_update_step << endl;
	}
	...
	double output_wedge_env = 0.0;
	if (parse_env_double(getenv("CFV_OUTPUT_WEDGE"), output_wedge_env) &&
		output_wedge_env > 0.0) {
		env_override_output_wedge = true;
		env_output_wedge = output_wedge_env;
		cout << "[override] output_wedge = " << env_output_wedge << endl;
	}
}
...
double v_func(DP x)
{
	...
	expt_value = 0.0;
	double a1_branch1 = a1;
	double a1_branch2 = std::max(a_l, a1 - std::max(0.0, branch2_asset_loss_amount));
	expt_value = expt_v(a1_branch1, x_state, i_occp_through[0], i_edu_occp);
	...
	expt_value = 0.0;
	expt_value = expt_v(a1_branch2, x_state, i_occp_through[1], i_edu_occp);
	double u_v_2 = -(utility + beta * expt_value);
	
	return prob_work_rate * u_v_1 + (1 - prob_work_rate) * u_v_2;
}

inline bool is_entrepreneur_state(int occp_state)
{
	return occp_state == OCCP_ENTR || occp_state == OCCP_ENTR_EMP;
}

inline int entrepreneur_state_from_n(double n_h)
{
	return (std::max(0.0, n_h) <= self_employment_n_threshold) ? OCCP_ENTR : OCCP_ENTR_EMP;
}

inline bool is_nonemployed_state(int occp_state)
{
	return is_entrepreneur_state(occp_state) || occp_state == OCCP_UNEMP_UI || occp_state == OCCP_UNEMP_NO_UI;
}

inline bool has_regular_ui(int occp_state)
{
	return occp_state == OCCP_UNEMP_UI;
}

inline int classify_business_type(bool entrepreneur_operated, double n_h)
{
	if (!entrepreneur_operated) {
		return BUSINESS_NONE;
	}
	const double n_pos = std::max(0.0, n_h);
	if (n_pos <= self_employment_n_threshold) {
		return BUSINESS_SELF_EMP;
	}
	if (n_pos <= 4.0) {
		return BUSINESS_EMP_1_4;
	}
	if (n_pos <= 19.0) {
		return BUSINESS_EMP_5_19;
	}
	return BUSINESS_EMP_20_PLUS;
}

inline bool is_employer_business_type(int business_type)
{
	return business_type == BUSINESS_EMP_1_4
		|| business_type == BUSINESS_EMP_5_19
		|| business_type == BUSINESS_EMP_20_PLUS;
}

inline const char* business_type_label(int business_type)
{
	switch (business_type) {
	case BUSINESS_NONE:
		return "nonbusiness";
	case BUSINESS_SELF_EMP:
		return "self_employed";
	case BUSINESS_EMP_1_4:
		return "employer_1_4";
	case BUSINESS_EMP_5_19:
		return "employer_5_19";
	case BUSINESS_EMP_20_PLUS:
		return "employer_20_plus";
	default:
		return "unknown";
	}
}

inline double ui_average_worker_wage(double w_base)
{
	double avg_wage = 0.0;
	for (int i_e = 0; i_e < Ne; ++i_e) {
		avg_wage += edu_share[i_e] * (1.0 + edu_z[i_e]) * w_base;
	}
	return avg_wage;
}

inline double regular_ui_benefit_amount(int edu_idx, double w_base)
{
	if (ui_benefit_mode == 1) {
		const int i_e = std::max(0, std::min(Ne - 1, edu_idx));
		const double prior_wage = (1.0 + edu_z[i_e]) * w_base;
		double ui_benefit = ui_benefit_replacement_rate * prior_wage;
		if (ui_benefit_cap_mult > 0.0) {
			const double ui_benefit_cap = ui_benefit_cap_mult * ui_average_worker_wage(w_base);
			ui_benefit = std::min(ui_benefit, ui_benefit_cap);
		}
		return ui_benefit_duration_share * ui_benefit;
	}
	return lowerincome_b * w_base;
}

bool save_welfare_benchmark(const std::string& path);
bool load_welfare_benchmark(const std::string& path);

// ---------------------------------------------------------------------------
// Self-employment static firm block
//
// Benchmark file:
// - firm scale is built around the old employer-only technology
// - the entrepreneurial problem effectively starts from hired labor
//
// This file:
// - owner labor enters production directly
// - `n_h = 0` is a feasible self-employment corner
// - the static problem is solved numerically so the branch can compare
//   self-employed vs employer choices cleanly
// ---------------------------------------------------------------------------
inline double entrepreneur_effective_hired_labor(double n_h)
{
	return std::max(0.0, self_employment_hired_scale * z_agg * std::max(0.0, n_h));
}

inline double entrepreneur_total_labor_input(double n_h)
{
	const double hired_labor = entrepreneur_effective_hired_labor(n_h);
	if (use_self_employment_root) {
		return std::max(0.0, self_employment_owner_labor + hired_labor);
	}
	return hired_labor;
}

inline double entrepreneur_effective_x(double x_val, double n_h)
{
	if (!use_self_employment_root) {
		return x_val;
	}
	const double n_pos = std::max(0.0, n_h);
	if (n_pos <= self_employment_n_threshold) {
		return self_employment_x_scale * x_val;
	}
	return x_val;
}

inline double entrepreneur_output_static(double x_val, double k_val, double n_h)
{
	if (!std::isfinite(x_val) || !std::isfinite(k_val) || !std::isfinite(n_h)) {
		return 0.0;
	}
	const double x_eff = entrepreneur_effective_x(x_val, n_h);
	if (x_eff <= 0.0 || k_val <= 0.0) {
		return 0.0;
	}
	const double labor_input = entrepreneur_total_labor_input(n_h);
	if (labor_input <= 0.0) {
		return 0.0;
	}
	return x_eff * pow(k_val, alpha) * pow(labor_input, gamma_param);
}

inline double entrepreneur_hiring_cost(double wage_val, double n_h)
{
	const double n_pos = std::max(0.0, n_h);
	if (n_pos <= 0.0) {
		return 0.0;
	}
	// `theta_ut` is the worker-side job-finding probability p(Theta).
	// If the paper's recruiting block is interpreted literally, the firm-side
	// denominator here should instead be the vacancy fill rate q(Theta).
	const double theta_denom = std::max(theta_ut, 1e-8);
	double hiring_cost = z_agg * wage_val * (1 + psi * tau) * n_pos
		+ (kappa * wage_val * rho_aver * z_agg * n_pos) / theta_denom;
	if (use_self_employment_root && n_pos > self_employment_n_threshold) {
		hiring_cost += self_employment_hiring_fixed_cost;
	}
	return hiring_cost;
}

inline double entrepreneur_capital_cost(double a_val, double k_val)
{
	const double k_pos = std::max(0.0, k_val);
	const double liquid_assets = std::max(0.0, a_val - m0_credit);
	if (k_pos <= liquid_assets) {
		return r0 * k_pos;
	}
	return r0 * liquid_assets + (1.0 + r_wedge) * r0 * (k_pos - liquid_assets);
}

inline double entrepreneur_profit_given_choices(double a_val, double x_val, double k_val, double n_h, double wage_val)
{
	return entrepreneur_output_static(x_val, k_val, n_h)
		- entrepreneur_hiring_cost(wage_val, n_h)
		- entrepreneur_capital_cost(a_val, k_val);
}

double entrepreneur_best_k_for_n(double a_val, double x_val, double n_h, double wage_val)
{
	const double labor_input = entrepreneur_total_labor_input(n_h);
	const double x_eff = entrepreneur_effective_x(x_val, n_h);
	if (!std::isfinite(labor_input) || labor_input <= 0.0 || x_eff <= 0.0) {
		return 0.0;
	}

	const double liquid_assets = std::max(0.0, a_val - m0_credit);
	const double revenue_term = x_eff * pow(labor_input, gamma_param);
	const double r_high = std::max((1.0 + r_wedge) * r0, 1e-8);
	const double r_low = std::max(r0, 1e-8);

	double candidates[4] = { 0.0, liquid_assets, 0.0, 0.0 };
	int candidate_count = 2;

	const double k_low = pow(alpha * revenue_term / r_low, 1.0 / (1.0 - alpha));
	if (std::isfinite(k_low) && k_low >= 0.0 && k_low <= liquid_assets + 1e-10) {
		candidates[candidate_count++] = k_low;
	}

	const double k_high = pow(alpha * revenue_term / r_high, 1.0 / (1.0 - alpha));
	if (std::isfinite(k_high) && k_high >= liquid_assets - 1e-10) {
		candidates[candidate_count++] = std::max(0.0, k_high);
	}

	double best_k = 0.0;
	double best_profit = -1.0e300;
	for (int i_candidate = 0; i_candidate < candidate_count; ++i_candidate) {
		const double k_candidate = std::max(0.0, candidates[i_candidate]);
		const double profit_candidate = entrepreneur_profit_given_choices(a_val, x_val, k_candidate, n_h, wage_val);
...
double entrepreneur_profit_best_k(double a_val, double x_val, double n_h, double wage_val, double* k_out = nullptr)
{
	const double k_best = entrepreneur_best_k_for_n(a_val, x_val, n_h, wage_val);
	if (k_out != nullptr) {
		*k_out = k_best;
	}
	return entrepreneur_profit_given_choices(a_val, x_val, k_best, n_h, wage_val);
}

void solve_self_employment_static_problem(double a_val, double x_val, double wage_val,
	double& k_opt_out, double& n_opt_out, double& rev_opt_out)
{
	k_opt_out = 0.0;
	n_opt_out = 0.0;
	rev_opt_out = 0.0;
	...
	double n_upper = 1.0;
	double k_tmp = 0.0;
	double profit_prev = entrepreneur_profit_best_k(a_val, x_val, 0.0, wage_val, &k_tmp);
	for (int i_expand = 0; i_expand < 8; ++i_expand) {
		double k_expand = 0.0;
		const double profit_expand = entrepreneur_profit_best_k(a_val, x_val, n_upper, wage_val, &k_expand);
		if (!std::isfinite(profit_expand) || profit_expand <= profit_prev) {
			break;
		}
		profit_prev = profit_expand;
		n_upper = std::min(256.0, n_upper * 2.0);
		if (n_upper >= 256.0) {
			break;
		}
	}
	...
	double best_n = 0.0;
	double best_k = 0.0;
	double best_profit = entrepreneur_profit_best_k(a_val, x_val, 0.0, wage_val, &best_k);

	const int coarse_points = 80;
	const double coarse_step = n_upper / static_cast<double>(coarse_points);
	for (int i_grid = 1; i_grid <= coarse_points; ++i_grid) {
		const double n_candidate = coarse_step * static_cast<double>(i_grid);
		double k_candidate = 0.0;
		const double profit_candidate = entrepreneur_profit_best_k(a_val, x_val, n_candidate, wage_val, &k_candidate);
		if (profit_candidate > best_profit) {
			best_profit = profit_candidate;
			best_n = n_candidate;
			best_k = k_candidate;
	...
	const double refine_low = std::max(0.0, best_n - coarse_step);
	const double refine_high = std::min(n_upper, best_n + coarse_step);
	const int refine_points = 40;
	for (int i_refine = 0; i_refine <= refine_points; ++i_refine) {
		const double share = static_cast<double>(i_refine) / static_cast<double>(refine_points);
		const double n_candidate = refine_low + (refine_high - refine_low) * share;
		double k_candidate = 0.0;
		const double profit_candidate = entrepreneur_profit_best_k(a_val, x_val, n_candidate, wage_val, &k_candidate);
		if (profit_candidate > best_profit) {
			best_profit = profit_candidate;
	...
	k_opt_out = std::max(0.0, best_k);
	n_opt_out = (best_n <= self_employment_n_threshold) ? 0.0 : std::max(0.0, best_n);
	rev_opt_out = std::max(0.0, best_profit);
}
	...
	double& k_opt_in, double n_opt_in, double& rev_opt_in, double prob_rho, double prob_theta_u)
{
	...
	if (i_occp1 == CHOICE_WORK) { // choose to be worker

		const double asset_income = (1 + r0 - dep_k) * a0 - t_lumpsum_hhd;
		income_tmp = asset_income + (z0 * w0_tmp * (1 - (1 - psi) * tau_y0));
		income_tmp_un = asset_income;
		...
		if (i_occp0 == OCCP_EMP) // currently employed
		{
			cx = income_tmp;
			prob_work_rate = 1 - prob_rho;
			income_tmp_un = asset_income + regular_ui_benefit_amount(i_edu_occp, w0_tmp);
			i_occp_through[0] = OCCP_EMP;
			i_occp_through[1] = OCCP_UNEMP_UI;
		}
		else if (i_occp0 == OCCP_UNEMP_UI) // currently unemployed with UI eligibility
		{
			cx = income_tmp;
			prob_work_rate = prob_theta_u;
			income_tmp_un = asset_income + regular_ui_benefit_amount(i_edu_occp, w0_tmp);
			i_occp_through[0] = OCCP_EMP;
			i_occp_through[1] = one_shot_ui ? OCCP_UNEMP_NO_UI : OCCP_UNEMP_UI;
		}
		else if (is_entrepreneur_state(i_occp0) || i_occp0 == OCCP_UNEMP_NO_UI) // exiting entrepreneurship or no-UI unemployed
		{
			cx = income_tmp;
			prob_work_rate = prob_theta_u;
			i_occp_through[0] = OCCP_EMP;
			i_occp_through[1] = OCCP_UNEMP_NO_UI;
		}
	...
	//entrepreneur's decision
	else if (i_occp1 == CHOICE_ENTR) { // choose to be enterprenur
		...
		l_opt_v = k_opt - (a0);
		double entrepreneur_entry_cost = 0.0;
		const bool was_entrepreneur = is_entrepreneur_state(i_occp0);
		const bool becomes_employer = (n_opt > self_employment_n_threshold);
		const bool was_employer = (i_occp0 == OCCP_ENTR_EMP);
		if (use_self_employment_root) {
			if (!was_entrepreneur) {
				entrepreneur_entry_cost += self_employment_entry_fixed_cost;
			}
			if (becomes_employer && !was_employer) {
				entrepreneur_entry_cost += self_employment_employer_entry_fixed_cost;
			}
		}

		const double asset_income = (1 + r0 - dep_k) * a0 - t_lumpsum_hhd;
		income_tmp = asset_income + rev_opt - tau_pi - entrepreneur_entry_cost;
		income_tmp_un = income_tmp;
		...
		//check
		if (use_entrepreneur_closure_risk) {
			prob_work_rate = 1.0 - clamp_probability(edu_rho_entr[i_edu_occp]);
			income_tmp_un = entrepreneur_closure_preserve_current_income ? income_tmp : asset_income;
			branch2_asset_loss_amount = entrepreneur_capital_loss_share * std::max(0.0, k_opt);
			i_occp_through[0] = entrepreneur_state_from_n(n_opt);
			i_occp_through[1] = OCCP_UNEMP_NO_UI;
		}
		else {
			prob_work_rate = 1.0;
			i_occp_through[0] = entrepreneur_state_from_n(n_opt);
			i_occp_through[1] = entrepreneur_state_from_n(n_opt);
		}
		...
		v_tmp_ins = -v_func(xmin1);
		// Current calibration keeps these shifters at zero, but the branch is
		// here so the Bellman logic can later distinguish nonemployer
		// self-employment from employer entrepreneurship without another rewrite.
		if (use_self_employment_root) {
			if (n_opt <= self_employment_n_threshold) {
				v_tmp_ins += chi_self_emp;
			}
			else {
				v_tmp_ins += chi_employer;
			}
		}
		a1_tmp_ins = xmin1;
		k_tmp_ins = k_opt;
	...
	do {
		...
		for (int i_e = 0; i_e < Ne; i_e++) {
			...
			for (int i_x = 0; i_x < Nx; i_x++) {
				...
				x0_in = (1 + edu_z[i_e]);// worker wage premium
				//change for new
				...
				for (int i_a = 0; i_a < Na; i_a++) {
					...
					for (int i_occp = 0; i_occp < Ni; i_occp++) {
						...
						for (int i_occp_e = 0; i_occp_e < Nchoice; i_occp_e++)
						{
							i_occp_en = i_occp_e;
...
double intp_2d(double& x_a0, double& x_x0, int& i_x_occp,
	...
	double policy[Ni][Nchoice][Na][Nx][Ne]);
...
double intp_opt_sim(double& a0_in, double& x0_in, int& i_x_occp, int& i_occp0_in, int& edu_occp,
	double opt_in[Ni][Nchoice][Na][Nx][Ne]);

//print out results:
bool save_transition_init_cross_section(const std::string& path);
bool load_transition_init_cross_section(
	const std::string& path,
	std::vector<double>& a_state,
	std::vector<double>& x_state,
	std::vector<int>& occp_state);
void maybe_export_transition_init_cross_section();
void export_transition_path_direct();
...
{
    // Coauthor note:
    // `occp0_sim` tracks realized previous-period labor-market state:
    //   0 = entrepreneur
    //   1 = employed worker
    //   2 = unemployed / nonemployed worker-side state
    //
    // This is still the benchmark 3-state structure. There is not yet a
    // separate post-closure entrepreneur state with distinct UI eligibility.
    //static
    ...
    for (int t_sim = 0; t_sim < Nsim_pd; t_sim++) {
        ...
        if (t_sim == 0) {
			...
			const double n_prev_for_flow = is_entrepreneur_state(occp0_sim_tmp) ? std::max(0.0, n_sim[ppl_sim]) : 0.0;
                const double incumbent_flow = incumbent_vacancy_flow_from_current_and_carried_n(
                    n_policy,
                    n_prev_for_flow);

                cacu_0 += prob_entrepreneur;
                expected_n_demand_pre += prob_entrepreneur * n_policy;
                v_incumbent_pre += prob_entrepreneur * incumbent_flow;
                if (occp0_sim[ppl_sim] != OCCP_EMP) {
                    cacu_2 += prob_work;
                }
                else if (occp0_sim[ppl_sim] == OCCP_EMP) {
            ...
            const double v_market_gap_pre = std::max(0.0, expected_n_demand_pre - n_supply);
            if (env_vacancy_source_rule >= 0) {
                v_t = build_vacancy_mass_from_source_rule(
                    env_vacancy_source_rule,
                    v_legacy_pre,
                    v_market_gap_pre,
                    v_incumbent_pre,
                    0.0);
            ...
            if (u_t > 1e-10 && std::isfinite(u_t)) {
                ...
                // This is worker job finding p(Theta) = bar_m * (v/u)^(1-mu),
                // not tightness Theta = v/u itself.
        ...
        for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ppl_sim++) {
            ...
            x0_sim_tmp = x0_sim[ppl_sim];
            occp0_sim_tmp = occp0_sim[ppl_sim];
            int occp_prev_state = occp0_sim_tmp;
            ...
            if (rnd_choice <= prob_work) {
                i_occp0_sim_tmp = CHOICE_WORK; // Worker
            } else {
                i_occp0_sim_tmp = CHOICE_ENTR; // Entrepreneur
            }
            // ---------------------------------------------------------
            n0_sim_tmp = 0.0;
            k0_sim_tmp = 0.0;
            ...
            if (i_occp0_sim_tmp == CHOICE_WORK) {
                if (occp_prev_state != OCCP_EMP) {
                    z_agg += (1 + edu_efficiency_z[ppl_edu]) * theta_ut_new;
                }
                else {
                    z_agg += (1 + edu_efficiency_z[ppl_edu]) * (1 - edu_rho[ppl_edu]);
                }
            }
            else if (i_occp0_sim_tmp == CHOICE_ENTR) {
                const double n0_raw = intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, i_occp0_sim_tmp, ppl_edu, opt_n_0);
                const double k0_raw = intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, i_occp0_sim_tmp, ppl_edu, opt_k_0);
                ...
                if (use_self_employment_root) {
                    y_sim[ppl_sim] = entrepreneur_output_static(x0_sim_tmp, k0_sim_tmp, n0_sim_tmp);
                }
                else {
            ...
            a1_sim_tmp = intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, i_occp0_sim_tmp, ppl_edu, opt_a_0);
            c0_sim_tmp = intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, i_occp0_sim_tmp, ppl_edu, opt_c_0);
            //change for edu dim
            rev_sim_tmp = intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, i_occp0_sim_tmp, ppl_edu, opt_rev_0);
            //cout << "6" << endl;
            //tracking one agent
            //change for no health
            ...
            w0_tmp = w0;
            bool entrepreneur_operated_this_period = (i_occp0_sim_tmp == CHOICE_ENTR);
            ...
            if (i_occp0_sim_tmp == CHOICE_ENTR) {// to be an entrepreneur
                if (use_entrepreneur_closure_risk) {
                    random_num = ((double)rand()) / RAND_MAX;
                    const double prob_closure = clamp_probability(edu_rho_entr[ppl_edu]);
                    if (random_num <= prob_closure) {
                        occp0_sim[ppl_sim] = OCCP_UNEMP_NO_UI;
                        a1_sim_tmp = std::max(a_l, a1_sim_tmp - entrepreneur_capital_loss_share * std::max(0.0, k0_sim_tmp));
                        if (!entrepreneur_closure_preserve_current_income) {
                            entrepreneur_operated_this_period = false;
                            n0_sim_tmp = 0.0;
                            k0_sim_tmp = 0.0;
                            y_sim[ppl_sim] = 0.0;
                            rev_sim_tmp = 0.0;
                        ...
                        occp0_sim[ppl_sim] = entrepreneur_state_from_n(n0_sim_tmp);
                    }
                }
                else {
                    occp0_sim[ppl_sim] = entrepreneur_state_from_n(n0_sim_tmp);
                ...
                if (occp_prev_state != OCCP_EMP) {// unemployed / entrepreneur searching
                    random_num = ((double)rand()) / RAND_MAX;
                    // `theta_ut_new` is already the worker-side job-finding
                    // probability p(Theta), so it is used directly here.
                    const double prob_find_job = clamp_probability(theta_ut_new);
                    if (random_num <= prob_find_job) {
                        occp0_sim[ppl_sim] = OCCP_EMP;
                        cacu_unemp += 1;
                    }
                    else {
                        if (one_shot_ui && occp_prev_state == OCCP_UNEMP_UI) {
                            occp0_sim[ppl_sim] = OCCP_UNEMP_NO_UI;
                        }
                        else {
                            occp0_sim[ppl_sim] = has_regular_ui(occp_prev_state) ? OCCP_UNEMP_UI : OCCP_UNEMP_NO_UI;
                        }
                    }
                    cacu_tot_unemp += 1;

                }
                else if (occp_prev_state == OCCP_EMP) {// employ
                    random_num = ((double)rand()) / RAND_MAX;
                    
                    const double prob_separation = clamp_probability(edu_rho[ppl_edu]);
                    if (random_num <= prob_separation) {
                        occp0_sim[ppl_sim] = OCCP_UNEMP_UI;
                    }
                    else{
                        occp0_sim[ppl_sim] = OCCP_EMP;
                        cacu_emp += 1;
            ...
            const double n_prev_for_flow = is_entrepreneur_state(occp_prev_state) ? std::max(0.0, n_sim[ppl_sim]) : 0.0;
            ...
            if (occp0_sim_tmp == OCCP_EMP) {//employ
                n_supply += (1 + edu_efficiency_z[ppl_edu]);
            }
            if (entrepreneur_operated_this_period) {
                entr_ratio += 1.0;
                entr_count += 1;
                entr_count_edu[ppl_edu] += 1;
                n_entr_sum += n0_sim_tmp;
                k_entr_sum += k0_sim_tmp;
                v_incumbent_post += incumbent_vacancy_flow_from_current_and_carried_n(
                    n0_sim_tmp,
                    n_prev_for_flow);
            ...
            if (entrepreneur_operated_this_period && n0_sim_tmp >= 100.0) {
                firm_big += 1.0;
            }

            //cacu income and tax
            if (i_occp0_sim_tmp == OCCP_EMP) {
                
                //if have a work pay tax and with aver_work_prob to have a work

                if (occp0_sim_tmp == OCCP_UNEMP_UI) {
                    const double ui_benefit = regular_ui_benefit_amount(ppl_edu, w0_tmp);
                    income_sim = (r0)*a0_sim_tmp + ui_benefit;
                    total_lower_out += ui_benefit;
                    tax_income = 0.0;
                }
                else if (occp0_sim_tmp == OCCP_UNEMP_NO_UI) {
                    income_sim = (r0)*a0_sim_tmp;
                    tax_income = 0.0;
                }
                else if (occp0_sim_tmp == OCCP_EMP) {//employ
			...
			else if (i_occp0_sim_tmp == CHOICE_ENTR) {//en
                if (entrepreneur_operated_this_period) {
            ...
            c_gov_payment += intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, i_occp0_sim_tmp, ppl_edu, c_gov_pay);
    ...
    int self_emp_count = 0;
    int employer_count = 0;
    int self_emp_count_edu[Ne] = { 0, 0, 0 };
    int employer_count_edu[Ne] = { 0, 0, 0 };
    double self_emp_share = 0.0;
    double employer_share = 0.0;
    if (use_self_employment_root) {
        // Added specifically for coauthor-facing diagnostics:
        // this makes it possible to see whether the new branch creates a true
        // mass of one-person firms, or whether entrepreneurs simply bunch just
        // above zero hired labor and remain employers in practice.
        for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
            if (i_occp0_sim[ppl_sim] != CHOICE_ENTR) {
                continue;
            }
            if (!entrepreneur_closure_preserve_current_income && !is_entrepreneur_state(occp0_sim[ppl_sim])) {
                continue;
            }
            const int ppl_edu = education_index_from_person(ppl_sim);
            if (n_sim[ppl_sim] <= self_employment_n_threshold) {
                self_emp_count += 1;
                self_emp_count_edu[ppl_edu] += 1;
            }
            else {
                employer_count += 1;
                employer_count_edu[ppl_edu] += 1;
            }
        }
    }
    cout << "" << endl;
    ...
    if (use_self_employment_root) {
        self_emp_share = (entr_count > 0) ? (static_cast<double>(self_emp_count) / static_cast<double>(entr_count)) : 0.0;
        employer_share = (entr_count > 0) ? (static_cast<double>(employer_count) / static_cast<double>(entr_count)) : 0.0;
        cout << "self-employed count/share among entrepreneurs: " << self_emp_count << " / " << self_emp_share
            << "  self-employed by edu: [" << self_emp_count_edu[0] << ", " << self_emp_count_edu[1] << ", " << self_emp_count_edu[2] << "]" << endl;
        cout << "employer count/share among entrepreneurs: " << employer_count << " / " << employer_share
            << "  employers by edu: [" << employer_count_edu[0] << ", " << employer_count_edu[1] << ", " << employer_count_edu[2] << "]" << endl;
    }
    ...
    const double v_market_gap_post = std::max(0.0, n_demand - n_supply);
    const double v_shrink_offset_post = 0.0;
    if (env_vacancy_source_rule >= 0) {
        v_t = build_vacancy_mass_from_source_rule(
            env_vacancy_source_rule,
            v_legacy_post,
    ...
    cout << "[THETA_DIAG] v_t=" << v_t
        ...
        << " v_rule=" << vacancy_source_rule_label(env_vacancy_source_rule)
        << " v_market_gap=" << v_market_gap_post
        << " v_incumbent=" << v_incumbent_post
        << " v_shrink_offset=" << v_shrink_offset_post
        << " c0=" << cacu_0
        << " c1=" << cacu_1
        << " c2=" << cacu_2
    ...
    for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ppl_sim++) {
        ...
        if (ppl_edu == 0) {
            if (is_entrepreneur_state(occp0_sim_tmp)) {
                entr_edu_0_0 += choose_entr_mass;
                entr_edu_1_0 += choose_work_mass;
            }
            else if (occp0_sim_tmp == OCCP_EMP) {
                emp_edu_0_0 += choose_entr_mass;
                emp_edu_1_0 += choose_work_mass;
            }
            else if (occp0_sim_tmp == OCCP_UNEMP_UI || occp0_sim_tmp == OCCP_UNEMP_NO_UI) {
    ...
    const double r_gap = fabs(r0 - r1);
    const double w_gap = fabs(w0 - w1);
    const double theta_gap = fabs(theta_ut - theta_ut_new);
    const double tau_gap = fabs(tau_y0 - tau_y1);
    const double agg_residual_l1 = r_gap + w_gap + theta_gap + tau_gap;
    const bool improved_best_agg = agg_residual_l1 < best_agg_residual_l1;

    if (improved_best_agg) {
        best_check_tol = agg_residual_l1;
        best_agg_residual_l1 = agg_residual_l1;
        best_agg_iter = iter_agg;
    }

    cout << "[AGG_DIAG] iter=" << iter_agg
        << " r_old=" << r0 << " r_new=" << r1 << " r_gap=" << r_gap
        << " w_old=" << w0 << " w_new=" << w1 << " w_gap=" << w_gap
        << " theta_old=" << theta_ut << " theta_new=" << theta_ut_new << " theta_gap=" << theta_gap
        << " tau_old=" << tau_y0 << " tau_new=" << tau_y1 << " tau_gap=" << tau_gap
        << " residual_l1=" << agg_residual_l1
        << " asset_gap=" << asset_gap << " labor_gap=" << labor_gap
        << " asset_demand=" << asset_demand << " asset_supply=" << asset_supply
        << " labor_demand=" << n_demand << " labor_supply=" << n_supply
        << " r1_candidate=" << r1_candidate << " w1_candidate=" << w1_candidate
        << " update_step=" << active_update_step1
        << " wage_update_step=" << active_wage_update_step
        << " theta_update_step=" << active_theta_update_step
        << " best_residual=" << best_agg_residual_l1
        << " best_iter=" << best_agg_iter
        << " entr_share=" << entr_share_sim
        << " avg_n_entr=" << n_entr_avg
        << " avg_k_entr=" << k_entr_avg
        << " self_emp_share=" << self_emp_share
        << " employer_share=" << employer_share
        << " output_per_household=" << (agg_y_sim / Nsim_ppl)
        << endl;

    {
        ofstream agg_diag_file(workingpath_new + "agg_iteration_diagnostics.csv", ios::app);
        agg_diag_file
            << iter_agg << ","
            << r0 << "," << r1 << "," << r_gap << ","
            << w0 << "," << w1 << "," << w_gap << ","
            << theta_ut << "," << theta_ut_new << "," << theta_gap << ","
            << tau_y0 << "," << tau_y1 << "," << tau_gap << ","
            << agg_residual_l1 << ","
            << asset_demand << "," << asset_supply << "," << asset_gap << ","
            << n_demand << "," << n_supply << "," << labor_gap << ","
            << r1_candidate << "," << w1_candidate << "," << active_update_step1 << "," << active_wage_update_step << "," << active_theta_update_step << ","
            << best_agg_residual_l1 << "," << best_agg_iter << ","
            << entr_share_sim << "," << n_entr_avg << "," << k_entr_avg << ","
            << self_emp_share << "," << employer_share << ","
            << c_gov_payment << "," << (agg_y_sim / Nsim_ppl)
            << endl;
    }

    if (improved_best_agg) {
        ofstream best_diag_file(workingpath_new + "best_agg_snapshot.txt", ios::trunc);
        best_diag_file << "iter=" << iter_agg << endl;
        best_diag_file << "residual_l1=" << agg_residual_l1 << endl;
        best_diag_file << "r_old=" << r0 << endl;
        best_diag_file << "r_new=" << r1 << endl;
        best_diag_file << "r_gap=" << r_gap << endl;
        best_diag_file << "w_old=" << w0 << endl;
        best_diag_file << "w_new=" << w1 << endl;
        best_diag_file << "w_gap=" << w_gap << endl;
        best_diag_file << "theta_old=" << theta_ut << endl;
        best_diag_file << "theta_new=" << theta_ut_new << endl;
        best_diag_file << "theta_gap=" << theta_gap << endl;
        best_diag_file << "tau_old=" << tau_y0 << endl;
        best_diag_file << "tau_new=" << tau_y1 << endl;
        best_diag_file << "tau_gap=" << tau_gap << endl;
        best_diag_file << "asset_demand=" << asset_demand << endl;
        best_diag_file << "asset_supply=" << asset_supply << endl;
        best_diag_file << "asset_gap=" << asset_gap << endl;
        best_diag_file << "labor_demand=" << n_demand << endl;
        best_diag_file << "labor_supply=" << n_supply << endl;
        best_diag_file << "labor_gap=" << labor_gap << endl;
        best_diag_file << "r1_candidate=" << r1_candidate << endl;
        best_diag_file << "w1_candidate=" << w1_candidate << endl;
        best_diag_file << "update_step=" << active_update_step1 << endl;
        best_diag_file << "wage_update_step=" << active_wage_update_step << endl;
        best_diag_file << "theta_update_step=" << active_theta_update_step << endl;
        best_diag_file << "entrepreneur_share=" << entr_share_sim << endl;
        best_diag_file << "avg_n_entrepreneur=" << n_entr_avg << endl;
        best_diag_file << "avg_k_entrepreneur=" << k_entr_avg << endl;
        best_diag_file << "self_emp_share=" << self_emp_share << endl;
        best_diag_file << "employer_share=" << employer_share << endl;
        best_diag_file << "govt_floor_pay=" << c_gov_payment << endl;
        best_diag_file << "output_per_household=" << (agg_y_sim / Nsim_ppl) << endl;
    }
...
namespace {
	inline int transition_education_index_from_person(const int ppl_sim_idx)
	{
		if (ppl_sim_idx < edu_share[0] * Nsim_ppl) {
			return 0;
		}
	...
	inline int normalize_transition_occp_state(int occp_raw)
	{
		if (occp_raw <= OCCP_ENTR) {
			return OCCP_ENTR;
		}
		if (occp_raw == OCCP_ENTR_EMP) {
			return OCCP_ENTR_EMP;
		}
		if (occp_raw == OCCP_EMP) {
			return OCCP_EMP;
		}
		if (occp_raw == OCCP_UNEMP_NO_UI) {
			return OCCP_UNEMP_NO_UI;
		}
		return OCCP_UNEMP_UI;
	}

	inline std::string resolve_transition_output_dir()
	{
		std::string out_dir = workingpath_new;
		if (!env_transition_output_tag.empty()) {
			out_dir += env_transition_output_tag;
			ensure_trailing_sep(out_dir);
...
bool save_transition_init_cross_section(const std::string& path)
{
	ofstream file(path);
	if (!file.is_open()) {
		cerr << "[transition] Failed to open transition init export: " << path << endl;
		return false;
	}
	for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
		file << a0_sim[ppl_sim] << " " << x0_sim[ppl_sim] << " " << occp0_sim[ppl_sim] << endl;
	}
	file.close();
	return true;
}
	...
	std::vector<int>& occp_state)
{
	ifstream file(path);
	if (!file.is_open()) {
		cerr << "[transition] Failed to open transition init file: " << path << endl;
		return false;
	}

	a_state.assign(Nsim_ppl, (a_l + a_h) / 4.0);
	x_state.assign(Nsim_ppl, x_shk[0][0]);
	occp_state.assign(Nsim_ppl, OCCP_UNEMP_UI);

	std::string line;
	int loaded = 0;
	while (loaded < Nsim_ppl && std::getline(file, line)) {
		if (line.empty()) {
			continue;
		}
		std::stringstream ss(line);
		double a_val = (a_l + a_h) / 4.0;
		double x_val = x_shk[0][0];
		double occp_val = OCCP_UNEMP_UI;
		ss >> a_val >> x_val >> occp_val;
		if (ss.fail()) {
			continue;
		}
		a_state[loaded] = clamp_value(a_val, a_vec[0], a_vec[Na - 1]);
		x_state[loaded] = x_val;
		occp_state[loaded] = normalize_transition_occp_state(static_cast<int>(std::round(occp_val)));
		loaded += 1;
	}
	file.close();

	if (loaded != Nsim_ppl) {
		cerr << "[transition] Loaded " << loaded << " rows from " << path
			 << " but expected " << Nsim_ppl << endl;
		return false;
	}
	return true;
}
...
void maybe_export_transition_init_cross_section()
{
	if (!env_export_transition_init) {
		return;
	}
	std::string out_dir = resolve_transition_output_dir();
	ensure_trailing_sep(out_dir);
	create_directory_if_missing(out_dir);
	const std::string init_path = out_dir + "transition_init_cross_section.txt";
	if (save_transition_init_cross_section(init_path)) {
		cout << "[transition] Saved steady-state cross section to " << init_path << endl;
	}
}

void export_transition_path_direct()
{
	if (!env_export_transition_path) {
		return;
	}
	...
	std::vector<double> x_state;
	std::vector<int> occp_state;
	bool loaded_init = false;
	if (!env_transition_init_state_file.empty()) {
		loaded_init = load_transition_init_cross_section(
			env_transition_init_state_file,
			a_state,
			x_state,
			occp_state);
	}
	if (!loaded_init) {
		a_state.assign(Nsim_ppl, 0.0);
		x_state.assign(Nsim_ppl, 0.0);
		occp_state.assign(Nsim_ppl, OCCP_UNEMP_UI);
		for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
			a_state[ppl_sim] = clamp_value(a0_sim[ppl_sim], a_vec[0], a_vec[Na - 1]);
			x_state[ppl_sim] = x0_sim[ppl_sim];
			occp_state[ppl_sim] = normalize_transition_occp_state(occp0_sim[ppl_sim]);
		}
		cout << "[transition] Using current steady-state cross section as MIT initial condition." << endl;
	}
	else {
		for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
			if (occp_state[ppl_sim] != OCCP_ENTR) {
				continue;
			}
			int ppl_edu = transition_education_index_from_person(ppl_sim);
			int occp_choice_entrepreneur = CHOICE_ENTR;
			const double n_loaded_raw = intp_opt_sim(a_state[ppl_sim], x_state[ppl_sim], occp_state[ppl_sim], occp_choice_entrepreneur, ppl_edu, opt_n_0);
			occp_state[ppl_sim] = entrepreneur_state_from_n(std::max(0.0, n_loaded_raw));
		}
	}
	...
	ofstream csv(out_dir + "transition_path.csv");
	if (!csv.is_open()) {
		cerr << "[transition] Failed to open transition CSV in " << out_dir << endl;
		return;
	}
	ofstream flow_csv(out_dir + "transition_flow_matrix.csv");
	if (!flow_csv.is_open()) {
		cerr << "[transition] Failed to open transition flow CSV in " << out_dir << endl;
		csv.close();
		return;
	}
	csv << "period,theta_used,rho_used,entrepreneur_share,entrepreneur_state_share,employed_state_share,unemployed_state_share,"
		<< "self_employed_share_among_entrepreneurs,employer_share_among_entrepreneurs,output_total,output_per_capita,output_per_entrepreneur,"
		<< "labor_demand,labor_supply,asset_demand,asset_supply,avg_entrepreneur_n,avg_entrepreneur_k,"
		<< "self_employed_business_share,employer_1_4_business_share,employer_5_19_business_share,employer_20_plus_business_share,"
		<< "entrepreneur_share_low,entrepreneur_share_mid,entrepreneur_share_high,"
		<< "unemployment_share_low,unemployment_share_mid,unemployment_share_high,"
		<< "nonbiz_to_selfemp_share,nonbiz_to_employer_share,selfemp_to_nonbiz_share,selfemp_to_employer_share,"
		<< "employer_to_nonbiz_share,employer_to_selfemp_share,employer_to_employer_share,"
		<< "employer_entry_share,employer_exit_share,selfemp_entry_share,selfemp_exit_share,"
		<< "emp20_plus_to_smaller_share,smaller_to_emp20_plus_share" << endl;
	flow_csv << "period,prev_business_type,curr_business_type,count,share" << endl;

	std::vector<double> a_next(Nsim_ppl, 0.0);
	std::vector<int> occp_next(Nsim_ppl, OCCP_UNEMP_UI);
	std::vector<int> business_type_state(Nsim_ppl, BUSINESS_NONE);
	std::vector<int> business_type_next(Nsim_ppl, BUSINESS_NONE);
	bool have_prev_business_types = false;
	if (!loaded_init) {
		have_prev_business_types = true;
		for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
			const bool operated_in_steady_state = is_entrepreneur_state(normalize_transition_occp_state(occp0_sim[ppl_sim]));
			const double n_prev = operated_in_steady_state ? std::max(0.0, n_sim[ppl_sim]) : 0.0;
			business_type_state[ppl_sim] = classify_business_type(operated_in_steady_state, n_prev);
		}
	}

	cout << "[transition] Exporting direct path from "
		 << (loaded_init ? env_transition_init_state_file : std::string("current steady state"))
		 << " to " << (out_dir + "transition_path.csv") << endl;

	for (int period = 0; period < Nsim_pd; ++period) {
		if (period > 0) {
			for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
				int ppl_edu = transition_education_index_from_person(ppl_sim);
				double rnd_x = rnd_x_shk[period][ppl_sim];
				int iid_state = 0;
				x_state[ppl_sim] = x0_rnd(rnd_x, iid_state, ppl_edu);
			}
		}

		double cacu_1_rho = 0.0;
		double cacu_1 = 0.0;
		double cacu_2 = 0.0;
		double expected_n_demand_pre = 0.0;
		double v_incumbent_pre_local = 0.0;
		for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
			int ppl_edu = transition_education_index_from_person(ppl_sim);
			double a0_sim_tmp = a_state[ppl_sim];
			double x0_sim_tmp = x_state[ppl_sim];
			int occp0_sim_tmp = occp_state[ppl_sim];

			const double prob_work = clamp_probability(intp_2d_opt(a0_sim_tmp, x0_sim_tmp, occp0_sim_tmp, ppl_edu, opt_occp_0));
			const double prob_entrepreneur = 1.0 - prob_work;
			int occp_choice_entrepreneur = CHOICE_ENTR;
			const double n_policy_raw = intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp0_sim_tmp, occp_choice_entrepreneur, ppl_edu, opt_n_0);
			const double n_policy = std::max(0.0, n_policy_raw);
			const double n_prev_for_flow = is_entrepreneur_state(occp0_sim_tmp) ? std::max(0.0, n_sim[ppl_sim]) : 0.0;
			const double incumbent_flow = incumbent_vacancy_flow_from_current_and_carried_n(
				n_policy,
				n_prev_for_flow);
			expected_n_demand_pre += prob_entrepreneur * n_policy;
			v_incumbent_pre_local += prob_entrepreneur * incumbent_flow;

			if (occp0_sim_tmp != OCCP_EMP) {
				cacu_2 += prob_work;
			}
			else {
				const double weighted_rho = prob_work * edu_rho[ppl_edu];
		...
		const double v_legacy_pre = cacu_1_rho;
		const double structural_source_pre_effective = std::max(std::max(0.0, expected_n_demand_pre), kVacancyStructuralMin);
		const double v_market_gap_pre_local = 0.0;
		double v_t_pre = 0.0;
		if (env_vacancy_source_rule >= 0) {
			v_t_pre = build_vacancy_mass_from_source_rule(
				env_vacancy_source_rule,
				v_legacy_pre,
				v_market_gap_pre_local,
				v_incumbent_pre_local,
				0.0);
		}
		else {
			v_t_pre = build_vacancy_mass(v_legacy_pre, structural_source_pre_effective);
		}
		const double u_t_pre = cacu_2;

		const bool anchor_export_period = (period < env_transition_export_anchor_periods);
		double theta_used = theta_ut;
		if (!anchor_export_period && u_t_pre > 1e-10 && std::isfinite(u_t_pre)) {
			const double vacancy_to_unemp = std::max(v_t_pre, 0.0) / u_t_pre;
			theta_used = bar_m * pow(vacancy_to_unemp, 1 - mu);
		}
		if (env_single_case == 51) {
			theta_used = std::max(theta_used, kMinThetaModel51);
		}
		if (env_override_fixed_theta) {
			theta_used = env_fixed_theta;
		}
		theta_used = clamp_probability(theta_used);

		double rho_used = rho_aver;
		if (!anchor_export_period && cacu_1 > 1e-10) {
			rho_used = cacu_1_rho / cacu_1;
		}
		if (env_override_fixed_rho) {
			rho_used = env_fixed_rho;
		}
		rho_used = clamp_probability(rho_used);

		double agg_y_sim = 0.0;
		double n_demand = 0.0;
		double n_supply = 0.0;
		double asset_demand = 0.0;
		double asset_supply = 0.0;
		int entr_count = 0;
		double n_entr_sum = 0.0;
		double k_entr_sum = 0.0;
		int self_emp_count = 0;
		int employer_count = 0;
		int employer_1_4_count = 0;
		int employer_5_19_count = 0;
		int employer_20_plus_count = 0;
		int state_entr_count = 0;
		int state_emp_count = 0;
		int state_unemp_count = 0;
		int entr_count_edu[Ne] = { 0, 0, 0 };
		int state_unemp_edu[Ne] = { 0, 0, 0 };
		int flow_counts[NbusinessTypes][NbusinessTypes] = { 0 };
		...
		for (int ppl_sim = 0; ppl_sim < Nsim_ppl; ++ppl_sim) {
			...
			double a0_sim_tmp = a_state[ppl_sim];
			double x0_sim_tmp = x_state[ppl_sim];
			int occp_prev_state = occp_state[ppl_sim];

			const double prob_work = clamp_probability(intp_2d_opt(a0_sim_tmp, x0_sim_tmp, occp_prev_state, ppl_edu, opt_occp_0));
			const double rnd_choice = ((double)rand()) / RAND_MAX;
			int occp_choice = (rnd_choice <= prob_work) ? CHOICE_WORK : CHOICE_ENTR;

			double n0_sim_tmp = 0.0;
			double k0_sim_tmp = 0.0;
			double y_sim_tmp = 0.0;
			double a1_sim_tmp = clamp_value(
				intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, occp_choice, ppl_edu, opt_a_0),
				a_vec[0],
				a_vec[Na - 1]);
			bool entrepreneur_operated_this_period = (occp_choice == CHOICE_ENTR);

			if (occp_choice == CHOICE_WORK) {
				if (occp_prev_state != OCCP_EMP) {
					z_agg += (1 + edu_efficiency_z[ppl_edu]) * theta_used;
				}
				else {
					z_agg += (1 + edu_efficiency_z[ppl_edu]) * (1 - edu_rho[ppl_edu]);
				}
				...
				n0_sim_tmp = std::max(0.0, intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, occp_choice, ppl_edu, opt_n_0));
				k0_sim_tmp = std::max(0.0, intp_opt_sim(a0_sim_tmp, x0_sim_tmp, occp_prev_state, occp_choice, ppl_edu, opt_k_0));
				y_sim_tmp = use_self_employment_root
					? entrepreneur_output_static(x0_sim_tmp, k0_sim_tmp, n0_sim_tmp)
					: x0_sim_tmp * pow(k0_sim_tmp, alpha) * pow(n0_sim_tmp, gamma_param);
			}

			int occp_realized_next = OCCP_UNEMP_UI;
			if (occp_choice == CHOICE_ENTR) {
				if (use_entrepreneur_closure_risk) {
					const double random_num = ((double)rand()) / RAND_MAX;
					const double prob_closure = clamp_probability(edu_rho_entr[ppl_edu]);
					if (random_num <= prob_closure) {
						occp_realized_next = OCCP_UNEMP_NO_UI;
						a1_sim_tmp = std::max(a_l, a1_sim_tmp - entrepreneur_capital_loss_share * std::max(0.0, k0_sim_tmp));
						if (!entrepreneur_closure_preserve_current_income) {
							entrepreneur_operated_this_period = false;
							n0_sim_tmp = 0.0;
						...
						occp_realized_next = entrepreneur_state_from_n(n0_sim_tmp);
					}
				}
				else {
					occp_realized_next = entrepreneur_state_from_n(n0_sim_tmp);
			...
			else if (occp_prev_state != OCCP_EMP) {
				const double random_num = ((double)rand()) / RAND_MAX;
				if (random_num <= theta_used) {
					occp_realized_next = OCCP_EMP;
				}
				else if (one_shot_ui && occp_prev_state == OCCP_UNEMP_UI) {
					occp_realized_next = OCCP_UNEMP_NO_UI;
				}
				else {
					occp_realized_next = has_regular_ui(occp_prev_state) ? OCCP_UNEMP_UI : OCCP_UNEMP_NO_UI;
				}
			}
			else {
				const double random_num = ((double)rand()) / RAND_MAX;
				if (random_num <= clamp_probability(edu_rho[ppl_edu])) {
					occp_realized_next = OCCP_UNEMP_UI;
				}
				else {
					occp_realized_next = OCCP_EMP;
				}
			...
			a_next[ppl_sim] = a1_sim_tmp;
			occp_next[ppl_sim] = occp_realized_next;
			const int current_business_type = classify_business_type(entrepreneur_operated_this_period, n0_sim_tmp);
			business_type_next[ppl_sim] = current_business_type;
			if (have_prev_business_types) {
				const int prev_business_type = std::max(0, std::min(NbusinessTypes - 1, business_type_state[ppl_sim]));
				flow_counts[prev_business_type][current_business_type] += 1;
			}

			if (is_entrepreneur_state(occp_realized_next)) {
				state_entr_count += 1;
			}
			else if (occp_realized_next == OCCP_EMP) {
				state_emp_count += 1;
				n_supply += (1 + edu_efficiency_z[ppl_edu]);
			}
			else {
				state_unemp_count += 1;
				state_unemp_edu[ppl_edu] += 1;
			}
			...
			if (entrepreneur_operated_this_period) {
				...
				n_entr_sum += n0_sim_tmp;
				k_entr_sum += k0_sim_tmp;
				if (n0_sim_tmp <= self_employment_n_threshold) {
					self_emp_count += 1;
				}
				else {
					employer_count += 1;
					if (n0_sim_tmp <= 4.0) {
						employer_1_4_count += 1;
					}
					else if (n0_sim_tmp <= 19.0) {
						employer_5_19_count += 1;
					}
					else {
						employer_20_plus_count += 1;
					}
				}
			}
		...
		const double entrepreneur_share = static_cast<double>(entr_count) / static_cast<double>(Nsim_ppl);
		const double entrepreneur_state_share = static_cast<double>(state_entr_count) / static_cast<double>(Nsim_ppl);
		const double employed_state_share = static_cast<double>(state_emp_count) / static_cast<double>(Nsim_ppl);
		const double unemployed_state_share = static_cast<double>(state_unemp_count) / static_cast<double>(Nsim_ppl);
		const double self_emp_share = (entr_count > 0) ? (static_cast<double>(self_emp_count) / static_cast<double>(entr_count)) : 0.0;
		const double employer_share = (entr_count > 0) ? (static_cast<double>(employer_count) / static_cast<double>(entr_count)) : 0.0;
		const double output_per_capita = agg_y_sim / static_cast<double>(Nsim_ppl);
		const double output_per_entrepreneur = (entr_count > 0) ? (agg_y_sim / static_cast<double>(entr_count)) : 0.0;
		const double avg_entrepreneur_n = (entr_count > 0) ? (n_entr_sum / static_cast<double>(entr_count)) : 0.0;
		const double avg_entrepreneur_k = (entr_count > 0) ? (k_entr_sum / static_cast<double>(entr_count)) : 0.0;
		const double self_employed_business_share = static_cast<double>(self_emp_count) / static_cast<double>(Nsim_ppl);
		const double employer_1_4_business_share = static_cast<double>(employer_1_4_count) / static_cast<double>(Nsim_ppl);
		const double employer_5_19_business_share = static_cast<double>(employer_5_19_count) / static_cast<double>(Nsim_ppl);
		const double employer_20_plus_business_share = static_cast<double>(employer_20_plus_count) / static_cast<double>(Nsim_ppl);
		const double entrepreneur_share_low = static_cast<double>(entr_count_edu[0]) / std::max(1.0, edu_share[0] * Nsim_ppl);
		const double entrepreneur_share_mid = static_cast<double>(entr_count_edu[1]) / std::max(1.0, edu_share[1] * Nsim_ppl);
		const double entrepreneur_share_high = static_cast<double>(entr_count_edu[2]) / std::max(1.0, edu_share[2] * Nsim_ppl);
		const double unemployment_share_low = static_cast<double>(state_unemp_edu[0]) / std::max(1.0, edu_share[0] * Nsim_ppl);
		const double unemployment_share_mid = static_cast<double>(state_unemp_edu[1]) / std::max(1.0, edu_share[1] * Nsim_ppl);
		const double unemployment_share_high = static_cast<double>(state_unemp_edu[2]) / std::max(1.0, edu_share[2] * Nsim_ppl);
		const double flow_scale = 1.0 / static_cast<double>(Nsim_ppl);
		const double nan_val = std::numeric_limits<double>::quiet_NaN();

		double nonbiz_to_selfemp_share = nan_val;
		double nonbiz_to_employer_share = nan_val;
		double selfemp_to_nonbiz_share = nan_val;
		double selfemp_to_employer_share = nan_val;
		double employer_to_nonbiz_share = nan_val;
		double employer_to_selfemp_share = nan_val;
		double employer_to_employer_share = nan_val;
		double employer_entry_share = nan_val;
		double employer_exit_share = nan_val;
		double selfemp_entry_share = nan_val;
		double selfemp_exit_share = nan_val;
		double emp20_plus_to_smaller_share = nan_val;
		double smaller_to_emp20_plus_share = nan_val;

		if (have_prev_business_types) {
			const int nonbiz_to_selfemp_count = flow_counts[BUSINESS_NONE][BUSINESS_SELF_EMP];
			const int nonbiz_to_employer_count =
				flow_counts[BUSINESS_NONE][BUSINESS_EMP_1_4]
				+ flow_counts[BUSINESS_NONE][BUSINESS_EMP_5_19]
				+ flow_counts[BUSINESS_NONE][BUSINESS_EMP_20_PLUS];
			const int selfemp_to_nonbiz_count = flow_counts[BUSINESS_SELF_EMP][BUSINESS_NONE];
			const int selfemp_to_employer_count =
				flow_counts[BUSINESS_SELF_EMP][BUSINESS_EMP_1_4]
				+ flow_counts[BUSINESS_SELF_EMP][BUSINESS_EMP_5_19]
				+ flow_counts[BUSINESS_SELF_EMP][BUSINESS_EMP_20_PLUS];
			const int employer_to_nonbiz_count =
				flow_counts[BUSINESS_EMP_1_4][BUSINESS_NONE]
				+ flow_counts[BUSINESS_EMP_5_19][BUSINESS_NONE]
				+ flow_counts[BUSINESS_EMP_20_PLUS][BUSINESS_NONE];
			const int employer_to_selfemp_count =
				flow_counts[BUSINESS_EMP_1_4][BUSINESS_SELF_EMP]
				+ flow_counts[BUSINESS_EMP_5_19][BUSINESS_SELF_EMP]
				+ flow_counts[BUSINESS_EMP_20_PLUS][BUSINESS_SELF_EMP];
			const int employer_to_employer_count =
				flow_counts[BUSINESS_EMP_1_4][BUSINESS_EMP_1_4]
				+ flow_counts[BUSINESS_EMP_1_4][BUSINESS_EMP_5_19]
				+ flow_counts[BUSINESS_EMP_1_4][BUSINESS_EMP_20_PLUS]
				+ flow_counts[BUSINESS_EMP_5_19][BUSINESS_EMP_1_4]
				+ flow_counts[BUSINESS_EMP_5_19][BUSINESS_EMP_5_19]
				+ flow_counts[BUSINESS_EMP_5_19][BUSINESS_EMP_20_PLUS]
				+ flow_counts[BUSINESS_EMP_20_PLUS][BUSINESS_EMP_1_4]
				+ flow_counts[BUSINESS_EMP_20_PLUS][BUSINESS_EMP_5_19]
				+ flow_counts[BUSINESS_EMP_20_PLUS][BUSINESS_EMP_20_PLUS];
			const int emp20_plus_to_smaller_count =
				flow_counts[BUSINESS_EMP_20_PLUS][BUSINESS_EMP_1_4]
				+ flow_counts[BUSINESS_EMP_20_PLUS][BUSINESS_EMP_5_19];
			const int smaller_to_emp20_plus_count =
				flow_counts[BUSINESS_EMP_1_4][BUSINESS_EMP_20_PLUS]
				+ flow_counts[BUSINESS_EMP_5_19][BUSINESS_EMP_20_PLUS];

			nonbiz_to_selfemp_share = nonbiz_to_selfemp_count * flow_scale;
			nonbiz_to_employer_share = nonbiz_to_employer_count * flow_scale;
			selfemp_to_nonbiz_share = selfemp_to_nonbiz_count * flow_scale;
			selfemp_to_employer_share = selfemp_to_employer_count * flow_scale;
			employer_to_nonbiz_share = employer_to_nonbiz_count * flow_scale;
			employer_to_selfemp_share = employer_to_selfemp_count * flow_scale;
			employer_to_employer_share = employer_to_employer_count * flow_scale;
			employer_entry_share = nonbiz_to_employer_share + selfemp_to_employer_share;
			employer_exit_share = employer_to_nonbiz_share + employer_to_selfemp_share;
			selfemp_entry_share = nonbiz_to_selfemp_share + employer_to_selfemp_share;
			selfemp_exit_share = selfemp_to_nonbiz_share + selfemp_to_employer_share;
			emp20_plus_to_smaller_share = emp20_plus_to_smaller_count * flow_scale;
			smaller_to_emp20_plus_share = smaller_to_emp20_plus_count * flow_scale;

			for (int prev_type = 0; prev_type < NbusinessTypes; ++prev_type) {
				for (int curr_type = 0; curr_type < NbusinessTypes; ++curr_type) {
					const int flow_count = flow_counts[prev_type][curr_type];
					const double flow_share = flow_count * flow_scale;
					flow_csv << period << ","
						<< business_type_label(prev_type) << ","
						<< business_type_label(curr_type) << ","
						<< flow_count << ","
						<< flow_share << endl;
				}
			}
		...
		csv << period << ","
			<< theta_used << "," << rho_used << ","
			<< entrepreneur_share << "," << entrepreneur_state_share << "," << employed_state_share << "," << unemployed_state_share << ","
			<< self_emp_share << "," << employer_share << ","
			<< agg_y_sim << "," << output_per_capita << "," << output_per_entrepreneur << ","
			<< n_demand << "," << n_supply << "," << asset_demand << "," << asset_supply << ","
			<< avg_entrepreneur_n << "," << avg_entrepreneur_k << ","
			<< self_employed_business_share << "," << employer_1_4_business_share << "," << employer_5_19_business_share << "," << employer_20_plus_business_share << ","
			<< entrepreneur_share_low << "," << entrepreneur_share_mid << "," << entrepreneur_share_high << ","
			<< unemployment_share_low << "," << unemployment_share_mid << "," << unemployment_share_high << ","
			<< nonbiz_to_selfemp_share << "," << nonbiz_to_employer_share << "," << selfemp_to_nonbiz_share << "," << selfemp_to_employer_share << ","
			<< employer_to_nonbiz_share << "," << employer_to_selfemp_share << "," << employer_to_employer_share << ","
			<< employer_entry_share << "," << employer_exit_share << "," << selfemp_entry_share << "," << selfemp_exit_share << ","
			<< emp20_plus_to_smaller_share << "," << smaller_to_emp20_plus_share
			<< endl;

		a_state.swap(a_next);
		occp_state.swap(occp_next);
		business_type_state.swap(business_type_next);
		have_prev_business_types = true;
	}
...
void assign_value()
{
	...
	double base_edu_rho_entr[Ne] = { 0.168, 0.150, 0.144 };
	...
	x_eps = 4.422;
	tau_s = 0.12;
	self_employment_entry_fixed_cost = 0.0;
	self_employment_employer_entry_fixed_cost = 0.0;
	...
	// Default: no self-employment root extension
	// Benchmark case 101 should behave like the separate benchmark file unless
	// one of the self-employment experiment cases below turns this on.
	use_self_employment_root = false;
	self_employment_owner_labor = 1.0;
	self_employment_hired_scale = 1.0;
	self_employment_n_threshold = 1e-4;
	self_employment_x_scale = 1.0;
	self_employment_hiring_fixed_cost = 0.0;
	chi_self_emp = 0.0;
	chi_employer = 0.0;
	use_entrepreneur_closure_risk = false;
	entrepreneur_closure_preserve_current_income = false;
	entrepreneur_capital_loss_share = 0.0;
	for (int i_rho = 0; i_rho < Ne; i_rho++) {
		edu_rho_entr[i_rho] = 0.0;
	}
	//========================================================================
	// BASELINE CASE
	//========================================================================
	...
	if (i_case == 101) {
		...
		lowerincome_b = 0.4;
		ui_benefit_mode = 0;
		ui_benefit_replacement_rate = 0.50;
		ui_benefit_cap_mult = 2.0 / 3.0;
		ui_benefit_duration_share = 0.50;
		underline_c = 0.3;
	...
	// Same baseline parameters as case 101, but entrepreneurs can operate with zero hired workers.
	// This is the clean comparison case against the benchmark model.
	if (i_case == 141) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		...
		kappa = 0.035;
		lowerincome_b = 0.4;
		underline_c = 0.081187; // Benchmark floor mapped from the $4,000 annual floor in Kitao (2014).
	...
	// Same as case 141, but the no-hire self-employed corner uses only half of x.
	// This was added to test whether the raw self-employment baseline was too
	// attractive because the zero-hire corner inherited the full `x` draw.
	if (i_case == 142) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
	...
	// Case 143: self-employment root with stronger own-account attenuation
	// Same as case 141, but the no-hire self-employed corner uses one quarter of x.
	if (i_case == 143) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
	...
	// Case 144: self-employment root with attenuated own-account productivity
	// and a fixed cost of hiring, to stop epsilon-labor avoidance of the no-hire corner.
	// In practice this case helped reveal that static penalties alone can push
	// the model into "all employers, no self-employed" outcomes.
	if (i_case == 144) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		...
		self_employment_n_threshold = 1e-4;
		self_employment_x_scale = 0.50;
		self_employment_hiring_fixed_cost = 1.0;
		chi_self_emp = 0.0;
		chi_employer = 0.0;
	...
	// Case 145: self-employment root with entrepreneur closure risk and
	// post-closure non-UI unemployment. The closure hazard comes from the
	// transparent initial calibration documented in notes/08_entrepreneur_closure_risk.md.
	if (i_case == 145) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		...
		chi_self_emp = 0.0;
		chi_employer = 0.0;
		use_entrepreneur_closure_risk = true;
		entrepreneur_closure_preserve_current_income = false;
		entrepreneur_capital_loss_share = 0.0;
		...
		for (int i_rho = 0; i_rho < Ne; i_rho++) {
			edu_rho[i_rho] = base_edu_rho[i_rho];
			edu_rho_entr[i_rho] = base_edu_rho_entr[i_rho];
		}
	}

	// Case 146: self-employment root with end-of-period closure risk and
	// partial capital loss. This is the first-pass benchmark for failure:
	// the entrepreneur receives current-period operating income, but if the
	// business closes, next-period assets lose a fraction of installed capital
	// and the household moves to the non-UI unemployment state.
	if (i_case == 146) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		...
		chi_employer = 0.0;
		use_entrepreneur_closure_risk = true;
		entrepreneur_closure_preserve_current_income = true;
	...
	// Case 147: case 146 plus a small fixed employer-threshold cost.
	// This keeps the smooth search-and-matching hiring cost kappa active, but
	// adds a discrete hurdle to crossing from self-employment (n_h = 0) to
	// employer status (n_h > 0). The value is an initial experimental choice
	// in model units, to be disciplined against the literature and baseline
	// employer/nonemployer shares.
	if (i_case == 147) {
		beta = 0.94;
		double x_share_dims[3] = { 0.782, 0.802, 0.822 };
		...
		mu = 0.68;
		kappa = 0.035;
		lowerincome_b = 0.4; // Legacy reduced-form object retained only for flat-UI robustness.
		underline_c = 0.081187; // Benchmark floor mapped from the $4,000 annual floor in Kitao (2014).
		ui_benefit_mode = 1;
		ui_benefit_replacement_rate = 0.50;
		ui_benefit_cap_mult = 1.0;     // Slack benchmark cap so the policy ladder is easy to interpret.
		ui_benefit_duration_share = 0.50;
		one_shot_ui = true;
	...
	if (env_override_underline_c) {
		underline_c = env_underline_c;
		cout << "[override] underline_c (active) = " << underline_c << endl;
	}

	if (env_override_ui_benefit_mode) {
		ui_benefit_mode = env_ui_benefit_mode;
		cout << "[override] ui_benefit_mode (active) = " << ui_benefit_mode << endl;
	}

	if (env_override_ui_benefit_replacement_rate) {
		ui_benefit_replacement_rate = env_ui_benefit_replacement_rate;
		cout << "[override] ui_benefit_replacement_rate (active) = "
			 << ui_benefit_replacement_rate << endl;
	}

	if (env_override_ui_benefit_cap_mult) {
		ui_benefit_cap_mult = env_ui_benefit_cap_mult;
		cout << "[override] ui_benefit_cap_mult (active) = "
			 << ui_benefit_cap_mult << endl;
	}

	if (env_override_ui_benefit_duration_share) {
		ui_benefit_duration_share = env_ui_benefit_duration_share;
		cout << "[override] ui_benefit_duration_share (active) = "
			 << ui_benefit_duration_share << endl;
	}

	if (env_override_one_shot_ui) {
		one_shot_ui = env_one_shot_ui;
		cout << "[override] one_shot_ui (active) = " << (one_shot_ui ? 1 : 0) << endl;
	}
	...
	if (override_self_employment_entry_fixed_cost) {
		self_employment_entry_fixed_cost = self_employment_entry_fixed_cost_override;
		cout << "[override] self_employment_entry_fixed_cost = "
			 << self_employment_entry_fixed_cost << endl;
	}

	if (override_self_employment_employer_entry_fixed_cost) {
		self_employment_employer_entry_fixed_cost = self_employment_employer_entry_fixed_cost_override;
		cout << "[override] self_employment_employer_entry_fixed_cost = "
			 << self_employment_employer_entry_fixed_cost << endl;
	}

	if (override_self_employment_x_scale) {
		self_employment_x_scale = self_employment_x_scale_override;
		cout << "[override] self_employment_x_scale = "
			 << self_employment_x_scale << endl;
	}
	...
	if (env_override_output_wedge) {
		x_scale = env_output_wedge;
		cout << "[override] applied output wedge via x_scale = " << x_scale << endl;
	}
	...
	cout << " ui_benefit_mode " << ui_benefit_mode << endl;
	if (ui_benefit_mode == 1) {
		cout << " ui_benefit_replacement_rate " << ui_benefit_replacement_rate << endl;
		cout << " ui_benefit_cap_mult " << ui_benefit_cap_mult << endl;
		cout << " ui_benefit_duration_share " << ui_benefit_duration_share << endl;
		cout << " one_shot_ui " << (one_shot_ui ? 1 : 0) << endl;
	}
...
void initialize_value()
{
	...
	if (env_override_init_r) {
		r0 = env_init_r;
	}
	if (env_override_init_w) {
		w0 = env_init_w;
	}
	if (!use_fixed_tax_rate && env_override_init_tau) {
		tau_y0 = env_init_tau;
	}
	if (env_override_init_theta) {
		theta_ut = clamp_probability(env_init_theta);
	}
...
void solve_model()
{
	...
	if (!env_welfare_load_file.empty() && !welfare_benchmark_loaded) {
		welfare_benchmark_loaded = load_welfare_benchmark(env_welfare_load_file);
		if (!welfare_benchmark_loaded) {
			cout << "[welfare] failed to load benchmark from " << env_welfare_load_file << endl;
		}
	}

	const int max_iter_agg = (env_max_iter_agg > 0) ? env_max_iter_agg : 1000;
	const double aggregate_update_step = (env_update_step1 >= 0.0) ? env_update_step1 : update_step1;
	const double wage_update_step = (env_wage_update_step >= 0.0) ? env_wage_update_step : aggregate_update_step;
	const double theta_update_step = (env_theta_update_step >= 0.0) ? env_theta_update_step : aggregate_update_step;
	active_update_step1 = aggregate_update_step;
	active_wage_update_step = wage_update_step;
	active_theta_update_step = theta_update_step;
	iter_agg = 0;
	while_stop_value = 0;
	best_check_tol = 10.0;
	best_agg_residual_l1 = std::numeric_limits<double>::infinity();
	best_agg_iter = 0;

	{
		ofstream agg_diag_file(workingpath_new + "agg_iteration_diagnostics.csv", ios::trunc);
		agg_diag_file
			<< "iter,r_old,r_new,r_gap,w_old,w_new,w_gap,theta_old,theta_new,theta_gap,"
			<< "tau_old,tau_new,tau_gap,residual_l1,asset_demand,asset_supply,asset_gap,"
			<< "labor_demand,labor_supply,labor_gap,r1_candidate,w1_candidate,update_step,wage_update_step,theta_update_step,"
			<< "best_residual_so_far,best_iter,entr_share,n_entrepreneur_avg,k_entrepreneur_avg,"
			<< "self_emp_share,employer_share,govt_floor_pay,output_per_household" << endl;
	}

	{
		ofstream best_diag_file(workingpath_new + "best_agg_snapshot.txt", ios::trunc);
		best_diag_file << "pending" << endl;
	}

	do {

		r0 = (1.0 - aggregate_update_step) * r1 + aggregate_update_step * r0;
		w0 = (1.0 - wage_update_step) * w1 + wage_update_step * w0;
		theta_ut = (1.0 - theta_update_step) * theta_ut_new + theta_update_step * theta_ut;

		if (env_override_fixed_r) {
			r0 = env_fixed_r;
		}
		if (env_override_fixed_w) {
			w0 = env_fixed_w;
		}
		if (env_override_fixed_theta) {
			theta_ut = clamp_probability(env_fixed_theta);
		}
		...
		if (use_fixed_tax_rate) {
			...
			tau_y0 = (1.0 - aggregate_update_step) * tau_y1 + aggregate_update_step * tau_y0;
		}
	...
	if (!env_welfare_save_file.empty()) {
		if (save_welfare_benchmark(env_welfare_save_file)) {
			cout << "[welfare] saved benchmark to " << env_welfare_save_file << endl;
		}
		else {
			cout << "[welfare] failed to save benchmark to " << env_welfare_save_file << endl;
		}
	}
...
void calibra()
{
	...
	// Apply any output wedge after loading the baseline productivity grid.
	for (int i_x = 0; i_x < Nx; i_x++) {
		for (int i_e = 0; i_e < Ne; i_e++) {
	...
	solve_model();
	maybe_export_transition_init_cross_section();
	export_transition_path_direct();
...
void calculate_cev()
{
	...
	if (ind_policy == 0 && !welfare_benchmark_loaded) {
		for (int i_e = 0; i_e < Ne; i_e++) {
			for (int i_a = 0; i_a < Na; i_a++) {
				for (int i_x = 0; i_x < Nx; i_x++) {
					for (int i_z = 0; i_z < Nz; i_z++) {
...
bool save_welfare_benchmark(const std::string& path)
{
	...
	const int dims[4] = { Na, Nx, Ni, Ne };
	out.write(reinterpret_cast<const char*>(dims), sizeof(dims));
	out.write(reinterpret_cast<const char*>(v_ben), sizeof(v_ben));
	return out.good();
}
...
bool load_welfare_benchmark(const std::string& path)
{
	...
	if (dims[0] != Na || dims[1] != Nx || dims[2] != Ni || dims[3] != Ne) {
		cout << "[welfare] benchmark dimensions mismatch in " << path << endl;
		return false;
	}

	in.read(reinterpret_cast<char*>(v_ben), sizeof(v_ben));
	if (!in.good()) {
		return false;
	}
	return true;
...
{
	// Coauthor note:
	// this is where the benchmark analytical firm block is replaced by the new
	// self-employment static problem. If this switch were absent, the file would
	// still simulate the old employer-only entrepreneur even though the paper
	// draft and diagnostics were talking about self-employment.
	...
	for (int i_e = 0; i_e < Ne; i_e++) {
		for (int i_x = 0; i_x < Nx; i_x++)
		{
			...
			if (use_self_employment_root) {
				solve_self_employment_static_problem(1.0e12, x_tmp, w0_with_edu, k_star_tmp, n_star_tmp, rev_star_tmp);
			}
			else {
			...
			for (int i_a = 0; i_a < Na; i_a++) {
				...
				if (use_self_employment_root) {
					solve_self_employment_static_problem(a_tmp, x_tmp, w0_with_edu, k_opt_tmp, n_opt_tmp, rev_opt_tmp);
				}
				else {
...
void policy_input()
{
	...
	const int legacy_policy_states = 3;
	...
	for (int i_e = 0; i_e < Ne; i_e++) {
		for (int i_a = 0; i_a < Na_in; i_a++) {
			for (int i_x = 0; i_x < Nx_in; i_x++) {
				for (int i_occp = 0; i_occp < legacy_policy_states; i_occp++) {
					for (int i_occp_en = 0; i_occp_en < Nchoice; i_occp_en++) {
				...
				// The legacy initialization file only has three current states.
				// Seed the new non-UI unemployment state from the old UI-unemployment state.
				v_in[i_a][i_x][OCCP_UNEMP_NO_UI][i_e] = v_in[i_a][i_x][OCCP_UNEMP_UI][i_e];
				opt_occp_in[i_a][i_x][OCCP_UNEMP_NO_UI][i_e] = opt_occp_in[i_a][i_x][OCCP_UNEMP_UI][i_e];
				for (int i_occp_en = 0; i_occp_en < Nchoice; i_occp_en++) {
					opt_a_in[OCCP_UNEMP_NO_UI][i_occp_en][i_a][i_x][i_e] = opt_a_in[OCCP_UNEMP_UI][i_occp_en][i_a][i_x][i_e];
					opt_k_in[OCCP_UNEMP_NO_UI][i_occp_en][i_a][i_x][i_e] = opt_k_in[OCCP_UNEMP_UI][i_occp_en][i_a][i_x][i_e];
					opt_n_in[OCCP_UNEMP_NO_UI][i_occp_en][i_a][i_x][i_e] = opt_n_in[OCCP_UNEMP_UI][i_occp_en][i_a][i_x][i_e];
				}
			}
...
double intp_2d_in_6arg(double& x_a0, double& x_x0, int& i_x_occp, int& x_occp0, int& edu_occp, double policy[Ni][Nchoice][Na_in][Nx_in][Ne])
{
