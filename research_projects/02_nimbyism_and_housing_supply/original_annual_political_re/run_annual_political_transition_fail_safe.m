...
p.addParameter('Candidates', [0, 0.10, 1.00; 0, 0.06, 1.00; 0.50, 0.10, 0.50]);
...
for is = 1:numel(cfg.TSchedule)
    ...
    [stage_summary, stage_paths] = run_stage(cfg, base, age_share, T, status_path, out_dir);
...
function [summary, paths] = run_stage(cfg, base, age_share, T, status_path, out_dir)
rows = {};
path_rows = {};
row_ix = 0;
path_ix = 0;
for ic = 1:size(cfg.Candidates, 1)
    rho = cfg.Candidates(ic, 1);
    phi = cfg.Candidates(ic, 2);
    gamma = cfg.Candidates(ic, 3);
    row_ix = row_ix + 1;
    write_status(status_path, 'running', sprintf('Running T=%d candidate %d/%d.', T, ic, size(cfg.Candidates, 1)), cfg.RunTag, out_dir);
    try
        [candidate_summary, candidate_paths] = run_candidate(cfg, base, age_share, T, rho, phi, gamma, row_ix);
        ...
        rows(row_ix, :) = {T, row_ix, rho, phi, gamma, NaN, NaN, NaN, NaN, NaN, "error", ME.message};
    end
end

summary = cell2table(rows, 'VariableNames', {'T','candidate_id','rho','phi','gamma','max_abs_vote_resid','mean_abs_vote_resid','max_abs_log_price_move','max_abs_restriction','mean_homeownership','verdict','message'});
if isempty(path_rows)
    paths = table();
else
    paths = cell2table(path_rows, 'VariableNames', {'T','candidate_id','period','year','rho','phi','gamma','price','vote','vote_resid','pressure','restriction','homeownership','housing_demand','inner_iter'});
end
end

function [summary_row, path_rows] = run_candidate(cfg, base, age_share, T, rho, phi, gamma, candidate_id)
eqprev = base.eq_ss;
restriction_prev = 0;
...
for t = 1:T
    ...
    if cfg.LaggedPassThrough
        ...
        price = base.reference_price * exp(gamma * restriction);
        ...
        restriction_target = rho * restriction_prev + phi * pressure;
        ...
        path_ix = path_ix + 1;
        path_rows(path_ix, :) = {T, candidate_id, t, 2000 + t, rho, phi, gamma, price, last.vote, last.vote_resid, pressure, restriction, last.homeownership, last.housing_demand, 1};
        eqprev = last.eq;
    ...
    homeownership(t) = last.homeownership;
    path_ix = path_ix + 1;
    path_rows(path_ix, :) = {T, candidate_id, t, 2000 + t, rho, phi, gamma, price, last.vote, last.vote_resid, pressure, restriction, last.homeownership, last.housing_demand, cfg.PeriodIter};
    eqprev = last.eq;
...
summary_row = {T, candidate_id, rho, phi, gamma, max_abs_vote, mean_abs_vote, max_abs_log_price, max_abs_restriction, mean_homeownership, verdict, message};
end
...
fprintf(fid, '## Summary\n\n');
fprintf(fid, '| T | verdict | rho | phi | gamma | max abs vote residual | max abs log price move | mean homeownership |\n');
fprintf(fid, '|---:|---|---:|---:|---:|---:|---:|---:|\n');
for i = 1:height(summary)
    fprintf(fid, '| %d | %s | %.3f | %.3f | %.3f | %.6f | %.6f | %.6f |\n', ...
        summary.T(i), string(summary.verdict(i)), summary.rho(i), summary.phi(i), summary.gamma(i), ...
