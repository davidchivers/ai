function run_pf_continuation_0511(row_id, varargin)
% Perfect-foresight continuation probes for the T80 model-grid survivor.
%
% Each row is independent. The goal is to learn whether the best current
% economic family is easier under horizon continuation or shock-amplitude
% continuation before committing to a chained second wave.

p = inputParser;
p.addRequired('row_id');
p.addParameter('GridCsv', fullfile(pwd, 'model_pf_continuation_0511.csv'));
...
if height(row) ~= 1
    error('Expected exactly one continuation row for row_id %.0f, found %.0f.', row_id_num, height(row));
end
...
fprintf('Running PF continuation row %.0f: %s\n', row_id_num, run_tag);
