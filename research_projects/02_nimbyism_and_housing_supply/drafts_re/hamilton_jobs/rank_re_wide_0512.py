#!/usr/bin/env python3
"""Rank the 2026-05-12 wide T80 RE transition packet."""
...
BOUNDED_ROUTES = {"bounded_horizon_expectations"}
DIAGNOSTIC_ROUTES = {
...
def verdict_from_gap(gap, route):
    if route in {"partial_equilibrium_pf"}:
        if math.isfinite(gap):
            return "diagnostic"
        return "missing"
    if route in BOUNDED_ROUTES:
        if not math.isfinite(gap):
            return "missing"
        if gap <= 0.0002:
            return "bounded_paper_safe"
        if gap <= 0.001:
            return "bounded_usable"
        if gap <= 0.003:
            return "bounded_survivor"
        return "bounded_dead"
    if route in DIAGNOSTIC_ROUTES or "diagnostic" in route.lower():
        if not math.isfinite(gap):
            return "missing"
        if gap <= 0.001:
...
def main():
    ...
    parser.add_argument("--grid", default="model_re_wide_0512.csv")
    parser.add_argument("--out-dir", default="truth/re_wide_0512")
    ...
    fields = [
        ...
        "post_report_mode", "tail_years", "terminal_anchor", "terminal_blend_weight",
        "terminal_relaxation", "terminal_iter", "shock_amplitude", "pass_through",
        "ttb_proxy_lag_years", "vote_shift_mode", "vote_shift_horizon",
        "vote_block_length", "source_mode", "source_run_tag", "source_outer",
        "nore_tag", "hybrid_switch_period", "seed_blend_weight", "outer_iter",
        "path_update_method", "basis_dim", "basis_damping", "broyden_damping",
        "broyden_step_cap", "basis_step_cap", "basis_tail_weight",
        "basis_end_weight", "basis_focus_start", "basis_focus_end",
        "basis_focus_weight", "path_relaxation", "vote_scale",
        "max_abs_log_price_move", "state", "verdict", "best_gap",
    ...
    out_csv = out_dir / "re_wide_ranked_0512.csv"
    with out_csv.open("w", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=fields, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(ranked)

    out_md = out_dir / "re_wide_report_0512.md"
    with out_md.open("w", encoding="utf-8") as fh:
        fh.write("# Wide T80 RE Transition Search 2026-05-12\n\n")
        headline = [r for r in ranked if r["verdict"] in {"paper_safe", "usable"}]
        if headline:
            best = headline[0]
            fh.write(f"Best headline-clearing row: `{best['run_tag']}` gap `{best.get('best_gap','')}` verdict `{best['verdict']}`.\n\n")
        elif ranked:
            best = ranked[0]
