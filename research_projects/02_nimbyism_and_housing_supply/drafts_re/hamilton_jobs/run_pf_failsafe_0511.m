function run_pf_failsafe_0511(row_id, varargin)
% Eleven-hour perfect-foresight failsafe probes after T20/T40 continuation clears.

p = inputParser;
p.addRequired('row_id');
p.addParameter('GridCsv', fullfile(pwd, 'model_pf_failsafe_0511.csv'));
...
if height(row) ~= 1
    error('Expected exactly one failsafe row for row_id %.0f, found %.0f.', row_id_num, height(row));
end
...
fprintf('Running PF failsafe row %.0f: %s\n', row_id_num, run_tag);
...
function value = row_text_default(row, name, default_value)
if ~ismember(name, row.Properties.VariableNames)
    value = default_value;
    return
end
...
text_value = string(raw);
if strlength(strtrim(text_value)) == 0 || lower(text_value) == "nan"
    value = default_value;
else
