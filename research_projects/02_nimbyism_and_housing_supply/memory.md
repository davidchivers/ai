...
### Session: 2026-03-09 (anticipated-demographics RE extension setup)
- Added separate extension workspace at `projects/02_nimbyism_and_housing_supply/extension/`.
- Wrote benchmark note `extension/anticipated_demographics_re.md` for a no-coalition, forward-looking voting experiment based on the boom-driven future electorate.
- Wrote current-results note `extension/current_findings.md` summarizing the toy benchmark and the structural next step.
- Wrote coalition follow-up note `extension/static_bloc_coordination.md` covering:
  - simplest threshold coalition premium
  - better smooth within-period bloc-coordination extension
  - recommendation to sequence coalition work after the anticipated-demographics RE benchmark
- Added MATLAB files:
  - `extension/matlab/make_boom_projection_path.m`
  - `extension/matlab/export_structural_endpoints.m`
  - `extension/matlab/run_anticipated_demographics_re_toy.m`
  - `extension/matlab/run_anticipated_demographics_re_structural.m`
  - `extension/matlab/run_extension_pipeline.m`
- Added PowerShell entrypoint `extension/run_extension_pipeline.ps1` so the extension stack can be run automatically once raw `SS_iter*.mat` files are restored.
- Verified the toy benchmark runs in local MATLAB and writes lightweight outputs to `extension/results/`.
- Toy benchmark implication: anticipated boom demographics can materially front-load/amplify price pressure relative to a myopic voting benchmark, but the current numbers are illustrative rather than structurally calibrated.
- Structural scaffold currently stops with `missing_inputs` because the repo does not include legacy `.mat` inputs / endpoint objects (`nl_zbl.mat`, transition objects, saved steady-state endpoints).
- Automatic plan once inputs are restored: drop raw `SS_iter.mat` and `SS_iter_boom.mat` into `extension/input/` and run `extension/run_extension_pipeline.ps1`; the pipeline will export lightweight endpoints and run the structural benchmark if possible.
- Explicit reminder: do not keep generated `.mat` outputs in the repo; extension scripts were left writing only lightweight CSV/PNG artifacts.
