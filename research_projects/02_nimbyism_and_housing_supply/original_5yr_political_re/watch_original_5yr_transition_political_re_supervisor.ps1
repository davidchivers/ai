...
Write-Log "Historical transition supervisor started."
...
do {
    ...
    Write-Log "Launching historical transition workflow run."
    $proc = Start-Process powershell -ArgumentList @(
        "-NoProfile",
        "-ExecutionPolicy", "Bypass",
        "-File", (Join-Path $scriptDir "run_original_5yr_transition_political_re_workflow.ps1"),
        "-LiveRoot", $LiveRoot,
        "-MatlabExe", $MatlabExe
