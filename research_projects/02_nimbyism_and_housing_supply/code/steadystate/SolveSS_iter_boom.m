function [distance,a_price,rbPos,totalvote,debtstock,vote_diagnostics] = SolveSS_iter_boom(x,vote_params)
...
dz=z(2)-z(1);
    ...
    if isempty(transition_file)
        error('SolveSS_iter_boom:MissingTransitionMatrix','TransitionMatrix.mat is not on the MATLAB path.');
    end
    ...
    if ~exist('y_mid','var') || ~exist('z_lifecycle','var')
        ...
        if isempty(transition_builder)
            error('SolveSS_iter_boom:MissingTransitionInputs', ...
...
switch lower(char(vote_params.mode))
    ...
    otherwise
        error('SolveSS_iter_boom:UnknownVoteMode','Unknown vote mode "%s".',vote_params.mode);
end
end
