function results_table = run_transition_re_validation_pack()
% Short validation pack comparing the config-11 control against the best
% non-baseline candidate from the candidate-selection follow-up.
...
summary_path = fullfile(this_dir, 'transition_re_validation_summary.csv');
results_path = fullfile(this_dir, 'transition_re_validation_results.mat');
live_summary_path = fullfile(this_dir, 'transition_re_validation_summary_live.csv');
live_results_path = fullfile(this_dir, 'transition_re_validation_results_live.mat');
...
cases = build_cases(candidate_table);
...
if isfile(live_results_path)
    ...
    if isfield(resume_data, 'raw_results') && isfield(resume_data, 'cases') && ...
        ...
        fprintf('Resuming validation checkpoint: %d / %d cases already have results.\n', ...
            completed_count, numel(cases));
    else
        fprintf('Ignoring incompatible validation checkpoint and starting fresh.\n');
    end
end
...
fprintf('=== Transition RE Validation Pack ===\n');
fprintf('Cases: %d\n', numel(cases));
...
function cases = build_cases(candidate_table)
ok_rows = candidate_table(strcmp(candidate_table.status, "ok"), :);
if isempty(ok_rows)
    error('No ok rows found in candidate-selection summary.');
end

baseline_row = ok_rows(strcmp(ok_rows.case_name, "config11_global_control"), :);
if isempty(baseline_row)
    error('Baseline control row config11_global_control not found in candidate-selection summary.');
end

variant_rows = ok_rows(ok_rows.case_name ~= "config11_global_control", :);
if isempty(variant_rows)
    error('No non-baseline candidate rows found in candidate-selection summary.');
end

best_variant_row = variant_rows(1, :);
validation_max_iter = 8;

cases = [ ...
    case_from_row(baseline_row(1, :), "baseline_control_validation", validation_max_iter)
    case_from_row(best_variant_row(1, :), "best_variant_validation", validation_max_iter)
    ];
end

function case_info = case_from_row(row, case_name, validation_max_iter)
case_info = struct();
