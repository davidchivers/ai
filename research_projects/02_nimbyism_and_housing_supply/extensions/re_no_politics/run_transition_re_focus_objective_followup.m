function results_table = run_transition_re_focus_objective_followup()
% Narrow follow-up that explicitly prioritizes focus-region / worst-period
% improvement rather than letting global residual gains dominate.

this_file = mfilename('fullpath');
this_dir = fileparts(this_file);
project_root = fileparts(fileparts(this_dir));
baseline_dir = fullfile(project_root, 'code', 'steadystate');
summary_path = fullfile(this_dir, 'transition_re_focus_objective_summary.csv');
results_path = fullfile(this_dir, 'transition_re_focus_objective_results.mat');
live_summary_path = fullfile(this_dir, 'transition_re_focus_objective_summary_live.csv');
live_results_path = fullfile(this_dir, 'transition_re_focus_objective_results_live.mat');
...
if isfile(live_results_path)
    ...
    if isfield(resume_data, 'raw_results') && isfield(resume_data, 'cases') && ...
        ...
        fprintf('Resuming focus-objective checkpoint: %d / %d cases already have results.\n', ...
            completed_count, numel(cases));
    else
        fprintf('Ignoring incompatible focus-objective checkpoint and starting fresh.\n');
    end
end
...
fprintf('=== Transition RE Focus-Objective Follow-Up ===\n');
fprintf('Cases: %d\n', numel(cases));
...
for i = 1:numel(cases)
    ...
    try
        ...
        fprintf('  focus gap tol = %.6g, focus slack = %.6g, targets = %d, half-width = %d\n', ...
            case_info.focus_gap_improvement_tol, case_info.focus_residual_slack, ...
            case_info.max_targeted_periods, case_info.target_block_half_width);
...
max_iter = 6;
cases = [ ...
    struct('case_name', "config11_focus_relaxed", 'base_profile', "config11_p2_b4", 'line_search_scales', [0.10 0.05 0.02 0.01], 'block_sweep_passes', 2, 'max_blocks_per_pass', 4, 'max_iter', max_iter, 'candidate_selection_mode', "focus", 'greedy_block_accept', false, 'candidate_residual_slack', 5e-5, 'focus_gap_improvement_tol', 5e-5, 'focus_excess_improvement_tol', 1e-5, 'focus_residual_slack', 5e-4, 'max_targeted_periods', 3, 'target_block_half_width', 1)
    struct('case_name', "aggressive_p2_b8_focus_relaxed", 'base_profile', "aggressive_p2_b8", 'line_search_scales', [0.20 0.10 0.05 0.02], 'block_sweep_passes', 2, 'max_blocks_per_pass', 8, 'max_iter', max_iter, 'candidate_selection_mode', "focus", 'greedy_block_accept', false, 'candidate_residual_slack', 5e-5, 'focus_gap_improvement_tol', 5e-5, 'focus_excess_improvement_tol', 1e-5, 'focus_residual_slack', 5e-4, 'max_targeted_periods', 3, 'target_block_half_width', 1)
    struct('case_name', "aggressive_p2_b8_focus_loose_slack", 'base_profile', "aggressive_p2_b8", 'line_search_scales', [0.20 0.10 0.05 0.02], 'block_sweep_passes', 2, 'max_blocks_per_pass', 8, 'max_iter', max_iter, 'candidate_selection_mode', "focus", 'greedy_block_accept', false, 'candidate_residual_slack', 5e-5, 'focus_gap_improvement_tol', 5e-5, 'focus_excess_improvement_tol', 1e-5, 'focus_residual_slack', 1e-3, 'max_targeted_periods', 3, 'target_block_half_width', 1)
    struct('case_name', "aggressive_p2_b8_focus_wider_target", 'base_profile', "aggressive_p2_b8", 'line_search_scales', [0.20 0.10 0.05 0.02], 'block_sweep_passes', 2, 'max_blocks_per_pass', 8, 'max_iter', max_iter, 'candidate_selection_mode', "focus", 'greedy_block_accept', false, 'candidate_residual_slack', 5e-5, 'focus_gap_improvement_tol', 5e-5, 'focus_excess_improvement_tol', 1e-5, 'focus_residual_slack', 5e-4, 'max_targeted_periods', 4, 'target_block_half_width', 2)
    struct('case_name', "aggressive_p2_b8_hybrid_gap_guard", 'base_profile', "aggressive_p2_b8", 'line_search_scales', [0.20 0.10 0.05 0.02], 'block_sweep_passes', 2, 'max_blocks_per_pass', 8, 'max_iter', max_iter, 'candidate_selection_mode', "hybrid_focus", 'greedy_block_accept', false, 'candidate_residual_slack', 5e-5, 'focus_gap_improvement_tol', 5e-5, 'focus_excess_improvement_tol', 1e-5, 'focus_residual_slack', 5e-4, 'max_targeted_periods', 3, 'target_block_half_width', 1)
    ];
end
