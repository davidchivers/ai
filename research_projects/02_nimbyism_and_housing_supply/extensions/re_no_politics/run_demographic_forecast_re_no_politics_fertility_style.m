function results = run_demographic_forecast_re_no_politics_fertility_style()
% Entry point for the transition-path RE no-coalition experiment using the
% fertility-style whole-path fixed-point relaxation.
...
fprintf('=== Transition RE No-Coalition Forecast (fertility-style loop) ===\n');
...
if ~isempty(missing)
    ...
    error('Transition RE fertility-style runner failed path validation.');
end
...
if isfile(previous_results_path)
    ...
    if isfield(loaded, 'results') && isfield(loaded.results, 'final_price_path') && ...
            numel(loaded.results.final_price_path) == numel(price_path_guess)
        price_path_guess = loaded.results.final_price_path(:);
        warm_start_source = "transition_re_no_politics_results.final_price_path";
    end
end

fprintf('Warm start source: %s\n', warm_start_source);
fprintf('\nRunning solve_transition_re_no_politics with fertility-style outer loop...\n\n');

results = solve_transition_re_no_politics(price_path_guess, demographic_path, params);
results.warm_start_source = warm_start_source;
save transition_re_no_politics_fertility_style_results results
...
fprintf('Final max abs RE price gap: %.4f\n', results.update_diagnostics.max_abs_gap);
fprintf('Final max abs update: %.4f\n', results.iteration_log(end).max_abs_update);
fprintf('Final accepted update label: %s\n', results.iteration_log(end).accepted_update);
...
fprintf('Final price range: [%.4f, %.4f]\n', min(results.final_price_path), max(results.final_price_path));
fprintf('Results saved in transition_re_no_politics_fertility_style_results.mat\n');
end
