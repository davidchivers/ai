function summary = run_transition_re_no_politics_k_step_ladder(max_k)
% Solve the NIMBY no-politics RE problem on progressively longer truncated
% horizons, starting from k = 1 and warm-starting each longer run from the
% previous shorter-horizon solution.
...
current_guess = [];
fprintf('=== NIMBY RE k-step ladder ===\n');
...
for k = 1:max_k
    ...
    try
        ...
        if is_unstable_k_step(summary_rows(k), params)
            fprintf('  stopping after k=%d because the bounded-horizon run looks unstable.\n', k);
        ...
        summary_rows = summary_rows(1:k);
        all_results = all_results(1:k);
        save(fullfile(this_dir, 'transition_re_no_politics_k_step_ladder_results.mat'), ...
...
writetable(summary, fullfile(this_dir, 'transition_re_no_politics_k_step_ladder_summary.csv'));
save(fullfile(this_dir, 'transition_re_no_politics_k_step_ladder_results.mat'), ...
    'summary', 'all_results', 'anchor_source', 'anchor_price_path');
end

function params = default_k_step_params()
params = struct();
params.max_iter = 25;
params.tol = 1e-4;
params.damping = 0.25;
...
demographic_path_k.description = sprintf('%s Truncated to first %d periods for bounded RE ladder.', ...
