...
for iter = 1:params.max_iter
    ...
    if use_fertility_style_outer_loop(params)
...
if params.compute_political_path
    sim.political = compute_transition_political_path_local( ...
        sim.density_by_period_age, preference_sign_by_period_age, env, price_path, ...
        params.coalition_params, params.save_political_details);
    if ~params.save_period_details
...
cache_key = sprintf(['terminal=%s|terminal_fixed=%.8f|policy=%s|policy_ref=%s|policy_fixed=%.8f|' ...
    ...
    'housing_weight=%.8f|housing_anchor=%.8f|path=%s'], ...
    ...
    housing_adjustment_weight, housing_adjustment_anchor_price, price_key);
end
...
function political = compute_transition_political_path_local(density_by_period_age, preference_sign_by_period_age, env, price_path, coalition_params, save_political_details)
...
for t = 1:T
    ...
    pref4 = zeros(env.I, env.J, env.K, age_n);

    for age_idx = 1:age_n
        dens4(:, :, :, age_idx) = density_by_period_age{t, age_idx};
        pref4(:, :, :, age_idx) = preference_sign_by_period_age{t, age_idx};
        political.equal_weight_vote_by_age(t, age_idx) = sum(dens4(:, :, :, age_idx) .* pref4(:, :, :, age_idx), 'all');
    end

    period_coalition_params = coalition_params;
    period_coalition_params.house_price = price_path(t);
    stats = compute_coalition_vote(dens4, pref4, housing_grid, liquid_grid, age_grid, period_coalition_params);
