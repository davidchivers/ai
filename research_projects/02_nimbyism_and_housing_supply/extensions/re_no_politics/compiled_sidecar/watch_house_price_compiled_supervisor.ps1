...
$workflowPath = Join-Path $scriptDir 'house_price_compiled_supervisor_workflow.ps1'
...
function Start-SupervisorRun {
    $args = @(
        ...
        '-SteadyStatePackName', $SteadyStatePackName,
        '-PriceMin', $PriceMin,
        '-PriceMax', $PriceMax,
        '-PriceCount', $PriceCount,
        '-TransitionPassPackName', $TransitionPassPackName,
        '-TransitionRePackName', $TransitionRePackName
    )
    if ($SkipBuild) {
        $args += '-SkipBuild'
    }
