...
%   supply_params = struct with fields:
%     .Hbar   — baseline housing stock
%     .Pbar   — reference price (normalisation)
%     .eta_s  — supply elasticity (0=fixed stock, Inf=fixed price)
%
% In steady state the price is constant, so RE is trivially satisfied:
% households know the price stays at a_price forever.

ensure_external_matlab_data_paths();

% Supply-side parameters (defaults calibrated from political SS)
...
% Vector for lifecycle age component
iage=0;
...
% NOTE: No price-preference perturbation needed — voting removed.
...
%% Housing market clearing (replaces voting equilibrium)

% Aggregate housing demand from the stationary distribution
Hdemand = sum(aaaa .* dens4, 'all');

% Elastic supply: Hsupply = Hbar * (a_price / Pbar)^eta_s
