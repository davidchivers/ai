...
1. `_shared/memory/RESEARCH_STYLE.md`
2. `_shared/memory/academic_voice.md`

Treat those files as the repository-specific override for style and sentence-level judgment.

## Instructions

### Step 0: Load repository voice rules

- Calibrate first to the user's own strong prior papers or published work when available.
- Treat anti-AI cleanup as writing-quality control, not detector evasion.
- Do not use em dashes in academic prose.
- Avoid stock AI patterns: throat-clearing openers, novelty inflation, `not just X, but Y` templates, synonym cycling, and empty signposting.
